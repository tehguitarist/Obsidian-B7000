...
private:
    ...
    // ⚠ THAT LAST SENTENCE IS TRUE **OF s196** AND IS NO LONGER TRUE OF THE SHIPPED TABLE — s199
    // re-points the Cut row's entries 0-3 (see the s199 block below).  Read it as the history of
    // s196's scope, not as a live claim about what Cut currently does.
    //
    ...
    //       ⚠ SUPERSEDED s199: the Cut row DOES ship, on a narrower arm (entries 0 and 2), after
    //       the two objections in this sentence were re-measured — the +2.67 turns out to be
    //       unavoidable at ANY arm that fixes that cell, and the 19 cells cost ~0.07 dB rendered.
    ...
    // and there is NO capture between cleanFrac 0.0242 and 0.4811 at flat or boost, nor between
    // 0.0242 and 0.2284 at CUT.  Inside those bands this table ADDS cut where it REMOVES it at the
    // corner -- up to +4.26 dB (flat), +7.42 dB (boost) and, since s199, +2.67 dB (cut) at
    // cleanFrac ~ 0.21, opposite-signed to the corner fix at 3 of 3 rows.  Those are settings a
    // player can dial and nothing on disk can grade (s185's CAPTURED-vs-DIALLABLE split).
    // ⛔ "no capture moved" is a statement about the GRID, not about the law.
    //
    ...
    //
    // ⭐⭐⭐ SESSION 199 — THE CUT ROW SHIPS, ENTRIES 0 AND 2 ONLY.  Action-list item (8), the
    // residual s196 knowingly left.  ✅✅ USER DECISION 2026-08-10.  Three things decided it, and
    // the FIRST one refutes the item's own proposed lever:
    //
    // ⛔⛔ (a) "MOVE JUST THE ONE ENTRY INSTEAD OF THE ROW" DOES NOT AVOID THE COLLATERAL s196
    //     DECLINED ON.  That collateral is the +2.67 dB this table adds inside the ungraded band,
    //     and it is generated by the Cut x DRIVE 0.50 entry ALONE: dK -5.086 x S_min(-0.525) =
    //     +2.67.  Measured IDENTICAL across all three candidate arms (all-rows, entry-2-only,
    //     entries-0+2).  It is a property of the CELL, not of the row's width, so no choice of
    //     which OTHER entries to move can reduce it.  ⇒ narrowing buys nothing on that axis, and
    //     the s196 objection is ACCEPTED here rather than dodged.
    //
    // ⭐⭐ (b) THE "19 CAPTURED CELLS MOVE" OBJECTION IS NEAR-EMPTY WHEN RENDERED.  s196 recorded
    //     the all-rows arm's mixed cells costing -0.00/+0.01 dB; re-measured on THIS arm the three
    //     largest movers cost +0.07 / +0.06 / -0.00 dB of AREA error against applied-cut changes of
    //     +2.39 / +1.78 / -1.26 dB.  The composite dilutes it.  ⇒ the scope observation was never a
    //     measured cost, and quoting the cell COUNT as the price overstates it.
    //
    // ⛔⛔ (c) DRIVE 1.00's CORRECTION IS REFUTED BY RENDER, WHICH IS WHY ENTRY 4 IS UNCHANGED.
    //     Applied -1.91 dB it realises +0.14 dB of AREA error -- ratio 0.075 and WRONG-SIGNED --
    //     where drv 0.00 and 0.50 realise 0.345 / 0.369, inside s185's measured 0.304-0.689
    //     composite-dilution range.  ⚠ n = 3 sweeps: a FLAG, not a mechanism, and it is NOT
    //     attributed (the pedal's null migrates at high drive, s151).  Dropping it is what makes
    //     this arm beat the s196-declined all-rows arm on BOTH axes: corner AREA gain -2.16 dB vs
    //     -2.02, 15 captured cells moved vs 19, ungraded excursion identical at +2.67.
    //
    // RENDERED RESULT at the two cells this ships (GATE BT's own curves/reader, 3 sweeps each):
    //     drv 0.50  AREA err 2.41 -> 0.62   POINT err 2.60 -> 3.84    applied -4.84 dB
    //     drv 0.00  AREA err 0.81 -> 0.44   POINT err 0.99 -> 0.79    applied -1.08 dB
    // ⚠⚠ QUOTE THE POINT COLUMN WITH IT: drv 0.50 gets 1.24 dB WORSE on the bottom-of-the-notch
    // reading while getting 1.79 dB better on the area one.  That is s153's metric trade, which
    // s196's USER DECISION already reversed AT THE CORNER for this table -- so this ships under the
    // governing metric, not in spite of a metric.  ⛔ It is NOT a demonstration that area is what
    // the ear tracks; s153's grounds (2) and (3) are still untouched and still argue the other way.
    // ⭐ Concretely: the shipped model's notch BOTTOM was already near-right (14.67 vs the pedal's
    // 13.92 at sweep_drv_-12) and its notch was too WIDE (area 11.73 vs 9.08).  This fixes the
    // width and overshoots the bottom (11.34).  Depth and width are two coordinates, not one.
    //
    // ⛔ DRIVE 1.00's OWN CORNER ERROR IS KNOWINGLY LEFT AT +1.91 dB, by (c).
    // ⛔ A GENUINELY NARROWER LEVER EXISTS AND IS DELIBERATELY NOT TAKEN: at one cell the cut is
    // `base + K*S`, so pinning the corner leaves a 1-D (dBase, dK) family, and 7 points on it beat
    // pure-K on BOTH axes (e.g. dK -4.50 / dBase -0.56 takes the ungraded excursion +2.67 -> +1.80).
    ...
    // that exactly (-8.61 = mean(-7.87, -9.34); -9.50 = mean(-9.34, -9.65)).  Moving entry 0 and 2
    // without re-deriving them would leave the row self-inconsistent, so they are recomputed and the
    // identity holds on the ROUNDED values too (-11.72 = mean(-9.01, -14.43) exactly; -12.04 =
    // mean(-14.43, -9.65) exactly).  ⭐ Rounding to this table's 2 dp costs 0.0036 dB against the
    // exact candidate that was rendered -- inside the 0.02 dB the rounding itself is worth.
    static constexpr double kNotchMixK[3][5] = {
        {  -9.01, -11.72, -14.43, -12.04,  -9.65 },  // Cut   — s199 e0/e2 (was -7.87/-8.61/-9.34/-9.50); e4 held, see (c)
