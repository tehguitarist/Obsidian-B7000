# Phase 9 — gap log (the per-gap and per-session narrative)

> Split out of `docs/phase9-validation.md` in session 89 (2026-07-31). **Verbatim, nothing
> edited** — the parent doc had grown to 10,981 lines, of which this was ~9,000, and the
> method/backlog sections were unreadable underneath it.
>
> **This is the DETAIL, not the plan.** For status and next steps read `CLAUDE.md` and
> `docs/phase9-validation.md` §0. For the generalisable lessons read
> `.claude/rules/measurement-discipline.md`. Come here for the evidence behind a specific gap.
>
> ⚠⚠ Every "the pedal" below means the **Neural DSP emulation** — see
...
### ⭐⭐ A3 step 23 — session 64's "the width is only reachable at absurd values" does NOT survive the corrected calibration. The width IS reachable at ±1 decade, and the RENDER confirms it: 1.51/1.72/1.90× → 0.87/1.29/1.03× (session 66)

Session 65's next-step (a): re-run `attack_shape_screen --fit --best` at the corrected calibration
and re-read session 64 item 7's verdict rather than re-quoting it. The verdict is **refuted**, and
the refutation lands on the real render, not just in the screen.

Tooling + analysis only. **NOTHING in `src/` or `tests/` changed**; **ctest 16/17**, the
pre-existing session-44 `OSValidationTest` with identical numbers
(`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`). **Baseline verified FIRST** — GATE B recovers
synthesised widths to ≤0.03 %, and GATE C reproduces session 65's calibration to the digit
(depth offsets `+0.32 / −0.64 / +0.06 dB`, width scales `1.028 / 1.124 / 1.022`, requirement
`316.25/15.25/80.1 · 327.75/32.07/30.4 · 333.75/16.07/73.5`) before anything was touched.

#### 1. ⛔ THE REFUTATION — and the SEARCH BOX was the thing carrying the old answer

Session 64 concluded the nine notch numbers were reachable **only** at `R7 ×572` (200 k → 114 MΩ),
`C6 ×62`, `C5/C9/C7 ×0.02–0.03`, with the tap on its bound — i.e. reachable, but only as session 62
item 4's "reachable via broadband nonsense" control, not as a candidate. At the corrected
calibration that is simply false. The frontier (tap pinned, notch scored alone) at the **default
±1 decade box**:

| w_f0 | f0 rms (bins) | width rms | spread Hz | widths cut/boost/flat | worst shared |
|---|---|---|---|---|---|
| 1.0 | 0.52 | 0.55 | 13.5 | 74.5 / 30.8 / 78.2 | ×8.6 |
| 10.0 | 0.30 | 0.56 | 13.2 | 76.9 / 33.1 / 73.8 | ×8.3 |
| **30.0** | **0.07** | **0.82** | **18.0** | **74.0 / 32.4 / 81.0** | **×7.6** |
| 100.0 | 0.06 | 1.27 | 17.8 | 90.3 / 32.2 / 86.0 | ×4.6 |

against `want 80.1 / 30.4 / 73.5` and a required f0 spread of 17.5 Hz — **nine numbers met, nothing
on a bound, no element moved more than ~8×.** Session 64's corresponding rows were f0 rms
0.67–8.0 bins and width rms 2.14–4.69 (21–47 %), with the spread collapsing to 1.5–5.0 Hz at the
weights that reached the width — the "switch the throws off rather than trade" signature. That
signature is **gone**.

⚠⚠ **AND THE MECHANISM OF THE OLD ANSWER IS A SEARCH SETTING, WHICH IS WHY IT IS WORTH RECORDING.**
`best_point()` searched box 3.0 **only**, justified in its own header as *"the sweep showed 1.0 was
still constraining"* — a statement true of session 64's box sweep, which was run against the
requirement transferred through the **wrong-`--grunt` calibration**. The box is now **SWEPT**, and
the two halves of the sweep separate completely:

```
  box  1.0  w_f0 1 … 100   f0 rms 0.06–0.52 bins   worst shared element  ×4.6 – ×8.6
  box  3.0  w_f0 1 … 100   f0 rms 0.31–1.30 bins   worst shared element  ×120 – ×790
```

Box 3.0 is **worse on the ranking at every weight** and gets there through three decades of
component change. ⭐ **GENERAL: a SEARCH SETTING justified by a measurement is as perishable as any
number derived from that measurement. When the measurement is corrected the setting has to be
re-derived, not carried forward.** The fix is to remove the choice — the tool now sweeps the box and
prints `worst shared multiplier` and `n on bound` per row, so the next session can see the trade
instead of inheriting it.

#### 2. ⚠ A RANKING DEFECT IN MY OWN FIRST VERSION, caught before it picked the candidate

The first swept run chose `box 1.0 / w_f0 = 100` because its f0 rms was **0.06 bins against 0.07**
— and that choice cost **12.7 % width error instead of 8.2 %**, plus a 4.02 dB depth shortfall at
cut. 0.01 bins is **0.06 Hz**; the pedal's f0 is quoted on the 5.86 Hz measurement grid, so that
difference does not exist in the data. Search noise in the tightest term was outvoting a real
difference in the next one. The key is now quantised at `F0_TIE_BINS = 0.25` (a quarter bin —
anything below it is equally "on the bin"), and width breaks the tie. ⭐ **GENERAL: quantise a
ranking key to the resolution of the quantity it ranks.** With that fixed the winner is
`box 1.0 / w_f0 = 30`, which is also the point the frontier table above already pointed at.

#### 3. ⭐⭐ THE ARBITER — the render gate, and the width residual largely CLOSES

`attack_render_gate.py --both --fits-json analysis/reports/s66_best.json`
(`analysis/reports/s66_render_gate.json`). GATE 0 CONDITION passes with **0 flags differing** from
`captures.render_args`, GATE 3 BLEED gives clean coefficient 0.000e+00 / OD 1.000000, GATE 4
CONVERGED passes on all three variants.

| | PEDAL | DRAWN | PROPOSAL (s63) | **CANDIDATE (s66)** |
|---|---|---|---|---|
| f0 cut/boost/flat, Hz | 316.4 / 328.1 / 334.0 | 398.4 ×3 | 316.4 / 328.1 / 334.0 | **316.4 / 328.1 / 334.0** |
| f0 spread | 17.58 | 0.00 | 17.58 | **17.58** |
| depth, dB | 14.93 / 32.70 / 16.01 | 3.26 / 3.08 / 3.21 | 14.42 / 33.26 / 15.79 | 15.36 / 29.81 / 16.44 |
| **width (interp), Hz** | **77.9 / 27.1 / 71.9** | 153.8 / 156.1 / 154.5 | 118.0 / 46.6 / 136.6 | **67.9 / 35.0 / 74.0** |
| **width ÷ pedal** | 1.00 | 1.97 / 5.76 / 2.15 | 1.51 / 1.72 / 1.90 | **0.87 / 1.29 / 1.03** |
| h boost median / slope | +8.63 / +1.23 | +0.84 / +3.19 | +8.49 / +0.56 | +8.54 / +0.06 |
| h cut median / slope | −2.38 / −1.38 | −0.34 / −0.93 | −2.30 / −0.00 | −2.28 / −0.31 |
| h boost / cut resid rms | floor 0.204 | 7.55 / 2.01 | 0.66 / 0.83 | 0.67 / 0.86 |
| h cut spread | 2.62 | 0.79 | 5.14 | **7.60** |
| boost level trend −18 → −36 | +4.43 | +0.21 | +1.28 | **+0.17** |

⭐ **The width excess that has been open since session 63 item 5(b) largely closes**: 1.51/1.72/1.90×
the pedal's becomes **0.87 / 1.29 / 1.03×**, with cut now marginally *narrower* than the pedal and
flat within 3 %. And f0 stays **identical to the bin at all three throws**, spread 17.58 Hz — the
requirement session 61 proved the drawn topology cannot reach at all (0 of 782 draws).

⭐ **It is not bought back out of the depth statistic, which was the obvious way for it to be an
artefact.** Session 63 item 6(c) had to replace a fixed −6 dB contour with half-depth precisely
because a deeper null reads wider on the former; the residual worry is the mirror — that a shallower
null reads narrower. Measured, there is no such coupling here: **cut and flat both got DEEPER
(14.42 → 15.36, 15.79 → 16.44) and NARROWER**, while boost got shallower and narrower. A
depth-driven artefact would move all three the same way.

#### 4. ⛔ THE CANDIDATE IS NOT SHIPPABLE, AND IS NOT PROPOSED — it is a REACHABILITY result

What it costs is as clear as what it buys, and the render gate prints both:

* **The broadband SHAPE gets worse where the proposal was better.** Boost slope **+0.56 → +0.06**
  against the pedal's +1.23 (session 65's headline improvement is given back), and **cut's spread
  goes 5.14 → 7.60** against the pedal's 2.62. Residual rms is flat (0.66 → 0.67 boost, 0.83 → 0.86
  cut) — so this is a **trade, not an improvement**: the coarse table shows the candidate
  undershooting through 41–252 Hz (+8.32 → +7.46 where the pedal holds ~+8.6) and overshooting above
  the notch (+8.94 / +9.18 / +9.02 at 351.6 / 380.9 / 421.9 Hz against the pedal's +8.18 / +8.50 /
  +8.46).
* **The level trend moves AWAY from the pedal**: +1.28 → +0.17 dB against the pedal's +4.43. Session
  65 item 4 reversed session 63's reading of this quantity and left the model compressing *less*
  than the pedal; this widens that gap rather than closing it.
* **`attackTapRa` rests on its bound** (×0.1, 470 k → 47 k) — unidentified, not a value.
* **Five schematic-verified ladder values move by 2.4–7.6×** (`R7 ×7.59`, `R12 ×0.39`, `R14 ×2.36`,
  `C9 ×0.235`, `C6 ×4.83`, plus `C7 ×0.228` on top of its existing 147× departure). That is a
  capture-vs-document claim of the `trebleC7` / `c21R` class, times five.
* **The 63-capture matrix has not judged it**, and it judged the plain proposal *against* shipping
  (session 63 item 0: `OD ex gain-n12` 1.903 → 2.218).

⇒ **the deliverable is that the width residual is NOT a structural limit of the two-pole topology.**
Session 64 could not distinguish "unreachable" from "search-limited" and correctly declined to call
it either; at the corrected calibration it is neither — it is *reachable at ordinary component
ratios*, and what remains open has moved from **width** to **broadband shape + level trend**.

#### 5. WHAT ELSE MOVED, AND WHAT DID NOT

* **The family table (`--fit`, joint objective, tap free).** Costs fall from session 64's 3.22–3.65
  to **2.03–2.18**, and the ordering changes in a way worth keeping: session 64 recorded *"17 dof
  beats 12 dof by 0.4 %"*, whereas now the over-parameterised 17-dof family **loses** to
  `{R12, C9}` at 12 dof (2.1169 vs 2.0311) because its broadband is worse (2.6037 vs 2.1221). Every
  family still misses badly on flat's f0 (312–316 Hz against 333.75) — **which is arbitration, not
  reachability**: the frontier reaches the same numbers once the non-interacting groups are scored
  separately (session 62 item 4's rule, doing its job again).
* **The Cp / J201-drain-pole diagnostic is UNCHANGED in conclusion and changed in every number**:
  ...
  (session 64: 3.1114 → 3.1114). So the constraint is still not the J201 drain pole. A conclusion
  that survives a 4.7× change in the residual it was drawn from is worth more than the number was.
* **The box sweep still prints "NOT saturated"** (cost spans 0.79 across 6× of widening, and
  non-monotonically — box 3.0 scores *worse* than box 2.0, which is DE failing to converge in a
  bigger space, not the objective). ⚠ That verdict is now **moot rather than informative**: the
  saturation test exists to separate "unreachable" from "search-limited", and the requirement is
  already met inside the smallest box tested.
* **GATE C still reads CHECK** (±16 % of width out-of-sample), so `attack_shape_screen` remains a
  LEVER FINDER and `attack_render_gate` remains the arbiter — which is exactly why item 3 above
  exists and why no number in item 1 is quoted as a match on its own.

#### 6. ⚠ ARTEFACT HYGIENE

`analysis/reports/s66_shape_screen.json` was written by the **pre-edit** `best_point()` (box fixed
at 3.0), so its `best` block is stale. Rather than leave a plausible-looking object in a file for a
future session to read, that key has been replaced in place by
`{"SUPERSEDED_BY": "analysis/reports/s66_best.json", "why": …, "stale_object": …}` — the comparison
is kept, the trap is not. Its `frontier`, `fits` and `scale_diagnostic` blocks are unaffected by the
edit and are what items 1 and 5 quote.

### ⭐⭐ A3 step 24 — the drive-noon ATTACK pair ANSWERS session 67's question in the NEGATIVE: cut's broadband-shape anomaly is the NON-COINCIDENT-NULL DIPOLE, not a network defect. Plus a GRUNT control that bounds the whole drive-dependence question, and two tool defects — one of which rejected a reference capture (session 68)

**Deliverables:** `docs/final-capture-window.md` (the complete forward-looking capture list — the
pedal is available ~5–6 more days), new `analysis/gen_notch_sweep.py` +
`analysis/notch_sweep_48k.wav`, and four fixes/additions to `analysis/attack_notch_probe.py`.
**Nothing in `src/` or `tests/` changed.** Baseline verified first; the drive-min JSON is a **strict
superset** (1454 shared values, 0 differing, 4 new keys).

**1. The captures.** All four landed in `analysis/captures/`: the two requested
(`level-1700_attack-{boost,cut}_base-od.wav`) plus two **bonus GRUNT** files at the same operating
point (`level-1700_grunt-{boost,flat}_base-od.wav`). 48 kHz / 83.700 s / float32; peaks 0.42–0.70;
each `render_args` differs from `level-1700_base-od.wav` in exactly one flag.

**2. ⚠⚠ DEFECT — the flat-topping gate was a false-positive generator and it REJECTED A REFERENCE
CAPTURE.** The test was ">16 consecutive samples above 0.985×peak". It killed the run on
`level-1700_attack-cut_base-od.wav` at 17 — at a peak of **0.415**, 7.6 dB below full scale, where
nothing can be clipping. A sine spends ~5.5 % of its period above 98.5 % of its own peak, so at the
20 Hz end of the log sweep that is ~30 samples at 48 kHz. Measured: all six bleed-free captures put
their longest loose run in `sweep_drv_-6` with the samples **still curving 1.3–1.5 % of peak**, and
**`level-1700_base-od.wav` — in the matrix since session 22 — scores 19.** Rebuilt on the real
signature (session 24's 14 lost files were **pinned at 0.98850**): a tight pin threshold
(0.9995×peak) **plus** a near-full-scale peak. Separation measured — worst clean run **4**, clipped
mutations **20 / 86 / 120**. ⭐ **Mutation-tested, and it failed my first attempt**: gating on
curvature *within the loose run* stays ~1.5 % even for a hard-clipped signal, because that window
spans the approach to the plateau. A plateau below full scale is the pedal's own rail limiting and is
reported, never rejected.

**3. ⚠ DEFECT — the verdict was NARRATED and printed "ATTACK MOVES THE NULL" above a 0.0 Hz spread**
(fourth occurrence of the session-34 trap). Now derived, and it states the opposite when the data
does. **And a WINDOW-SWALLOW guard was needed:** `grow()` assumes "a narrow notch on a flat
background", so when `h` has a broad slope the walk never stops — boost's window came out
**0.0–1154.3 Hz**, making the quoted **+4.64 dB "broadband" median** really the 1154–1600 Hz
remainder while `h` ran **+9.76 → +4.16 dB**. Flagged above 40 % of the band.

**4. ⭐⭐ THE ANSWER — cut's 350–520 Hz structure is an ARTEFACT OF THE DIFFERENCE.** Bleed-free at
drive noon by the identical mechanism, `h` cut reads **median −1.99 dB, spread 0.90 dB over the full
80–1600 Hz band**, its measured window collapses to **287–375 Hz** (from 269–521), and it **passes
the shared-421.9 Hz cancellation check (0.26 dB)** which it **failed at drive min (1.16 dB)**.
⭐ **The mechanism is the dipole, and it predicts the observed asymmetry.** At drive min the nulls sit
at **316.4 / 328.1 / 334.0 Hz**, so `h = throw − flat` contains **two offset nulls**, with structure
width scaling as the separation: cut is **17.6 Hz** apart from flat → window **252 Hz**; boost is
**5.9 Hz** → window **100 Hz** (3.0 vs 2.5). **That is why cut and not boost fails the 421.9 Hz
check** — an asymmetry sessions 60–66 recorded as unexplained. ⇒ **session 60 item 11 / 61 item 5 /
63 item 5(a) / 66 item (a) are ONE item and it is not a network defect. Do not fit a topology against
cut's 350–520 Hz shape.**

**5. ⭐⭐ THE BONUS GRUNT PAIR BOUNDS THE DRIVE-DEPENDENCE.** GRUNT is a schematic+BOM-verified
**linear** cap bank sitting at the **clipper input** — exactly the "known-linear element that moves
the operating point" reference. Median `h` over 80–1600 Hz, drive min → noon: **GRUNT boost +11.56 →
+7.54 (−4.02 dB), GRUNT flat +9.19 → +7.78 (−1.42) | ATTACK boost +8.61 → +6.73 (−1.88), ATTACK cut
−2.38 → −1.99 (+0.38).** ⇒ **every ATTACK change is inside the known-linear control's range**, so the
apparent drive-dependence is operating point, not network error — session 65 item 5's warning now
with a measured bound. ⭐ And a **saturation ceiling** is visible: at 1600 Hz drive-noon reads
**+4.06 / +4.21 / +4.16 dB** for three different elements, so the drive-noon HF numbers are the
compressor's ceiling, not network values.

**6. ⛔ At drive noon the null loses ALL ATTACK dependence — and this does NOT refute the drive-min
spec.** f0 **328.1 / 328.1 / 328.1 Hz (spread 0.0)** against drive min's 17.6, depths **29.8 / 30.2 /
33.1** (boost **0.91×** flat vs **2.04×**), stable to the bin across all three quiet levels. Drive
noon has the clipper working, so the measured transfer is a **describing function** and the notch
there is not a linear-network property; drive min remains the only valid place to read the spec. The
tool now says so rather than printing a specification off it. **Recorded as unexplained.**

**7. ⭐ My own tempting explanation was refuted by its own test.** boost (drive-min depth **32.70 dB**)
puts f0 in the same bin at both conditions while cut (**14.93**) and flat (**16.01**) disagree by 11.7
and 5.9 Hz — the signature of a **floor-limited** notch bottom, which would have implied the 17.6 Hz
spread driving sessions 61–66 was an artefact. ⛔ **A floor is shared, so floor-limited bottoms must
cluster: drive-min bottoms spread 8.00 dB against shoulders spreading 10.73 dB — no cluster.** Floor
limiting **refuted**; the n=3 pattern is coincidence. Recorded because the joined reading is natural
and wrong.

**8. NEW STIMULUS — `analysis/gen_notch_sweep.py`.** A stepped sine that removes both depth biases.
⚠ **Its span was corrected during validation:** the first draft (250–450 Hz) read a broad null's depth
as **7.56 dB against a true 15 — worse than the instrument it replaces** — because every depth here is
referred to the **200–270 Hz shoulder** and there was no data below 250 Hz. Now **150–550 Hz** (2 Hz
through the 280–380 core, 4 Hz across the skirts), 175.9 s. Validated on five synthetic notches: on
the **sharp** null depth error **0.26 dB vs the swept instrument's 4.31**, width **12.1 vs 15.6 Hz**,
f0 **exact in all five**. ⭐ **GENERAL: a finer grid does not help if the grid does not span the
REFERENCE the quantity is defined against.**

### ⭐⭐ A3 step 25 — the "63 % LOCAL" lead is ANSWERED, and it is a NEGATIVE result: LOCAL is not one unexplored feature. One third of it is the polynomial EDGE artefact plus incoherent spread; the only genuinely SHARED narrow feature in the whole matrix is 320 Hz, and its size tracks the clean-bleed dilution across seven condition groups (session 69)

**Deliverables:** `analysis/shape_gate.py` gains a **band census with a COHERENCE column**, a
`--rowset` membership freeze, and a third plot panel; `analysis/reports/s69_shape_gate.json`,
`build/shape_gate/s69_local_census.png`. **Nothing in `src/` or `tests/` changed.** Session 63's
next-step (d) / sessions 64–68's item (c), which every session since 63 has carried as *"the single
largest unexplored lead in A3"*.

#### 0. ⚠⚠ THE BASELINE DID NOT REPRODUCE — AND THE CAUSE WAS MEMBERSHIP, NOT VALUES (the session-49 item-7 trap, SIXTH appearance)

`shape_gate` on `analysis/reports/comprehensive_data.json` returned **OD ex `gain-n12` 268 rows,
rms(q) 3.040** against session 63's recorded **252 rows, 2.611** — while `CLEAN` (120) and
`OD gain-n12` (16) reproduced to the digit. Chased before reading anything else:

* The file is a **104-capture** shipped-default render (`meta.num_captures 104`, `fit_overrides []`).
  Session 63's `shape_gate` ran on the **100-capture** membership.
* ⭐ **Restricted to the 100-capture set, the current file reproduces session 63 EXACTLY — worst
  |Δ| 0.000e+00 across all four terms on all 252 rows** (and 2.834 / 3.040 for the 85- and
  104-capture sets, i.e. the statistic really does move with membership alone). ⇒ **no value moved,
  no regression; 16 rows were ADDED.**
* The 16 are the 4 session-60 captures (`drive-0700_level-1700_{attack-boost,attack-cut,
  grunt-boost,grunt-flat}`) × 4 sweeps, and they are **the worst rows in the matrix: rms(q) 6.890
  against the other 252 rows' 2.611**, with **11 of 16 putting their worst LOCAL band at 320 Hz**.
  That is not a defect either — they are the LEVEL-max/**bleed-free** condition, where the OD path is
  no longer diluted, so the model's errors are seen undiminished for the first time.

⭐ **GENERAL: an aggregate that moves when a capture set grows is not a regression, and checking
that costs one restriction.** New `--rowset OTHER.json` freezes membership to another report's
capture set so an A/B across a capture-set change means something; it prints how many rows it
dropped. **Every number below is on the frozen 252-row membership unless it says otherwise.**

#### 1. THE INSTRUMENT — a per-band census, because `worst_local` provably points at the wrong band

Session 63 asked "which features carry the 63 %?" and the tool could not answer: its per-row
`worst_local` column reports ONE band per row, so a feature present in **every** row at moderate size
loses to a different one-off excursion in each. (Measured: the worst-LOCAL rows all name **4064 /
6451 / 12902 Hz** — and the census below shows those are exactly the bands that are *not* shared.)
The census aggregates across the fixed row set instead, per band `k`:

    rms_k = sqrt(mean_rows(loc_k^2))      how much LOCAL lives there
    mean_k = mean_rows(loc_k)             SIGNED -- a shared feature keeps its sign
    coh_k = |mean_k| / rms_k in [0,1]     COHERENCE across rows
    share_k = rms_k^2 / sum_j rms_j^2     share of the group's LOCAL mean square

⭐ **COHERENCE IS THE STATISTIC, NOT SHARE.** A share only says a band is noisy; the sign says
whether it is ONE thing. **New GATE 3 proves the distinction with a discriminating pair**: 40
synthetic rows with a same-sign −8 dB feature at 320 Hz vs 40 with a **random-sign ±8 dB** feature of
identical magnitude at the same band. Both are found at 320 Hz with **essentially identical share
(79.5 % vs 80.2 %)** and **opposite coherence (1.00 vs 0.06)**. ⇒ a census row reading SYSTEMATIC is
evidence of a shared feature; a share on its own never is. Gates 1 (exactness 7.1e-15) and 2
(attribution) still pass.

#### 2. ⭐⭐ THE ANSWER — LOCAL IS NOT ONE FEATURE, AND MOST OF IT IS NOT A NARROW FEATURE AT ALL

`OD ex gain-n12`, 252 rows, LOCAL = **63.2 %** of the group mean square (2.0748² / 2.6107²):

| | share of LOCAL | note |
|---|---|---|
| the 4 **EDGE** bands (25 / 32 / 8128 / 12902 Hz) | **18.5 %** | polynomial leverage — **not narrow-feature evidence** |
| **320 Hz** | **16.4 %** (mean **+2.835 dB**, coh 0.64) | the one coherent shared feature; already GAP #2 |
| everything else, spread over ~20 bands | ~65 % | coh **0.13–0.63**, i.e. row-dependent, not one network error |

⇒ **the single biggest genuinely narrow, genuinely shared feature is 320 Hz and it carries ~16 % of
LOCAL = ~10 % of the OD mean square** — while the EDGE artefact alone is larger than it. The "63 %
LOCAL" headline is real arithmetic but it is **not 63 % of one findable thing**, and no new element
is hiding in it.

⭐ **THE CLEAN PATH IS THE CONTROL THAT MAKES THIS READABLE.** Same statistic, 120 clean rows:
**every band INCOHERENT (coh ≤ 0.35), no band above 8.2 %, 320 Hz mean +0.094 dB.** A well-fitted
linear path's LOCAL is take-to-take noise, which is exactly what it should be — so the OD reading is
a property of the OD path, not an artefact the decomposition manufactures.

#### 3. ⭐⭐ AND THE 320 Hz FEATURE'S SIZE TRACKS THE CLEAN-BLEED DILUTION ACROSS SEVEN GROUPS

Split by condition (`--by`), 320 Hz is the **top LOCAL band in every single group**, and both its
size and its coherence order themselves by how much clean bleed sits on top of the OD path:

| group | rows | 320 Hz mean | coh | share of LOCAL |
|---|---:|---:|---:|---:|
| `level-1700` — LEVEL max, bleed **exactly zero by topology** | 24 | **+11.76 dB** | **0.95** | 27.5 % |
| `grunt-boost` | 32 | +5.36 | 0.74 | 24.7 % |
| `grunt-flat` | 28 | +5.04 | 0.69 | 24.5 % |
| `drive-1700` | 56 | +3.28 | 0.64 | 21.2 % |
| `attack-boost` | 60 | +2.94 | 0.63 | 17.5 % |
| `attack-cut` | 60 | +2.27 | 0.59 | 10.9 % |
| `blend-` ladders (most clean bleed) | 148 | +1.13 | 0.53 | 10.4 % |
| **CLEAN** (control) | 120 | **+0.09** | 0.30 | 4.5 % |

The sign is positive throughout = **plugin above pedal = the pedal has a notch the model does not**.
At LEVEL max `level_blend_tf` puts the bleed at **exactly zero** (session 59 item 6), and there the
feature reads **+11.76 dB at coherence 0.95 — the strongest and most systematic single feature
anywhere in the matrix**; by the blend ladders it is diluted to +1.13 dB.

⇒ **a matrix-wide, quantitative confirmation of GAP #2 / session 46's mechanism from an instrument
that shares no machinery with the notch probes** (no solve, no taper, no `b0`, no bleed estimate —
just the residual's shape). Session 46 measured this on single captures at full resolution and
inferred the burial; this measures the burial itself, as a monotone ordering over 428 rows.

#### 4. ⚠ WHAT THE PLOT SHOWS THAT THE TABLE DOES NOT

`build/shape_gate/s69_local_census.png` panel 3: the OD LOCAL-rms curve has **one isolated narrow
spike, at 320 Hz**, and it is the only place the coherence trace peaks; the CLEAN trace is flat and
near-zero across the whole band. Panel 1 (the six worst rows) shows why `worst_local` misled: every
row carries the **same-signed ~320 Hz bump**, but each also carries a **larger (up to +21 dB) 4–8 kHz
excursion that differs in sign and shape row to row** — bigger per row, incoherent across rows.

#### 5. ⚠ WHAT IS NOT CLAIMED, AND THE SECONDARY ITEM

* The 3.2–8.1 kHz cluster holds ~22 % of LOCAL at coherence 0.32–0.68 — **mixed, not systematic**. In
  `attack-cut` it appears as **4064 Hz −2.89 dB (22.6 %) beside 6451 Hz +3.04 dB (18.7 %)**, adjacent
  bands of opposite sign, which is the signature of a feature *between* them rather than noise. It
  overlaps session 30's still-unlocalised **A3-adjacent 5.1–6.4 kHz collapse** and A2e. Recorded as
  the secondary item; **its coherence does not support fitting one element against it.**
* LOCAL on a 1/3-octave grid remains a **lower bound** on a narrow feature (session 46: −3.4 dB
  banded vs −24 dB at full resolution), so `+11.76 dB` at 320 Hz is a floor, not a measurement.
* This does not re-open `trebleLadderDampR`. It measures the SIZE and the CONDITION-ORDERING of the
  gap the two-pole ATTACK topology (§4 "A3 step 20") already exists to close.

⇒ **REMOVE "the 63 % LOCAL pass" from the queue.** It is answered: there is no unexplored feature in
it, and the one shared narrow feature it contains is 320 Hz — the network sessions 60–68 have been
building. The `level-1700` rows are the sharpest place in the whole matrix to gate that network,
because that is where the bleed stops hiding it.

---

### ⭐⭐ A3 step 26 — the stepped-sine notch measurement lands, and it CORRECTS the ATTACK specification that sessions 61–66 were built on: the null moves HALF as far as recorded and the boost throw is 29 % NARROWER (session 70)

**Status: measurement only. NOTHING in `src/` or `tests/` changed. No topology was fitted or
scored — that is `attack_render_gate.py`'s job and it is the next step, not this one.**

New tools: `analysis/read_notch_sweep.py`, `analysis/verify_new_captures.py`. New report
`analysis/reports/s70_notch_sweep.json`.

#### 0. The captures, and one of them is rejected

35 captures landed (§1 §2 §3 §4 §5 §7 of `docs/final-capture-window.md`), gated **before** anything
was read out of them. 25 pass. **The 10-file §2 repeatability set is REJECTED** — see item 6.

⚠ **My own gate was wrong first, and the captures were right.** Its first draft hardcoded the notch
duration as **175.9 s**, transcribed from the capture document, and failed all ten notch captures at
**176.100 s**. The document's figure was mis-stated when written; the stimulus has always been
176.100 s. The gate now reads the length off `notch_sweep_48k.wav` itself. ⭐ **GENERAL: a gate that
transcribes a constant from prose inherits that prose's errors** — the session-33 trap, in a gate
rather than a target.

#### 1. ⚠⚠ The reader could not use the project's own loader, and the stimulus's header claimed it could

`captures.load_capture()`'s sample-rate-mislabel guard inspects a **FIXED window at t = 0.5–1.45 s**
expecting the main test signal's 1 kHz cal tone. This stimulus puts the 10 s `sweep_clean` anchor
there — it has to, or the aligner would be correlating behind 100 tones — so the cal tone sits at
~10.6 s where the guard never looks. The guard reads the sweep's ~30 Hz content, infers a huge speed
error, and **resamples a 176.100 s capture down to 89.9 s**; every segment offset is then wrong and
the −18 dBFS half of the file falls off the end and reads **−600 dB**, which is exactly what the
first run printed.

⭐ **`gen_notch_sweep.py`'s own header asserted the cal tone was there "so `captures.load_capture()`
works".** It never did. Session 13 hit this identical trap on the jfet ladder and recorded the fix
("use the raw loader"); session 68 wrote a header claiming to have solved it **without testing that
it had**. Both the header and the runtime print are corrected, and `read_notch_sweep.load_raw`
carries the warning at the point of use.

#### 2. ⭐ GATE 1 — the self-test, and it FALSIFIED MY OWN FIRST FIX

Six synthetic two-pole notches (depth exact in closed form, `w0` prewarped so the bilinear maps the
null to exactly f0), pushed through the real stimulus and read by the identical extraction:

| quantity | worst error | what it replaces |
|---|---|---|
| **f0** | **0.05 Hz** | the swept instrument's ~4.2 Hz, on a 5.86 Hz grid |
| **width** | **1.3 %** | a ±25 % bin-span quantisation |
| depth (shoulder, record-compatible) | −4.23 dB, **never over-states** | unchanged in kind |
| **depth (2-pole fit)** | **0.03 dB** | a lower bound |

⛔ **The obvious depth fix does not work and the gate caught it.** My first attempt fitted a
polynomial baseline outside a fixed exclusion window and evaluated it at f0. It made the error
**WORSE — 7.03 dB against the shoulder method's 4.23** — because a 158 Hz-wide null's skirts extend
past any exclusion window that still leaves data inside a 150–550 Hz span, so the polynomial fits the
**skirt** and the "baseline" lands below the true one. Widening starves the fit; narrowing worsens
the contamination. **There is no setting that works — that is a property of the span, not of the
tuning.** The working version is parametric: fit the two-pole magnitude plus a linear background to
the whole curve, so depth is a fitted parameter rather than a difference of two contaminated points.
⚠ It buys that with an assumption (the null is two-pole), which is why the definitional `depth` is
still reported beside it.

⭐ **AND THIS CORRECTS THE CAPTURE DOCUMENT'S OWN CLAIM.** §3 and `gen_notch_sweep.py`'s header both
said a stepped sine makes depth "a directly measured value rather than a lower bound". Only half
true: it fixes **bin smearing** (a resolution artefact) but not **shoulder contamination** (which is
definitional). Depth did become a value — but from a fix in the *reader*, not the stimulus.

#### 3. ⭐⭐ GATE 2 — the same-file instrument comparison, which is what makes any of this arbitrable

`gen_notch_sweep.py` puts a 10 s `sweep_clean` **inside every capture**, so the OLD instrument can be
run on the SAME audio. That removes the take-to-take term and the knob-repositioning term, leaving
only the instrument.

✅ **The swept re-implementation is proven identical to `attack_notch_probe.py` first** — run on the
NEW drive-min captures it returns `f_bin` **316.41 / 328.12 / 333.98**, depth **14.9305 / 32.7022 /
16.0107**, width **77.88 / 27.07 / 71.89** against the record's 316.4/328.1/334.0, 14.93/32.70/16.01,
77.9/27.1/71.9 — **to the digit**. So GATE 2 compares instruments, not implementations.

**Drive min, quiet level, same audio both columns:**

| throw | f0 step | f0 swept | Δf0 | dep step | dep swept | **Δdep** | wid step | wid swept | **Δwid** |
|---|---|---|---|---|---|---|---|---|---|
| cut | 323.03 | 318.37 | +4.66 | 15.27 | 14.93 | +0.34 | 75.4 | 77.9 | −3.2 % |
| **boost** | 326.41 | 327.75 | −1.34 | **37.98** | 32.70 | **+5.28** | **19.2** | 27.1 | **−29.1 %** |
| flat | 330.17 | 332.75 | −2.58 | 15.58 | 16.01 | −0.43 | 75.6 | 71.9 | +5.2 % |

⭐ **THE SIGN AND THE SELECTIVITY ARE BOTH AS PREDICTED, WHICH IS WHAT MAKES IT CREDIBLE.** Bin
smearing understates, and only where the feature is narrow. The sharp throw (boost) reads **5.28 dB
deeper and 29 % narrower**; the two broad throws move by ≤0.43 dB and ≤5.2 %. A replacement that had
read boost *shallower* would have been adding a new bias, not removing one.

#### 4. ⭐⭐ THE CORRECTED RECORD — and it is a materially different specification

**ATTACK, drive min (the near-linear condition, the only valid place to read the spec), −30 dBFS:**

| | cut | boost | flat | spread |
|---|---|---|---|---|
| **f0 (Hz)** | **323.03** | **326.41** | **330.17** | **7.13** |
| **depth (dB)** | **15.27** | **37.98** | **15.58** | boost/flat **2.44×** |
| **width (Hz)** | **75.4** | **19.2** | **75.6** | |
| *(superseded record)* | *316.4 / 14.93 / 77.9* | *328.1 / 32.70 / 27.1* | *334.0 / 16.01 / 71.9* | *17.58 / 2.04×* |

Two changes matter, and both cut against what the two-pole topology was built to hit:

**(a) ⚠⚠ THE NULL MOVES ABOUT HALF AS FAR AS RECORDED — 7.13 Hz, not 17.58.** Like-for-like on the
same audio the swept instrument gives 14.38 Hz refined / 17.57 Hz on its bin grid, against the
stepped 7.13 Hz. Session 61's headline — *"ATTACK moves the null 17.6 Hz = 3.0× the bin"* — is the
premise that sent sessions 62–66 to a two-pole topology, and it was measured on a grid whose
quantisation is 5.86 Hz. The throws are still **resolved** (3.4 and 3.8 Hz apart against a 0.05 Hz
accuracy) and the **ordering is unchanged** (cut < boost < flat), so the qualitative finding stands —
but the magnitude the topology is fitted against has halved.

**(b) ⚠⚠ THE BOOST NULL IS 29 % NARROWER, WHICH RE-REFERENCES SESSION 66'S HEADLINE.** That session's
candidate was scored as **0.87 / 1.29 / 1.03×** the pedal's widths. Against the corrected pedal
widths the same candidate reads **0.90 / 1.82 / 0.98×**, and the session-63 proposal **1.56 / 2.43 /
1.81×**. ⇒ **the width residual on the sharp throw is roughly 1.8×, not 1.3×** — substantially worse
than recorded, and concentrated entirely on boost. The two broad throws are essentially matched.

⇒ **The open ATTACK item is now specifically "the boost null is too broad", not "all three widths are
too broad".** That is a narrower and better-posed question than sessions 63–66 had, and it changes
which element to look at: a *uniform* factor pointed at a SHARED ladder element (session 63 item 5b),
a *boost-only* factor does not.

#### 5. What is NOT claimed

⚠ **Drive noon is NOT a specification.** Its rows are in `s70_notch_sweep.json` and the null there
sits at 325.8/327.9/327.4 Hz (spread 2.1 Hz), reproducing session 68's finding that ATTACK dependence
largely vanishes at drive noon. But the clipper is working there, so the measured transfer is a
**describing function** and the notch is not a linear-network property. Drive min remains the only
valid read. ⚠ The **−18 dBFS** column shows boost's depth falling 37.98 → 31.95 and its width
widening 19.2 → 27.1 Hz while cut and flat barely move — session 61 item 3's mechanism exactly (boost
pushes ~8 dB more into the J201, which never idles), so **quote the quiet level**. ⚠ Magnitude only.
⚠ The two-pole assumption behind `fitted_depth` is unvalidated on the pedal, which is why the
definitional depth is printed beside it.

#### 6. ⛔⛔ THE REPEATABILITY SET IS REJECTED — one take each, duplicated five times

`repeat_ref-{od,clean}_[1-5]` carry **zero take-to-take information**. Within each family all five
differ by **rms −147 to −164 dBFS**, and `repeat_ref-od_*` are additionally **copies of the existing
`ref-od.wav`** from a prior session (−163 dBFS).

⭐ **The discriminator is a physical floor, not a tuned threshold:** any converter's own noise is
~−90 to −110 dBFS, so two genuine analogue re-recordings **cannot** agree better than that. −163 dBFS
is float32 rounding, 50–70 dB below it. Peaks agreeing to six decimals (`0.148862` ×5, `0.780394` ×5)
and an alignment lag of exactly 0 samples in every pair say the same thing two more ways.

`verify_new_captures.py` now carries a permanent **duplicate-take gate** that scans each replicate
family against the *whole* capture set — a siblings-only check would have reported these as five
perfectly consistent takes.

⭐ **PARTIAL COMPENSATION, from an unrelated direction.** GATE 2 (item 3) shows the swept instrument
reproducing the session-60 record **to four significant figures** on a completely independent
re-recording weeks later. That is a genuine take-to-take datum and it says repeatability on this
condition is excellent. ⚠ But it is **one replicate on one condition**, not the distribution §2 was
for — the 0.144 dB floor stays a point estimate, and several open findings sit at 2–5× it.

#### 7. ▶ What this changes in the queue

- **The ATTACK width question is re-scoped to the BOOST throw alone** (item 4b), and it is ~1.8×, not
  ~1.3×. Re-run `attack_shape_screen.py --fit --best` against the corrected requirement before
  re-reading session 66's reachability verdict — that verdict was reached against 27.1 Hz.
- **The f0 spread requirement halves** (item 4a). Every fit since session 62 targeted 17.58 Hz.
- **Do NOT re-derive the record from `attack_notch_probe.py`.** It is not wrong — it is
  resolution-limited, and item 3 quantifies exactly where (sharp features only).
- §2 needs re-recording while the pedal is present. Everything else on the list is analysis.

---

### ⭐⭐ HARMONIC LADDER — our even-order structure is measured against BOTH references, and the answer SPLITS BY DRIVE: at mid drive we already have hardware's asymmetry (94–112 %), at low drive we sit at ND's (2–12 %) (session 72)

Session 71's next-step (a), and the one question that gates the whole even-order decision: **did we
inherit ND's symmetry, or does our fitted asymmetry already land between the two references?**
Tooling + analysis only — **nothing in `src/` or `tests/` changed, no constant moved, no capture
read.** New `analysis/harmonic_ladder.py`; report `analysis/reports/s72_harmonic_ladder.json`.

#### 0. ⭐⭐ THE ANSWER, AND IT IS NOT THE ONE SESSION 71 EXPECTED

Session 71 recorded the even-order half of our fit as *"aimed at a target ~27 dB low"* and queued an
even-order correction. Measured, that is **only true at low drive**:

| | H2−H3 corrected | ND | HW | position |
|---|---|---|---|---|
| **mid drive**, 997 Hz | **−1.7 dB** | −27.0 | 0.0 | **94 %** |
| **mid drive**, 800 Hz | **−1.5 dB** | −27.0 | 0.0 | **94 %** |
| mid drive, H4−H5, 997 | −0.4 dB | −28.0 | 0.0 | 99 % |
| mid drive, H4−H5, 800 | +3.4 dB | −28.0 | 0.0 | 112 % |
| **low drive**, 997 Hz | **+0.4 dB** | 0.0 | +18.5 | **2 %** |
| **low drive**, 800 Hz | **+2.3 dB** | 0.0 | +18.5 | **12 %** |

⇒ **At mid drive our evens already sit AT the adjacent odds — hardware's signature, not ND's.**
Session 44's fitted asymmetry (`clipSatLo` 0.4377 / `clipSatHi` 0.5979 = 1.37×; `jfetSatPos` 0.4559 /
`jfetSatNeg` 0.7605) did **not** inherit ND's symmetric behaviour, and **must not be re-opened on the
premise that it did.** ⛔ **At low drive we do sit at ND**: hardware puts H2 **18.5 dB ABOVE** H3 there
(−22.5 vs −41) and we deliver **+0.4 to +2.3 dB**.

⭐ **That is a sharper and more actionable statement than "correct the evens".** The deficit is
specifically **low-drive** even-order — asymmetry that should already be present while the clipper is
barely working. The clipper is not the natural carrier for it (at low drive it is near-linear); the
**J201 stage is**, because it sits UPSTREAM of the DRIVE pot and therefore never idles (session 59
item 3). Any even-order work should be aimed there, and gated on **leaving mid drive alone**, which is
the opposite of what session 71's queue implied.

#### 1. ⭐⭐ THE ANCHORING IS THE METHOD, AND IT IS WHAT MAKES THE CLAIM POSSIBLE AT ALL

The source states no drive setting, no input level and no blend/level condition, so "low drive" and
"mid drive" are not settings we can dial — two unknowns, and guessing either lets the tool report
whatever we like. **The fix is to anchor on the ODD harmonics, which is exactly where the two
references AGREE**: define "low drive" as the operating point where OUR H3/H1 = −41 dB and "mid drive"
as where it = −12 dB, then READ our evens there. The comparison becomes a statement about the ladder's
SHAPE at a matched odd-order reference, which is the only claim the source's data can support.

⭐ **AND THE DISCRIMINATOR IS EVEN-MINUS-ADJACENT-ODD, NOT THE ABSOLUTE EVEN LEVEL.** Absolute Hn/H1
at the output carries the chain's own linear shape, and this chain's is **large and known to be
wrong**: the measured linear gain at 2f..5f is **+9.4/+10.6/+8.9/+5.9 dB at 997 Hz and
+12.5/+15.5/+15.5/+13.9 dB at 800 Hz** re the gain at f — the bridged-T scoop the fundamental sits in,
which session 64 measured as **~2× too deep** vs the pedal. So an absolute comparison against a
published Hn/H1 inherits that error wholesale. **H2−H3 and H4−H5 do not**: their correction is only
the filter's SLOPE between two adjacent harmonics (1.2–3.0 dB, measured and printed), against a
discriminator of 27 dB. That is why the verdict is quoted on the pairs and not on the absolute levels.

⭐ **THE TONE FREQUENCY IS NOT A DETAIL EITHER — and it partly explains the references' own H4/H5
gap.** HW was measured at 997 Hz and ND at 800 Hz, and the 2nd Sallen-Key sits at ~3.3 kHz, so H4
lands at 3988 Hz for HW and 3200 Hz for ND. Measured through the shipped chain, the cross-tone penalty
is **H2 −3.2 / H3 −4.9 / H4 −6.7 / H5 −8.1 dB** — i.e. a HW-column harmonic is filtered that much
harder than the same order in the ND column, **purely because of the tone**. HW's H4 reads 3.5 dB
BELOW ND's at low drive (−60.5 vs −57); the tone alone accounts for ~6.7 dB of difference in that
direction, so that particular "ND's late harmonics run hot" reading is **not supported** once the tone
is accounted for. Both tones are measured here and each compared against its own column.

#### 2. THE CONDITION, AND WHY IT IS BLEED-FREE BY CONSTRUCTION

LEVEL max / BLEND max, EQ flat, ATTACK flat, GRUNT flat, MASTER max, OS 8. **GATE 0** proves the
bleed-free claim from `eq_reference.level_blend_tf` (the same oracle `LevelBlendTest.cpp` uses) rather
than asserting it: feed a unit clean and a zero OD, and what comes out IS the bleed coefficient —
**exactly 0.000000 at LEVEL=1/BLEND=1, and 1.39e-02 (−37.2 dB) at LEVEL=0.99**, which is the
discriminating control that makes the zero meaningful. So every Hn/H1 here is the OD path's own ratio
with no dilution term.

#### 3. ⭐ H3 IS NOT MONOTONE IN DRIVE, AND THAT IS A REAL PROPERTY, NOT NOISE — GATE 5a

At input levels ≤ −24 dBFS, H3 rises monotonically with the DRIVE knob at every setting. **At −18/−12/
−6 dBFS it rises, then CRASHES** — to −43.0 dB at (−12 dBFS, drive 0.85) and −54.1 at (−12, 1.00),
from ~0 dB two settings earlier. That is an **H3 cancellation null**: the anti-phase interference
between the JFET drain-current ceiling's H3 and the clipper's H3 that session 12 established from
amplitudes and session 13 confirmed from phase, reappearing on a completely different axis.

⚠ **It broke the first version of this measurement and the gate caught it.** With crossings taken
anywhere, "the point where H3 = −41 dB" was hit BOTH at (−42 dBFS, drive 0.26) and at (−12 dBFS,
drive 0.84) — on the far side of the null, a totally unrelated operating point — and the two reported
H2 values **44.7 dB apart**. The anchor now takes the **FIRST UPWARD crossing on the rising branch**
only, and GATE 5a prints where each level's branch ends so the null cannot hide.

#### 4. ⚠⚠ THREE OF MY OWN GATES WERE WRONG FIRST, AND ONE OF THEM CAUGHT A REAL STIMULUS DEFECT

**(a) GATE 3 compared quantities at the NUMERICAL FLOOR and manufactured a 13.45 dB failure.** Its
first version tested window-insensitivity on a cubic polynomial, whose H4/H5 sit at **−296 dB** — a
floor-level quantity is not window-stable by construction, and the extractor was perfect (GATE 1
recovers a closed-form ladder to **0.0000 dB** with −296 dB inter-harmonic leakage). Rebuilt on an
asymmetric clipper, where every order is genuinely live, it passes at **0.0015 dB**; it now PRINTS
which orders cleared the floor and fails if fewer than four did, so it cannot quietly go vacuous.
⭐ **GENERAL: a dB quantity computed from something at the numerical floor is not a measurement, and
differencing two of them across conditions manufactures spread.** Sibling of
`ratio-statistics-need-a-denominator-guard` — same failure, numerator instead of denominator.

**(b) The "position between the references" percentage had NO DENOMINATOR GUARD.** At low drive HW
and ND are only **1 dB** apart on H4−H5, and dividing a sane +7.4 dB reading by that produced
**−668 %**. It now refuses to emit a percentage when the two references separate by less than 6 dB
and says so. Third occurrence of that trap in this project.

**(c) ⭐ GATE 4 CAUGHT A GENUINE STIMULUS DEFECT — inter-segment contamination.** The 800 Hz/−42 dBFS
cell read **60 dB more inharmonic content than its 997 Hz twin** (−59.0 vs −118.8) purely because it
happened to follow the file's LOUDEST segment: the output coupling network corners at ~0.72 Hz, a
~220 ms time constant, so a loud segment's envelope transient was still decaying inside the next
segment's analysis window at a 0.25 s guard. **Every cell is now rendered TWICE in the same file, once
with ascending-level neighbours and once with descending**, and GATE 4 requires the pair to agree —
a threshold-free test for "does this reading depend on what came before it", whatever the cause. At
a 1.0 s guard it passes at **0.053 dB over 126 cells** and the contaminated cell reads −93.7 dB.

**(d) GATE 5b was scored on the wrong bins at first.** Its verdict used the worst H2 spread over ANY
H3 bin (8.14 dB at 800 Hz), but that bin contains no anchor. Scored on the **anchor bins** — the ones
the conclusion actually rests on — it is **5.26 / 5.77 dB, WELL-POSED**; both are printed so the gate
cannot be quietly narrowed later.

#### 5. ⚠ WHAT IS NOT CLAIMED

- **The references are chart reads with no underlying data** (`reference-sources.md` §5). The POSITION
  is the finding; the exact percentage is not. A ±3 dB error in either reference moves mid drive from
  94 % to ~83–105 % and does not touch the conclusion; low drive is a 16–19 dB gap and is likewise safe.
- **The absolute drive knob at each anchor is OURS, not theirs** — the references' own input level is
  unstated, so only the ladder SHAPE transfers. Our anchors land at drive 0.19–0.88 depending on input
  level, over 3–4 independent levels per anchor, and the tool prints every one.
- **The DE-EMB column is not comparable to the published numbers** — those carry the reference
  device's own filter, which we do not have. It exists so the 997 and 800 Hz columns can be read on
  one axis.
- **Secondary observation, recorded not chased:** at low drive our H4−H5 is +7.4/+7.5 dB against
  **+14.0 (ND) and +15.0 (HW)** — 7 dB off BOTH references in the same direction. Not a HW-vs-ND
  discriminator, but a shared deficit in the late-harmonic spacing at low drive.
- **Inharmonic (non-harmonic) residual is reported per cell** and grows to −12…−16 dB re fundamental
  at the hottest drive/level corners even at OS 8 — our own aliasing floor, on the same axis where the
  source describes ND's. Cells above −25 dB are excluded from the anchor and the threshold is printed.

---

### ⭐⭐ J201 EVEN-ORDER PIVOT GATE — the low-drive lever is CONFIRMED and PRECISELY localised, but it CANNOT reach hardware's target; what it does have is a genuine interior optimum ~7.9 dB better than shipped (session 73)

Session 72's next-step (a): the low-drive even-order item, aimed at the J201, **gated on leaving mid
drive alone.** Tooling + analysis only — **nothing in `src/` or `tests/` changed, no constant moved,
no capture read.** ctest **16/17** (the pre-existing session-44 `OSValidationTest`, identical
`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`). New `analysis/jfet_even_screen.py`; report
`analysis/reports/s73_jfet_even_screen.json`.

#### 0. ⭐ BASELINE VERIFIED FIRST, AND THE `--fit` PASSTHROUGH IS A PROVEN NO-OP

`harmonic_ladder.py` reproduced every session-72 figure exactly before anything was touched
(**2 % / 12 % low, 94 % / 94 % mid, 99 % / 112 % on H4−H5**), and a full run is only **46 s**, which
is what makes a sweep affordable. The tool gained `--fit KEY=VALUE` (repeatable), `--tag`, `--brief`,
and a `fit` field in its JSON. **Proven inert on the default path twice** — once after the
passthrough landed and again after the rename in (5) — at **1720 shared JSON leaves bit-identical,
worst |Δ| 0.000e+00, nothing added or lost.** The condition is now stamped above every table
*including the empty case*, so a candidate log cannot be mistaken for a baseline one.

#### 1. ⭐⭐ THE GATE RAN BEFORE ANY FIT, AND ITS PREDICTION WAS STRUCTURAL, NOT GUESSED

This project has twice built a reshape and then failed its own pre-registered pivot gate (session
12's `clipk_pivot_check.py`, session 14's `ceilk_pivot_check.py`) — both times *after* the code was
written. So the gate ran first, on parameters that already exist. Its prediction is read off the
shape in `JfetStage.h`, not assumed: for |w| ≪ s the even bump `(a·s²/2)·tanh²(w/s)` is `(a/2)·w²`, so
the stage's small-signal even coefficient is **a/2, independent of s and of both ceilings** — and the
low-drive anchor lands at input levels **−42…−54 dBFS**, i.e. vgs of order 1–10 mV, two to three
orders of magnitude below s (0.456) and below either L (0.657 / 2.011). Therefore `jfetSatNeg` must
move low drive, and `jfetSatPos` / `jfetCeil*` must not.

**All five arms PASS:**

| arm | result |
|---|---|
| GATE 4 repeatability | held for all 12 candidates |
| **LIVENESS** — `a` moves low-drive H2−H3 | best **d(low) = +20.71 dB** |
| **MONOTONE** in `a` | rises +4.12 → +8.14 → +10.37 → +15.27 → +20.71 dB, no turnover |
| **CONTROLS INERT** — knee/ceiling alone | worst \|d(low)\| = **0.00 dB** |
| SELECTIVITY | a feasible candidate moves low >1 dB with \|d(mid)\| ≤ 3 dB |

⭐ **The two inert controls are what make the liveness reading mean anything.** `jfetSatPos = 0.20`
alone and `jfetCeilNeg = 0.33` alone move low-drive H2−H3 by **−0.00 and +0.00 dB** — exactly as the
small-signal algebra requires. ⇒ **the mechanism is specifically the small-signal quadratic a/2**,
not the knee and not the ceiling asymmetry. A gate showing only that `a` is live could not have
separated "this is the quadratic" from "anything in this stage moves that number".

#### 2. ⛔ BUT THE TARGET IS NOT REACHABLE FROM THIS LEVER, AND THE LIMIT IS SELECTIVITY, NOT FEASIBILITY

Corrected H2−H3, mean of both tones (baseline low **+1.33**, mid **−1.62**; HW wants low +18.5, mid 0):

| candidate | a·s | d(low) | d(mid) | selectivity |
|---|---|---|---|---|
| ship | 0.35 | +0.00 | +0.00 | — |
| a=2.0 | 0.91 | +4.12 | +0.76 | **5.4×** |
| a=4.0 | 1.82 | +8.14 | +1.79 | 4.6× |
| a=5.6 | 2.55 | +10.37 | +2.49 | 4.2× |
| a=11 s=0.20 | 2.20 | +15.27 | +4.37 | 3.5× |
| a=22 s=0.10 | 2.20 | +20.71 | +6.95 | 3.0× |

Reaching HW needs **d(low) = +17.2 dB**, so at a 3 dB mid-drive hold the **required selectivity is
5.72× and the best observed is 5.43×** — and **selectivity DECLINES as `a` rises** (the J201's even
content becomes a bigger share of the mid-drive total too), so pushing harder makes the trade worse.
⚠ **That verdict depends on a tolerance I chose, so the tool prints its own sensitivity:** the target
is unreached at a 2, 3, 4 **and 5 dB** hold, and only reached at **7 dB**. Robust to the threshold.
⭐ Note the binding constraint is **not** the monotonicity bound — `a·s` can be traded (a=22 at
s=0.10 is comfortably feasible), so this is a selectivity result, not a feasibility one.

#### 3. ⭐⭐ WHAT THE LEVER DOES HAVE IS A GENUINE INTERIOR OPTIMUM — AND THE POINT THAT "REACHES" THE TARGET IS WORSE THAN SHIPPING NOTHING

Summed |error| against all four hardware statistics (H2−H3 and H4−H5, at both anchors):

| candidate | ALL 4 | net | ex low H4−H5 | net | H2−H3 only | net |
|---|---|---|---|---|---|---|
| ship | 27.8 | — | 20.3 | — | 18.8 | — |
| a=2.0 | 23.3 | −4.6 | 15.5 | −4.8 | 13.9 | −4.9 |
| **a=4.0** | **19.9** | **−7.9** | 11.8 | −8.5 | 9.2 | −9.6 |
| a=5.6 | 20.0 | −7.8 | **11.5** | **−8.8** | 7.7 | −11.1 |
| a=11 s=0.20 | 23.1 | −4.7 | 13.4 | −6.9 | **4.6** | **−14.1** |
| a=22 s=0.10 | **32.7** | **+4.9** | 23.4 | +3.1 | 8.9 | −9.9 |

⇒ **an interior optimum at a ≈ 4.0–5.6, worse on BOTH sides** — the non-degeneracy signature this
project requires, and the opposite of the monotone "make the clipper see less" degeneracy that killed
sessions 5/6. ⛔ **And `a=22 s=0.10` — the only point that reaches the low-drive target — is WORSE
THAN SHIPPED overall (+4.9 dB)**, because it wrecks mid-drive H4−H5 (error 1.5 → 14.6 dB). So
"reaching the target" and "getting closer to hardware" point at different places, which is the whole
value of scoring all four statistics rather than the one the item is named after.
⚠ **Both weightings are printed and neither is silently preferred.** Dropping low-drive H4−H5 — the
pair session 72 flagged as NON-discriminating (HW and ND sit 1 dB apart and we are ~7 dB off **both**,
a shared deficit this lever was never going to fix) — moves the optimum from a=4.0 to a=5.6, i.e. the
basin is flat across that range and the choice between them is not resolved by this screen.

⭐ **At a = 4.0 mid-drive H2−H3 lands essentially exactly on hardware** (−1.62 → +0.2 dB, error
1.6 → 0.2), so the lever does not merely *hold* mid drive there, it slightly improves it. The cost is
~1 dB on each H4−H5 pair. **a = 4.0 is also comfortably inside the monotonicity bound (a·s = 1.82 vs
2.598), unlike a = 5.6 which sits at the edge with s held** — `bound-resting-means-unidentified`
applies to the latter, and is a reason to prefer 4.0 of the two.

#### 4. ⭐⭐ AND THE PHYSICALLY COHERENT VERSION OF THE MOVE IS JOINTLY INFEASIBLE

`a` = 1/Vov and the cutoff ceiling `cn` = Vov/2 derive from the **same** overdrive voltage, so a real
device moving to a smaller Vov moves both: `2·a·cn = 1`. **That identity holds EXACTLY at the shipped
point** (2 × 0.76054 × 0.65743 = 1.0000) and was session 44's one independent corroboration — so
raising `a` alone breaks it (to 5.26 at a = 4.0). Honouring it was tested as its own family and
**3 of 4 candidates are REFUSED as non-monotone**: a small `cn` shrinks the core's cutoff-side slope
exactly where the even bump's slope is most negative, and the map folds back into a rectifier. Only
`Vov = 0.60` survives, worth d(low) = +3.2 dB (19 % of the gap).
⭐ **And `cn` buys nothing anyway** — the `CTL cn` control is inert, and `SQ Vov=0.18` reads the same
d(low)/d(mid) as `a=5.6` to 0.05 dB. ⇒ **the square-law identity and the required even strength are
JOINTLY INFEASIBLE in this shape**: the identity costs feasibility and contributes no signal. Any
proposal here has to choose between the corroboration and the correction, and cannot have both.

#### 5. ⚠ A REAL BUG IN MY OWN CHANGE, CAUGHT ONLY BY PARALLELISM, AND TWO REPORTING DEFECTS

**(a) ⭐ A NEW PARAMETER SHADOWED AN EXISTING LOOP VARIABLE IN THE SAME FUNCTION.** `measure_all`
gained a `tag` parameter for the render filenames — but its duplicate-cell loop already does
`for tag in ("", "#b")`, so **every render after the first drive setting went to one shared
`render#b_*.wav`**, and with 8 workers writing that file the screen died on a torn WAV
(`File format b'' not understood`). ⚠ **It is invisible serially** — the array is read straight back
from the writer, so the filenames are wrong but every number is right, which is exactly why it
survived a bit-identical single-process A/B. Renamed to `stem`; the no-op proof was then re-run and
still holds. **GENERAL: a parameter added to an existing function must be checked against that
function's own local names, not just its callers — and a bug that only manifests under concurrency
will pass every serial verification you have.**

**(b) My verdict line reported "target REACHED" on a candidate that broke the very constraint the
gate existed to protect.** It ranked all feasible candidates by d(low) and named `a=22 s=0.10`, which
moves mid drive **+6.95 dB**. Fixed to rank only inside the hold set and to state the required-vs-
available selectivity explicitly. Same class as `defective-rows-must-not-vote` and session 63 item
6(b) — **score the verdict on the constraint the conclusion rests on**, not on the headline quantity.

**(c) The selectivity comparison printed "5.7× vs 5.7× (NOT AVAILABLE)"**, which reads as a bug; the
real values are 5.43 and 5.72. Quantised to 2 dp (`quantise-ranking-keys`, one field over).

#### 6. ⚠ WHAT IS NOT CLAIMED, AND WHY NOTHING SHIPPED

- **NOTHING IS PROPOSED FOR SHIPPING.** `a = 4.0` is a **located candidate**, on the same footing as
  session 47's `btC17`: the **104-capture ND matrix has not judged it**, and per
  `reference-sources.md` §1(0) it **must** regress there — the captures ARE the ND column, whose
  even-order ladder is the thing being corrected away from. That regression has to be *measured and
  reported*, not discovered later and mistaken for a fault.
- **It breaks the square-law identity** (4), which is a real loss of corroboration, not a footnote.
- **It moves H4−H5 the wrong way** at both anchors (~1 dB each at a = 4.0), and the low-drive H4−H5
  deficit (~7 dB off both references) is untouched and unexplained by anything here.
- The targets remain **chart reads** (`reference-sources.md` §5): the POSITION is the finding, the
  percentages are not. A ±3 dB error in either reference moves the required d(low) by the same amount.
- `jfetGm` was **not** swept. It scales vgs (via 1/(1+gm·R6)) and so does have small-signal even
  authority — but it is the session-4 anchor that every nonlinear fit since rests on, and moving it
  changes the whole level structure. Flagged, deliberately not touched.

---

### ⭐⭐ THE MATRIX JUDGES `jfetSatNeg` — the lever is REAL and even-selective, its value is NOT a=4.0, and the matrix exposes a ~9 dB COMMON-MODE harmonic deficit every difference-based instrument since session 71 was blind to (session 74)

Session 73's next-step (a): run the 104-capture matrix on `jfetSatNeg = 4.0`. Analysis + tooling
only — **nothing in `src/` or `tests/` changed, no constant moved, nothing proposed for shipping.**
ctest **16/17** (the pre-existing session-44 `OSValidationTest`, identical
`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`). New `analysis/matrix_harmonics.py`; reports
`analysis/reports/s74_{baseline129,a2.0,jfetSatNeg_4p0,a5.6,matrix_harmonics,shape_gate}.json`.

#### 0. ⚠⚠ THE MATRIX IS NO LONGER 104 CAPTURES, AND THE FIRST A/B WAS INVALID

The candidate rendered at **129 captures**, because session 70's 35 new files are on disk and
`comprehensive_report.py` reads whatever it finds. Compared against the 104-capture
`comprehensive_data.json` it read as **OD 2.350 → 2.818**, an apparently large regression — and
**CLEAN "improving" 0.427 → 0.408**, which is impossible for an OD-path parameter and is the tell.
Both figures are artefacts of differently-populated sets. ⭐ **`aggregate-moved-check-membership-first`,
seventh appearance in this project.** A shipped-default baseline was re-rendered at the same 129
captures (`s74_baseline129.json`) and every number below is 129-vs-129.

⇒ **`analysis/reports/comprehensive_data.json` is now a STALE ANCHOR** — it is the 104-capture
render, so every "current baseline" figure carried in the handover is quoted on a capture set the
harness no longer produces. Quote `matrix_grade` totals with their capture count, and prefer
`s74_baseline129.json` as the comparison baseline from here.

#### 1. ✅ PLUMBING VERIFIED BOTH WAYS BEFORE ANY NUMBER WAS READ

All **43 CLEAN captures bit-identical** across every numeric leaf (worst |Δ| **0.000e+00**) — as a
J201 (OD-path) parameter must be — while all **86 OD captures differ** (worst |Δ| 57.3). So the
override is live and surgical, and a CLEAN movement would have been a fault rather than a result.

#### 2. THE FR COST IS MONOTONE AND NEGLIGIBLE; THE THD GAIN IS LARGE AND HAS AN INTERIOR OPTIMUM

`matrix_grade`, `OD ex gain-n12` (320 rows): **2.743 (ship) → 2.762 (a=2.0) → 2.778 (a=4.0) →
2.794 (a=5.6)** — a **monotone** ~0.02 dB-per-step cost with **no optimum**, i.e. FR pins nothing.
504 shared rows: 4 better >0.5 dB, 11 worse, 184 bit-identical. `shape_gate --vs` shows all four FR
terms moving together (level +0.045, tilt +0.029, curv +0.010, local +0.030) — a coherent uniform
cost, not a localised trade. ⚠ Mind `shape_gate`'s sign convention: **A is the positional report
(candidate), B is `--vs`**, so `B − A` negative means the candidate is worse.

THD is the opposite — a large improvement with a genuine **interior optimum**, worse on both sides
(signed error, pedal-THD > 1% guard): **−3.477 → −1.755 → −0.118 → +0.815**, mean|e|
**6.569 → 5.324 → 5.087 → 5.245**. `shape_gate`'s THD decomposition, 228 rows both sides:
**rms(a) 9.292 → 6.883 (−2.409)**, carried by the **level** term (6.202 → 3.731), which is exactly
the term session 63 identified as THD's largest. Robust to the denominator guard at every floor from
0 % to 3 %, and improving at all three drive levels.

#### 3. ⛔ BUT THE THD WIN IS AN AMOUNT FIX, NOT A STRUCTURE FIX — AND THE PER-ORDER READ REVERSES THE SHIP CASE

New `analysis/matrix_harmonics.py` scores the **absolute per-order** error over 1458 (file, sweep,
order) cells. Signed mean (plugin − pedal), negative = model under-produces:

| order | ship | a=2.0 | a=4.0 | a=5.6 | |
|---|---|---|---|---|---|
| H2 | −9.94 | −6.41 | −3.35 | −1.71 | EVEN |
| H3 | −12.44 | −12.97 | −14.16 | −15.49 | odd |
| H4 | −8.97 | −6.44 | −3.37 | −1.88 | EVEN |
| H5 | −10.72 | −11.20 | −12.70 | −11.37 | odd |
| H6 | −7.49 | −6.17 | −4.71 | −4.00 | EVEN |
| H7 | −8.93 | −11.48 | −7.98 | −6.48 | odd |

⭐ **The lever IS even-selective, exactly as session 73's small-signal algebra predicted** — all
three even orders move 3–6 dB toward the reference while the odds do not. But mean|e| by parity:
**EVEN 15.51 → 14.24 / 14.28 / 14.75** (optimum at **a=2.0**) and **odd 14.84 → 15.75 / 16.21 /
16.35** (monotonically worse). Per `reference-sources.md` §1 the **odd column is AUTHORITATIVE**
(ND and hardware agree there to the dB) and the **even column is not** (ND runs ~27 dB below
hardware) — so the even regression is expected and desirable, and the **odd regression is a real
cost against the only fully-trustworthy harmonic target we have.**

⇒ **a = 4.0 IS DOMINATED BY a = 2.0 ON EVERY MATRIX STATISTIC** — EVEN mean|e| (14.24 vs 14.28),
odd mean|e| (15.75 vs 16.21), ALL mean|e| (15.00 vs 15.24) **and** FR (2.762 vs 2.778). **The
matrix's optimum for this lever is a ≈ 2.0, not the screen's 4.0–5.6 basin.**

#### 4. ⭐⭐ AND IT DECOMPOSES SESSION 73's OWN HEADLINE STATISTIC, WHICH A DIFFERENCE CANNOT DO

The matrix independently reproduces the screen's `d(H2−H3)`: **+4.06 / +8.30 / +11.27 dB** at
a = 2.0 / 4.0 / 5.6 against the screen's **+4.12 / +8.14 / +10.37** — two instruments sharing no
machinery (129 captures and the report's own harmonic extractor, vs two anchored tones in
`harmonic_ladder.py`), agreeing to **0.16–0.90 dB**. ⭐ That is strong corroboration of session 73's
measurement.

⛔ **But it also shows where the gain comes from: 13 % / 21 % / 27 % of it is H3 FALLING, not H2
rising.** A difference statistic cannot separate those, so the screen scored a partly-illusory
improvement — bought by degrading the authoritative column.

#### 5. ⭐⭐ THE FINDING THAT MATTERS MOST IS NOT ABOUT THIS LEVER AT ALL

The baseline **under-produces EVERY harmonic order by 7–12 dB against ND** (H2 −9.94 … H7 −8.93),
roughly uniformly across drive (**−8.62 / −8.61 / −11.86 dB** at −18 / −12 / −6 dBFS). That is a
large, systematic, **common-mode** deficit — and **every harmonic instrument built since session 71
is blind to it by construction**, because `harmonic_ladder.py` and `jfet_even_screen.py` both score
`H2−H3` and `H4−H5`, and a common-mode error cancels exactly in a difference. Session 72's "we
deliver 94 % of hardware's mid-drive asymmetry" and this −10 dB deficit are **both true**: our
even/odd *ratio* is close while our absolute harmonic *levels* are ~10 dB low.

⭐ It was not invisible everywhere — `shape_gate`'s THD **level** term (6.2 dB, the dominant term,
recorded in session 63) was measuring precisely this on a different axis all along. Two instruments
had half the picture each and neither reading was joined up until the matrix scored absolute orders.

⇒ **fitting an even-order SHAPE parameter on top of an unfixed ~9 dB LEVEL deficit is fitting a
shape to absorb a level error** — the compensating-error pattern this project has already caught
twice (`clipC15` at 1.5 nF, session 37; `trebleLadderDampR` at 30k, session 46). That is the
principal reason nothing is proposed for shipping here.

#### 6. ⚠ A REAL DEFECT IN MY OWN FIRST ANALYSIS, WHICH INVERTED THE CONCLUSION

The first per-order table guarded the floor on **both** sides (`pedal > −60 AND baseline plugin >
−60`) and reported the evens as already correct (H2 −1.17) and the odds as the problem. **That guard
selects away exactly the cells where the model under-produces**, which is the defect under test. With
the guard on the **reference only**, H2 reads **−9.94** and the conclusion inverts in both halves.
Caught by an identity that must hold — the baseline-vs-itself row has to be 0.00 and read +8.81.
⭐ **GENERAL: a floor guard belongs on the REFERENCE, never on the quantity under test** — same
family as `ratio-statistics-need-a-denominator-guard`. `matrix_harmonics.py --selftest` GATE 3
demonstrates the bias rather than asserting it (true −16.00 dB vs model-guarded −4.42 dB, an
**11.58 dB** understatement), alongside a self-identity gate and a known-common-mode-offset recovery
(both 0.000e+00).

#### 7. ⚠ NOT CLAIMED / WHY NOTHING SHIPPED

- **`a = 2.0` is NOT proposed either.** It is merely the matrix's preferred point *for this lever*;
  moving it before the common-mode deficit (5) is understood risks exactly the compensating error
  described there.
- The per-order comparison is against **ND**, and its even column carries no authority. What makes
  the odd-order cost decisive is §1's authority split, not the raw numbers.
- `matrix_harmonics.py` reads `plugin_db`/`pedal_db` as the report computed them; it does not
  re-derive the harmonic extraction, so it inherits whatever that extractor does.
- The −60 dB reference floor is a choice. The parity ordering is stable across it, but the absolute
  deficit is not — at a 1 % THD floor the aggregate deficit reads −3.5 dB rather than −9.2, i.e.
  **the deficit is concentrated where the pedal distorts least**, which is consistent with a
  compression-onset error rather than a uniform gain error.
- **▶ NEXT, IN ORDER: (a)** ⭐⭐ chase the **common-mode harmonic deficit** (5) — it is now the
  largest quantified nonlinear error in the project and it is an AMOUNT/onset problem, so it belongs
  to the clipper and gain-staging (GAP #3a), not to the J201's even bump. Gate it on
  `matrix_harmonics.py`'s signed per-order rows and on `shape_gate`'s THD **level** term.
  **(b)** only then revisit `jfetSatNeg`, and re-derive its value against the corrected baseline
  rather than re-quoting a = 4.0 or a = 2.0 (`search-settings-are-derived-artefacts`).
  **(c)** ⚠ `harmonic_ladder.py` and `jfet_even_screen.py` should print an ABSOLUTE per-order row
  beside their difference statistics, so this blind spot cannot recur. **(d)** re-examine session
  70's §2 rejection under session 71 (4b). **(e)** `c21R` toward 130–150k. **(f)** the A3 / GAP #3b
  low-mid item and the ATTACK-notch depth item. **(g)** everything session 70 listed behind that.

### ⛔⛔ SESSION 74's COMMON-MODE HARMONIC DEFICIT DOES NOT EXIST — it was three measurement defects stacked, and on the bleed-free subset there is NO deficit at any order (session 75)

Session 74's next-step (a): chase the ~9 dB common-mode harmonic deficit, "now the largest
quantified nonlinear error in the project". **It is withdrawn.** Analysis + tooling only —
**nothing in `src/` or `tests/` changed, no constant moved, nothing proposed for shipping.**
`analysis/matrix_harmonics.py` corrected; new reports
`analysis/reports/s75_matrix_harmonics{,_bleedfree}.json`.

#### 0. ✅ THE BASELINE REPRODUCED FIRST, AND STILL DOES ON DEMAND

Before anything was touched, the unmodified tool reproduced every session-74 figure exactly
(H2 −9.94 / H3 −12.44 / H4 −8.97 / H5 −10.72 / H6 −7.49 / H7 −8.93; EVEN mean|e|
15.51 / 14.24 / 14.28 / 14.75; odd 14.84 / 15.75 / 16.21 / 16.35; d(H2−H3) +4.06 / +8.30 / +11.27).
⭐ **`--no-silence-guard` still reproduces those numbers to the digit**, so the record stays reachable
and this is a correction to the *reading*, not a different instrument. One exception, stated rather
than glossed: the by-level block's MEAN column now pools band values instead of averaging per-cell
means, to match the median printed beside it, so it reads −10.07 / −7.78 / −11.84 where session 74
recorded −8.62 / −8.61 / −11.86.

#### 1. ⚠⚠ DEFECT #1 — ONE SILENT CAPTURE CARRIED 4–6 dB OF THE "DEFICIT"

`matrix_harmonics.cells()` did not exclude the zero-knob captures `matrix_grade.rows_of` has dropped
since **session 18** (the −640 dB trap). Of the 67 band values where the model's Hn sits at the
extractor's floor, **47 are `level-0700_base-od.wav` alone** — LEVEL = 0, so the model's OD path
contributes nothing and its Hn is numerically zero (−400 dB, the `+1e-20` epsilon in
`harmonics_at_anchors`). Those cells moved the signed means by **4.3–6.8 dB**:

| order | s74 (mean) | corrected (mean) | corrected (median) |
|---|---|---|---|
| H2 | −9.94 | **−2.27** | −3.67 |
| H3 | −12.44 | **−5.71** | −3.72 |
| H4 | −8.97 | **−3.49** | −1.14 |
...
⚠ **A reference-side floor cannot catch this, so session 74's item-6 rule was necessary but not
sufficient.** At LEVEL = 0 the *reference's* own Hn/H1 divides one floor-level number by another and
comes out as high as **−1.7 dB**, sailing through the −60 dB reference floor. The exclusion has to be
on the **condition**, decidable before any number is read.

⭐ **AND THE ROBUST STATISTIC ALONE WOULD HAVE PREVENTED THE WHOLE ERROR.** With the degenerate
captures still in, the pooled **median** already reads −3.97 / −3.90 / −1.25 / −3.79 / +0.40 / −3.59 —
essentially the corrected answer. Session 74 averaged a distribution with a −400 dB tail, where the
mean is set by how many such cells exist and by the extractor's epsilon rather than by the physics.
`--selftest` **GATE 4** demonstrates it instead of asserting it: against a true −3.00 dB deficit, one
degenerate capture moves the **mean 28.72 dB** and the **median 0.00 dB**.
⚠ Note the median has to be **pooled over band values** — a per-cell median averaged across cells is
NOT robust, because a whole degenerate cell survives the inner reducer intact and then enters the
outer mean at full weight. My first version made exactly that mistake and GATE 4 failed it.

#### 2. ⚠⚠ DEFECT #2 — BLEND = 0 IS THE SAME DEFECT AND THE SILENCE TEST CANNOT SEE IT

BLEND = 0 is **100 % clean**: not silent, so `matrix_grade`'s FR criterion passes it, yet the OD path
is out of circuit exactly as at LEVEL = 0. Measured, those 14 band values carry a **median "deficit"
of −118.93 dB**. New `no_od_path_rows()` drops both cases from the knob settings (5 files, 16
(file,sweep) rows), printed by name — never silently.

#### 3. ⭐⭐ DEFECT #3, AND IT IS THE WHOLE REMAINDER — Hn/H1 IS DILUTED BY THE CLEAN BLEED

`Hn/H1` is measured at the **output**, and the clean BLEND bleed adds fundamental and no harmonics.
So it lowers Hn/H1 on both sides — but by **different amounts whenever the model's OD-vs-clean
balance is wrong, which is exactly A3.** A mixed-BLEND harmonic statistic therefore reports A3 as a
distortion error. Sessions 59/60 established the bleed is **exactly zero at LEVEL max AND BLEND max**;
new `--bleed-free` scores only there (270 cells, 661 band values, **0 at the extractor floor**):

| order | mixed BLEND (median) | **BLEED-FREE (median)** | parity |
|---|---|---|---|
| H2 | −3.67 | **+2.59** | EVEN |
| H3 | −3.72 | **−0.46** | odd |
| H4 | −1.14 | **+3.17** | EVEN |
...
⇒ **there is no harmonic deficit at all where the measurement is clean.** The **odd orders — the
authoritative column per `reference-sources.md` §1 — are met to within 0.8 dB at every order**, and
the evens run **+2.6 to +3.4 dB ABOVE ND**, which is the direction hardware wants (§4 puts hardware's
evens above ND's). The by-level trend goes with it: pooled over BLEND the deficit looked like it GREW
with level (−0.94 / −2.12 / −4.15 dB at −18 / −12 / −6 dBFS, which I read mid-session as a
compression-onset error); bleed-free it is **+1.72 / +2.03 / +0.07** — no trend, because dilution is
itself level-dependent (the OD path compresses, the bleed does not).

⭐ **Same mechanism as session 60 item 7** (`dilution-fakes-a-resonance`), where the bleed sculpted a
flat ATTACK gain into an apparent +4.5 dB resonance at 202 Hz and three sessions chased a resonator.
Fourth occurrence: **a mixer between the nonlinearity and the measurement contaminates any statistic
that is not a ratio of like quantities on the same path.**

#### 4. ⚠⚠ AND THE CANDIDATE RANKING DOES NOT SURVIVE ON THE UNCONFOUNDED SUBSET

The cell-level scatter is **sd 11.3 dB, IQR [−4.2, +7.0], p5–p95 −20 … +19**, and a variance
decomposition explains **almost none** of it (anchor 0.4 %, order 1.6 %, drive 0.1 %, sweep −0.9 %;
anchor × order jointly 11.9 %). So the per-order `mean|e|` numbers are ~9 dB of scatter around signed
medians of ~1–3 dB, and an unpaired reading of a 0.3–1.5 dB difference would be meaningless. The
candidates share cells, so the tool now **always** prints a paired bootstrap (10k resamples):

| set | group | a2.0 | a4.0 | a5.6 |
|---|---|---|---|---|
| full-guarded (1125 cells) | EVEN | **−1.40** [−1.70, −1.11] | **−1.33** [−1.80, −0.87] | **−0.80** [−1.33, −0.27] |
| | odd | **+0.84** [+0.60, +1.09] | **+1.43** [+1.03, +1.82] | **+1.64** [+1.12, +2.15] |
| | ALL | **−0.32** [−0.52, −0.12] | +0.01 [−0.31, +0.32] | +0.38 [−0.01, +0.76] |
| **bleed-free (253 cells)** | EVEN | −0.39 [−0.98, +0.20] | +0.43 [−0.47, +1.27] | **+1.25** [+0.26, +2.22] |
| | odd | +0.25 [−0.25, +0.76] | **+0.81** [+0.04, +1.58] | **+1.34** [+0.29, +2.34] |
| | ALL | −0.07 [−0.48, +0.32] | **+0.61** [+0.00, +1.19] | **+1.29** [+0.55, +1.99] |

⇒ session 74's "**a = 2.0 dominates a = 4.0 on every matrix statistic**" is **significant only on the
diluted set** — i.e. it is a statement about A3, not about the J201's even bump. **Bleed-free, the
matrix cannot separate ship from a = 2.0 at all** (ALL −0.07 [−0.48, +0.32]) and can only say
**a = 4.0 and a = 5.6 are significantly WORSE**. That is a cleaner and stronger version of session
74's "nothing ships": the shipped value is not merely preferred on a contaminated aggregate, it is
**indistinguishable from the best candidate and better than the rest on the only clean read.**

#### 5. ⭐⭐ AND THE PREMISE BEHIND SESSIONS 72–74 IS NOT SUPPORTED BY OUR OWN CAPTURES

`reference-sources.md` §4 records, from the third-party chart, ND at "mid drive" with **H2 −39 /
H3 −12 ⇒ H2−H3 = −27 dB**, and concludes ND has essentially no even-order mechanism. Sessions 72–74
compared **our plugin against that chart column** and never measured ND's own parity off the captures
— which **are** the ND reference. Measured now, bleed-free (median Hn/H1):

| sweep | H2 | H3 | H4 | H5 | H2−H3 | H4−H5 |
|---|---|---|---|---|---|---|
| −18 dBFS | −34.4 | −26.9 | −41.7 | −30.6 | **−7.5** | −11.1 |
| −12 dBFS | −28.7 | −26.2 | −38.2 | −31.2 | **−2.4** | −7.1 |
| −6 dBFS | −25.9 | −25.4 | −34.6 | −28.4 | **−0.5** | −6.2 |

⇒ **ND's H2−H3 runs −7.5 → −0.5 dB across the matrix, never −27**, and by the DRIVE knob (diluted set,
where all five settings are populated) it runs **+8.2 dB at drive min → −5.8 at noon → −14.2 at max**,
i.e. ND is even-DOMINANT at low drive. ⚠ **State the limit precisely: this does not prove the chart
wrong.** ND's H3 never exceeds ≈ **−25 dB** re fundamental anywhere in this matrix, while the chart's
mid-drive column has H3 = **−12 dB** — a far hotter operating point that **the capture matrix does not
contain.** So the matrix cannot test the chart's mid-drive claim, and equally the chart cannot be
quoted as a property of ND at the operating points we actually fit against.

#### 6. ⚠ WHAT WAS TESTED AND CAME BACK NEGATIVE

Our own linear-FR error at the harmonic frequencies (the bridged-T scoop, the 320 Hz notch and A3's
low-mids all sit between 200 Hz and 2.8 kHz, where the harmonics of a 100/200/400 Hz anchor land) was
de-embedded per cell as `L = FRresid(n·f) − FRresid(f)` from each file's own `sweep_clean` row — the
per-row gain match is a constant across bands so it cancels exactly in that difference. **It moves the
signed means by ≤1 dB and does not dissolve the residual** (H2 −4.18 → −3.67, H3 −6.14 → −7.05,
H6 −1.01 → −0.13). ⚠ It also *raises* `mean|e|` (9.04 → 10.04) because the correction has sd 2.83 dB
at cell level, so **do not rank on a de-embedded mean|e|** — only the signed means are readable.

#### 7. ⚠ NOT CLAIMED

- The bleed-free numbers rest on **661 band values across 3 drive settings**, and with sd ≈ 11 dB the
  standard error of each per-order median is ≈ **1.3 dB**. So "the odds are met to within 0.8 dB"
  means *consistent with zero*, not *measured at 0.8 dB*.
- Every number here is vs **ND**, and §5 shows the matrix covers only ND's lower-drive regime. Nothing
  here says anything about hardware's mid-drive column, which remains the reason the even-order
  question is open at all.
- `matrix_harmonics.py` still inherits `comprehensive_report.py`'s harmonic extractor unchanged.
- **ctest was NOT run this session** — the Bash tool's classifier was unavailable for non-Python
  commands throughout. Nothing in `src/` or `tests/` was touched (only
  `analysis/matrix_harmonics.py`), so the suite cannot have moved from the session-44 **16/17**, but
  that is an inference, not a measurement: **run it at the start of the next session.**

#### 8. ▶ NEXT, IN ORDER

**(a)** ⛔ **DROP session 74's next-step (a) from the queue** — there is no common-mode harmonic
deficit, so there is no GAP #3a clipper-amount item here. What §3 leaves behind is not a new lead but
a **re-pointing at A3**: the diluted-vs-bleed-free gap (−3.7 dB vs +2.6 dB at H2) is the size of A3's
OD-vs-clean balance error *measured on a harmonic axis*, which is a genuinely independent instrument
for A3 and could be used as one.
**(b)** ⭐ the even-order question is **entirely a hardware-reference question now**, not a matrix
question: bleed-free we already sit 2.6–3.4 dB above ND on the evens and within 1 dB on the odds, and
every `jfetSatNeg` candidate pushes the evens 5–10 dB above ND while degrading the odds. Before any
further even-order work, **resolve §5** — either obtain the chart's underlying data / its operating
point, or capture ND at an input hot enough to reach H3 ≈ −12 dB (captures are cheap and
re-renderable per `reference-sources.md` §0, so this is a re-render, not a hardware session).
**(c)** ⚠ still owed from session 74: make `harmonic_ladder.py` and `jfet_even_screen.py` print an
ABSOLUTE per-order row — and now also **a bleed-free-vs-diluted split**, since §3 shows dilution is
the bigger of the two blind spots.
**(d)** re-examine session 70's §2 rejection under session 71 (4b). **(e)** `c21R` toward 130–150k.
**(f)** the A3 / GAP #3b low-mid item and the ATTACK-notch depth item. **(g)** everything session 70
listed behind that.

### ⭐⭐ THE ANCHORED LADDER HAS THREE DEGREES OF FREEDOM AND WE ONLY EVER MEASURED TWO — the third is a 14.5–16.9 dB AUTHORITY-FREE deficit at H5, and `clipK` is a large but non-selective lever on it (session 76)

Session 75's next-step (c), which turned into a finding. Tooling + analysis only — **nothing in `src/`
or `tests/` changed, no constant moved, NOTHING PROPOSED FOR SHIPPING.** `analysis/harmonic_ladder.py`
gained `level_rows()`, `gate_dof()` (GATE 6), an `authority` column and a `--guard` flag;
`analysis/jfet_even_screen.py` gained a TERTIARY block.

#### 0. ✅ THE OWED CHECKS RAN FIRST, AND THE BASELINE REPRODUCES

**ctest 16/17** — session 75 could not run it and flagged that its 16/17 was an inference. Measured:
the identical pre-existing session-44 `OSValidationTest` failure, same numbers to the digit
(`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`). Nothing moved. Then `harmonic_ladder.py` reproduced
every session-72 figure exactly before anything was touched (2 %/12 % low, 94 %/94 % mid, 99 %/112 %
H4−H5), in 47 s.

#### 1. ⚠ HALF OF NEXT-STEP (c) WAS UNNECESSARY, AND THE TOOL ALREADY PRINTED THE OTHER HALF

Session 75 asked for an absolute per-order row **and** a bleed-free-vs-diluted split in
`harmonic_ladder.py` and `jfet_even_screen.py`. Checked rather than implemented:

- **The diluted split does not apply to either tool.** `harmonic_ladder.BASE_ARGS` renders at
  `--blend 1.0 --level 1.0`, and `jfet_even_screen` inherits it through `HL.measure_all`. GATE 0
  reads the `level_blend_tf` oracle at that knob pair: clean coefficient **0.000000e+00**, OD
  coefficient 1.000000, with a **non-zero control at LEVEL 0.99 (−37.16 dB)** so the zero is not
  vacuous. Both tools were already bleed-free by construction. ⇒ session 75's §3 dilution defect is
  a `matrix_harmonics` problem only.
- **The absolute per-order row already existed** — the ANCHORED COMPARISON table has printed
  `OURS raw / HW / ND / vs HW / vs ND / DE-EMB` per order since session 72. The blind spot was never
  a missing number; it was that the tool's own verdict section told the reader not to use it
  (*"the discriminator is EVEN-MINUS-ADJACENT-ODD, not the absolute even level"*), which is correct
  about PARITY and was read for four sessions as "only the pairs are readable".

⇒ what was actually owed is neither of the two: an **aggregate of the per-order errors**, and the
statement of what the pair statistics discard.

#### 2. ⭐⭐ THE COMPLETENESS RESULT — the anchored error vector has exactly 3 dof

The anchor is *the operating point where OUR H3/H1 equals the reference's H3/H1*, so against the HW
column **`e[H3] ≡ 0 EXACTLY, by construction`** — verified, it prints `+0.000` at all four
(tone, anchor) pairs. That leaves the anchored error vector as three numbers, `(e2, e4, e5)`, and
this file reported two contrasts of them:

| statistic | is | reported since |
|---|---|---|
| H2−H3 | `e2` | session 72 |
| H4−H5 | `e4 − e5` | session 72 |
| **late-harmonic LEVEL, (H4+H5)/2 re the anchor** | **`(e4+e5)/2`** | **NOTHING, ever** |

The three **span** the vector, so anything the third carries is invisible to the other two *by
construction*. New **GATE 6** demonstrates that rather than asserting it: completeness by
round-tripping 2000 random vectors through the inverse (worst **3.55e-15 dB**), then routing — a pure
late-LEVEL error of −12.00 dB reads **+0.00 on H2−H3 and +0.00 on H4−H5**. ⭐ **A gate that checked
only completeness would pass for three statistics that all mix the modes together, which is useless
for attributing a residual; the routing half is what makes the split mean anything** — same reason
session 69's coherence gate needed a discriminating pair.

#### 3. ⭐⭐ THE FINDING, AND IT IS AUTHORITY-FREE AT H5

Late-harmonic level, error vs HW, filter-corrected exactly as the pairs are (`(g4+g5)/2 − g3`,
measured and printed):

| | 997 Hz | 800 Hz | pooled |
|---|---|---|---|
| **low drive** | **+5.8** | **+5.6** | **+5.7** |
| **mid drive** | **−10.0** | **−13.7** | **−11.8** |

i.e. a **−17.5 dB drive-dependent swing**: at low drive our late harmonics run slightly hot, at mid
drive our harmonic series decays far too fast. ⭐ **The two tones agreeing after correction is the
check that the correction works** — their raw filter slopes are **−1.21 dB/order (997 Hz) and +0.43
(800 Hz)**, opposite in sign, and the corrected readings land 0.2 dB apart at low drive.

⭐⭐ **AND ONE ORDER CARRIES IT WITH NO AUTHORITY ARGUMENT AT ALL.** `reference-sources.md` §1 splits
authority by order (ND governs the odds, has none over the evens), so the useful question per order
is whether the two reference columns AGREE — a new `authority` column prints it. At mid drive:

| order | HW | ND | authority | OUR error |
|---|---|---|---|---|
| H2 | −12.0 | −39.0 | split 27 dB | −2.9 / −4.5 |
| H3 | −12.0 | −12.0 | AGREE (the anchor) | +0.0 |
| H4 | −24.0 | −52.0 | split 28 dB | −11.9 / −12.0 |
| **H5** | **−24.0** | **−24.0** | **AGREE → authority-free** | **−14.5 / −16.9** |

⇒ **our H5 sits 14.5–16.9 dB below BOTH references at mid drive.** That is not an even-order
question (H5 is odd), not a chart-authority question (the columns agree exactly), and it is the
largest authority-free nonlinear error currently measured in this project. It independently
quantifies `reference-sources.md` §4's qualitative note that *"at MID drive hardware has the longer
series (out past H20), ND's dies after H5"*: hardware falls 12 dB from H3 to H5, we fall 26.5.
⚠ **The ± column is the null-check and it passes.** The anchor is reached at 3–4 independent input
levels, and an H5 cancellation null would make them disagree wildly (session 72 saw H2 values
**44.7 dB apart** across such a null). Measured: **±0.2 dB (997) / ±1.2 dB (800)**. No null.
⚠ Anchoring on H3 also removes any common error in the fundamental, so this cannot be a level artefact.

#### 4. ⛔ THE J201 EVEN-ORDER LEVER IS NOT ITS CARRIER

New TERTIARY block in `jfet_even_screen.py`, all 12 session-73 candidates re-scored (mid-drive late
level vs HW, shipped = −11.85): `a=2.0` −0.00 | `a=4.0` +0.36 | `a=5.6` +0.79 | `a=11` +1.81 |
`a=22` +3.98 | the four `SQ` rows −0.02…+1.40. ⭐ **And the two discriminating controls read exactly
−0.00**, as they did on the parity statistic in session 73. ⇒ against a −11.8 dB deficit the whole
`jfetSatNeg` family is worth ≤4 dB, and the point that delivers most of it (`a=22`) was already
rejected. **The late-harmonic deficit needs its own carrier**, and nothing in sessions 72–75 is
retroactively damaged by it.

#### 5. ⭐⭐ `clipK` IS A LARGE LIVE LEVER — and GATE 4 forced the guard question first

`clipK` is the clipper VTC's knee hardness (`u/(1+u^k)^(1/k)`, session-11 reshape), the natural
lever on series-decay rate, and it has **never been scored against a series statistic** — session 12
rejected it against a noon H3−H2 ramp, and sessions 15/44 fitted it inside a harmonic-RATIO objective.

⚠⚠ **The sweep failed its own gate first, and the failure was informative.** At `clipK` ≥ 4 GATE 4
(duplicate cells, ascending vs descending neighbours) fired, escalating monotonically:
**k=1.2 0.0034 dB / 0 cells | ship 0.053 | k=4 1.02 / 3 | k=8 9.40 / 9 | k=16 9.68 / 9** — and every
contaminated cell was at **800 Hz, at the two quietest levels**, exactly the cells session 72 built
GATE 4 for. Two candidate causes with very different consequences: the coupling network's ~220 ms
envelope transient still inside the window, or the clipper's warm-started 6-iteration Newton solve
going history-dependent as the knee sharpens. ⭐ **Discriminated rather than guessed:** a new
`--guard` flag re-ran the worst point at 2.5 s instead of 1.0 s and the contamination
**collapsed 9.3966 dB / 9 cells → 0.0803 dB / 0 cells.** ⇒ envelope settling, **not** numerics; the
lever is measurable and the solve is fine. ⭐ **GENERAL: the guard's ADEQUACY IS CANDIDATE-DEPENDENT
— it is sized against a linear settling time, but a harder nonlinearity responds to that residual
transient more strongly, so any candidate that sharpens the nonlinearity must have GATE 4 re-checked
rather than inherited.** The whole sweep was then re-run at 2.5 s, and the shipped point's
four-statistic score reproduces session 73's recorded **27.80 exactly**, so the longer guard does not
bias the comparison.

⭐⭐ **AND IT MOVES THE STATISTIC SESSION 73 PROVED THE J201 COULD NOT REACH.** Low-drive H2−H3,
position between the references — shipped **2 % / 12 %** → `clipK=4` 23 %/37 % → **`clipK=8`
63 % / 83 %** → `clipK=16` 70 %/86 %, while mid drive holds (94 % → 107 %/110 %). Session 73's
verdict was that reaching hardware needed d(low) = +17.2 dB and the J201's best available selectivity
was 5.43× against a required 5.72×. `clipK=8` delivers **d(low) ≈ +12.2 dB**. On the new statistic it
is equally live: mid-drive late level **−11.85 → −3.88 (k=4) → +0.56 (k=8) → +2.74 (k=16)**, i.e.
monotone with a genuine **sign change**, crossing the hardware target near **k ≈ 7–8** — not a
saturating degeneracy.

#### 6. ⛔ BUT IT IS NOT SELECTIVE, AND ON SESSION 73's OWN METRIC IT IS NOT AN IMPROVEMENT

Every gain is paid for at low drive. Session 73's four-statistic summed |error| vs HW
(recorded there: ship 27.8 | a=2.0 23.3 | **a=4.0 19.9** | a=5.6 20.0 | a=11 23.1 | a=22 32.7):

| candidate | low H2−H3 | low H4−H5 | mid H2−H3 | mid H4−H5 | SUM |err| |
|---|---|---|---|---|---|
| HW target | +18.5 | +15.0 | 0.0 | 0.0 | — |
| ship (k=2.4653) | +1.35 | +7.45 | −1.60 | +1.50 | **27.80** |
| clipK=4.0 | +5.55 | +2.85 | +0.05 | +4.40 | 29.55 |
| clipK=8.0 | +13.50 | +2.45 | +2.35 | +6.00 | **25.90** |
| clipK=16.0 | +14.45 | +1.85 | +2.65 | +7.40 | 27.25 |

⇒ `clipK=8` is modestly better than shipped (25.90 vs 27.80) and **clearly worse than the J201's
a=4.0 (19.9)**; `clipK=4` is worse than shipped. It buys low-drive H2−H3 (error 17.15 → 5.00) by
spending low-drive H4−H5 (7.55 → 12.55) and both mid-drive pairs. And on the new statistic the
low-drive late level goes **+5.7 → +21.7 dB hot**. ⇒ **a trade, not a fix. NOTHING IS PROPOSED.**
⚠ `clipK=1.2` is excluded rather than scored: at 800 Hz its H3/H1 spans −39.4…+4.3 dB and never
reaches the −41 dB low anchor, so the tool refuses it instead of extrapolating.

⭐ **The forward-looking point is that the two levers are COMPLEMENTARY, which no session has
exploited.** `jfetSatNeg` is selective for low drive (5.4×) but small, and barely touches the late
level (≤4 dB); `clipK` is large on low-drive H2−H3 and on the late level, but inflates low drive.
**A JOINT `clipK` × `jfetSatNeg` fit has never been run** and is the obvious next experiment.

#### 7. ⚠ NOT CLAIMED

- **`clipK = 8` is not a candidate.** It is a located lever with a measured trade. The **129-capture
  matrix has not judged it**, and per `reference-sources.md` §1(0) it must regress there (the
  captures ARE the ND column) — that regression has to be measured and reported, not discovered later.
  It is also a large departure from the session-44 A5 fit, whose harmonic-ratio objective was
  evaluated at the shipped `clipK`; moving it invalidates that fit and would require re-running it.
  (No ADAA blocker: the clipper carries no ADAA, so `clipK`'s closed-form-antiderivative anchor at
  k=2 never binds — only the `k==2` sqrt fast path is skipped and `pow()` is exact for any k.)
- **The low-drive H4/H5 targets are the least reliable numbers in the reference set** — HW's low
  column is a chart read at **−60.5 and −75.5 dB re fundamental**, at the bottom of a PNG. So the
  SIZE of the low-drive cost above is uncertain; its SIGN and rough magnitude (>15 dB) are not.
- The late-level statistic is **complete but mixes two authorities** — `(H4+H5)/2` averages H4 (split
  28 dB at mid drive) with H5 (columns agree). It is a liveness/size statistic; the authority-free
  claim rests on the per-order H5 row, which the new `authority` column marks.
- All of this is vs **chart reads** (`reference-sources.md` §5): the POSITION is the finding, the
  exact dB is not. And `harmonic_ladder.py` still inherits `phase_harmonics.fit_harmonics` unchanged.
- **Session 75's next-step (a) — the harmonic-axis A3 instrument — was NOT built** (see §8).

#### 8. ▶ NEXT, IN ORDER

**(a)** ⭐⭐ **the JOINT `clipK` × `jfetSatNeg` fit** (§6) — the first genuinely new lever pairing in
the even-order work, and the only thing that could move low-drive H2−H3 without paying the whole
low-drive cost. Gate on all FIVE statistics now available (the four pair statistics plus the late
level), and re-check GATE 4's guard for every candidate that sharpens the knee (§5).
**(b)** then the **129-capture matrix** on whatever that lands on, expecting and REPORTING an ND
regression per `reference-sources.md` §1(0).
**(c)** ⭐ session 75's next-step (a), still owed: the **harmonic-axis A3 instrument**. The dilution
term is order-INDEPENDENT in dB (`(Hn/H1)_out = (Hn/H1)_OD − 20log10(1 + 1/µ)`, µ = the OD/bleed ratio
at the fundamental), so it is a pure common-mode offset that separates cleanly from the per-order
shape, and with the model's own µ known exactly from `a3_blend_decompose`'s superposition taps the
pedal's µ can be solved and cross-checked against `a3_blend_axis`'s `r_ped` — two instruments sharing
no machinery.
**(d)** session 75's (b): resolve the chart-vs-matrix operating-point gap (§5 there) — our matrix
never reaches H3 ≈ −12 dB, so the chart's mid-drive column cannot be tested against a capture.
**(e)** re-examine session 70's §2 rejection under session 71 (4b). **(f)** `c21R` toward 130–150k.
**(g)** the A3 / GAP #3b low-mid item and the ATTACK-notch depth item. **(h)** everything session 70
listed behind that.

### ⛔ THE JOINT `clipK` × `jfetSatNeg` FIT DOES NOT PAY — and the verdict is decided by the AUTHORITY SPLIT, not by the model. Plus: my own "robust subset" was exactly backwards and would have manufactured a false positive (session 77)

Session 76's next-step (a). Tooling + analysis only — **nothing in `src/`, `tests/` or the captures
touched, no constant moved, NOTHING PROPOSED FOR SHIPPING.** New `analysis/joint_even_fit.py`; one
additive `stim` parameter on `harmonic_ladder.measure_all`, proven a strict no-op. New report
`analysis/reports/s77_joint_even_fit.json`.

#### 0. ✅ THE OWED CHECKS RAN FIRST, AND SIX RECORDED VALUES REPRODUCE IN ONE RUN

**ctest 16/17** — the identical pre-existing session-44 `OSValidationTest` failure, same numbers to
the digit (`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`). Then `harmonic_ladder.py` reproduced every
session-72 and session-76 figure exactly before anything was touched (2 %/12 % low, 94 %/94 % mid,
99 %/112 % H4−H5; late level +5.8/+5.6 low and −10.0/−13.7 mid, pooled −11.8, swing −17.5 dB), in
46 s.

⭐ **And the grid then reproduced the whole recorded score table in ONE run at ONE guard**, which no
prior session had done — sessions 73 and 76 measured their candidates separately, at different
guards, so every cross-candidate comparison rested on a cross-session number:

| cell | S4 measured | record | Δ |
|---|---|---|---|
| ship | 27.83 | 27.80 | +0.03 |
| a=2.0 | 23.26 | 23.3 | −0.04 |
| a=4.0 | 19.93 | 19.9 | +0.03 |
| a=5.6 | 19.98 | 20.0 | −0.02 |
| clipK=4 | 29.51 | 29.55 | −0.04 |
| clipK=8 | 25.89 | 25.90 | −0.01 |

**Worst |Δ| 0.04 dB over six independent values.** Sessions 73 and 76 corroborate each other
directly. The tool has the record built in (`SHIP_RECORD`) and refuses to rank if the shipped cell
does not reproduce, so the baseline-first rule is a mechanism rather than an operator habit.

#### 1. ⚠ ONE STRUCTURAL FIX FIRST — A WARNING IS NOT A MECHANISM

`measure_all` always read one shared `build/harmonic_ladder/stimulus.wav`, while
`build_stimulus(path, guard_sec)` can write any guard anywhere. So session 76's legitimate 2.5 s
sweep left a 2.5 s stimulus sitting where the next default run would index it with **guard-1.0
bounds** — every analysis window landing in the wrong place. Session 76 caught it and wrote a
warning into the handover. It is now a `stim` parameter with guard-stamped paths
(`stimulus_g2.50.wav`), so the file and the bounds cannot disagree.
⭐ Checked against the function's own LOCAL names before adding it (session 73 item 5a: a new
parameter named `tag` was silently rebound by that function's own loop) — `stim` is free.
✅ **Proven a strict no-op on the default path: 1748 shared JSON leaves bit-identical, worst |Δ|
0.000e+00, 0 lost, 0 new.**

#### 2. ⭐ THE BASIS IS SIX, NOT FIVE — a correction to session 76's own next-step

Session 76 §8(a) said to gate on "all FIVE statistics (the four pair statistics plus the late
level)". Its own completeness result (§2, GATE 6) says the anchor forces `e[H3] = 0`, leaving
**three** degrees of freedom `(e2, e4, e5)` **per anchor**, and there are **two** anchors ⇒ the
complete basis is **SIX**: `H2−H3`, `H4−H5`, `(H4+H5)/2` at each of low and mid drive. "Five"
counted the late level once for two anchors, and that is not pedantic — the low- and mid-drive late
levels move in **opposite** directions (shipped +5.7 dB hot at low, −11.8 dB short at mid), so
pooling them cancels session 76's own headline. GATE 6 is re-run unchanged and still reports
completeness to **3.55e-15 dB** with a pure late-level error reading **+0.00** on both pairs.

#### 3. ⛔ THE ANSWER: THE PAIRING DOES NOT PAY, AND THE LEVERS ARE PERFECTLY SEPARABLE WHERE IT MATTERS

Full factorial `clipK` ∈ {2.4653 (ship), 4, 6, 8} × `jfetSatNeg` ∈ {0.76054 (ship), 2, 4, 5.6} = 16
cells, guard 2.5 s, all 16 **ADMITTED** (feasibility scanned per cell, GATE 4 re-checked per cell —
worst 0.176 dB, 0 contaminated cells anywhere, against 9.40 dB / 9 cells at guard 1.0 in session 76).

On **S_free**, the two statistics the two independent reference columns AGREE on (§4):

| clipK ↓ / a → | 0.76054 | 2.0 | 4.0 | 5.6 |
|---|---|---|---|---|
| **2.4653 (ship)** | **13.21** | 13.21 | 13.24 | 13.27 |
| 4.0 | 26.91 | 26.94 | 27.00 | 27.06 |
| 6.0 | 31.34 | 31.31 | 31.30 | 31.31 |
| 8.0 | 34.22 | 34.18 | 34.18 | 34.23 |

⇒ **the SHIPPED point is the best of all 16 cells.** `clipK` degrades it **monotonically** (13.21 →
34.22, 2.6×, no interior optimum), and `jfetSatNeg` moves it by **0.06 dB across a 7.4× change in
`a`**. ⚠ State that precisely rather than "inert": the two authority-free statistics each move
~0.9 dB and in **opposite** directions (low H4−H5 error 7.53 → 8.43 worse, low LATE 5.68 → 4.84
better), so they nearly cancel in the sum.

⭐ **And the separability test settles the hypothesis rather than leaving it rhetorical.** Prediction
= `S(k,a0) + S(k0,a) − S(k0,a0)`, i.e. what the score would be if the levers acted independently:

- **on S_free the interaction is ≈ 0 — worst 0.09 dB, mean 0.06** ⇒ the levers are *perfectly*
  separable there, so the whole degradation is `clipK`'s and pairing cannot rescue it.
- on S6 the interaction is **harmful and grows with both levers**: +0.52 (k=4,a=2) → +3.29 (k=6,a=4)
  → +11.23 (k=8,a=4) → **+15.31 (k=8,a=5.6)**, mean 4.59. Near-zero in the weak-lever corner and
  large only when both are pushed = the signature of **overshoot**, not of a mechanism conflict:
  both levers raise the same low-drive even content, so together they sail past the target.

#### 4. ⭐⭐ THE REAL FINDING — FOUR WEIGHTINGS, FOUR WINNERS, AND THE SPLIT IS EXACTLY THE AUTHORITY SPLIT

`reference-sources.md` §1 assigns authority by which column speaks. The operative question per
statistic is therefore whether the two **independent** reference columns AGREE — and that is
answerable from `HL.REF` and `HL.MIN_SPAN_DB` alone, with no judgement:

| statistic | HW | ND | \|HW−ND\| | authority |
|---|---|---|---|---|
| low H4−H5 | +15.0 | +14.0 | **1.0** | **AGREE → authority-free** |
| low LATE | −27.0 | −22.0 | **5.0** | **AGREE → authority-free** |
| low H2−H3 | +18.5 | +0.0 | 18.5 | SPLIT → HW alone |
| mid H2−H3 | +0.0 | −27.0 | 27.0 | SPLIT → HW alone |
| mid H4−H5 | +0.0 | −28.0 | 28.0 | SPLIT → HW alone |
| mid LATE | −12.0 | −26.0 | 14.0 | SPLIT → HW alone |

Scoring the two halves separately:

| weighting | winner | winner's score | ship |
|---|---|---|---|
| **S_free** (2 authority-free) | **ship** | **13.21** | 13.21 |
| **S_hw** (4 HW-only) | **k=6, a=4** (interior) | **10.06** | 32.15 |
| S6 (complete) | a=5.6 | 35.87 | 45.36 |
| S4 (session 73's) | a=4.0 | 19.93 | 27.83 |

⇒ **every statistic that a second reference corroborates prefers the shipped point; every statistic
where only HW's chart column speaks prefers a large joint move** (and on S_hw the winner IS an
interior cell, 3.2× better than shipped — the complementarity hypothesis "working"). The two halves
point in exactly opposite directions.

⭐⭐ **So the even-order/series item cannot be advanced by more fitting. It is gated on the reference
data.** Any candidate quoted from S_hw rests entirely on HW's chart column being right where ND
contradicts it by 14–28 dB — which is precisely what `reference-sources.md` §5 says not to lean on
("the POSITION is the finding, the exact dB is not"). This is session 75's next-step (b) / session
76's (d) arriving as a hard blocker rather than a nice-to-have.

#### 5. ⚠⚠ AND MY OWN "ROBUST SUBSET" WAS EXACTLY BACKWARDS — it scored `k=6,a=4` as a 3.2× WIN

The first version of this tool carried a third weighting, `S4r`, built by DROPPING the two
low-drive late statistics on session 76 §7's note that HW's low H4/H5 are chart reads at −60.5 and
−75.5 dB re fundamental, at the bottom of a PNG. I called it the "robust" subset. **On it, the
interior cell `k=6,a=4` scores 10.06 against shipped 32.15 — a 3.2× improvement at a genuinely
joint point, i.e. exactly the headline "the joint fit pays, decisively".** It is the wrong test:

- The two statistics I dropped are the only two a **second, independent reference corroborates**
  (1 dB and 5 dB apart). The four I kept are the ones where HW alone speaks.
- They are also **exactly where the `clipK` candidates pay their cost.** So a subset chosen by my
  prose about which numbers looked shaky silently became a subset chosen to exclude the candidate's
  costs — `self-selecting-scores` / `defective-rows-must-not-vote`, and it inverted the verdict.
- ⭐ **The tell was free: `HL.MIN_SPAN_DB` and both reference columns were already in the module I
  was importing.** I reasoned about reliability instead of asking the data. Third session running
  that "check what the tool already holds" has bitten — session 76 §1 found the absolute per-order
  row had been printing since session 72.

⭐ **GENERAL: A ROBUSTNESS SUBSET MUST BE DEFINED BY THE REFERENCE SET'S OWN AGREEMENT, NOT BY PROSE
ABOUT WHICH TARGETS LOOK UNRELIABLE.** Legibility of a chart read and authority over a statistic are
different things; conflating them let the least-corroborated half of the evidence outvote the
corroborated half while wearing the label "robust". `FREE_KEYS`/`SPLIT_KEYS` are now DERIVED from
`HL.REF`, printed by the selftest with the gap per statistic, and the two halves are never summed.

#### 6. ⭐ THE PER-ORDER AUTHORITY-FREE TRACKER LOCALISES WHAT A REAL CARRIER MUST DO

At LOW drive HW and ND agree on H3/H4/H5 (0/3.5/4.5 dB apart) and split only on H2; at MID drive
they agree on H3 and H5 only. So the per-order view is where the authority-free evidence is densest.
Raw Hn/H1 error (different convention from the six statistics — absolute, no filter correction —
carried beside them, never summed in):

| cell | low H4 | low H5 | mid H5 |
|---|---|---|---|
| ship | +0.3 / +1.9 | +4.9 / +7.8 | **−14.5 / −16.9** |
| a=5.6 | −1.0 / +0.6 | +4.5 / +7.4 | −14.7 / −17.5 |
| clipK=6 | **+11.0 / +12.8** | **+20.5 / +23.6** | −7.9 / −3.2 |
| clipK=8 | +13.6 / +15.4 | +23.2 / +26.4 | −8.8 / −2.3 |

⇒ `clipK` **buys 6.6–13.7 dB at mid-drive H5 and pays 10–11 dB at low-drive H4 and 11–16 dB at
low-drive H5** — all three authority-free. A trade on ground where no chart-authority argument
applies, and net negative.

⭐ **And the sharper reading: it does not reduce the drive-dependence of the series-length error, it
shifts the whole curve up.** The low→mid H5 swing goes **ship 19.4 / 24.7 dB → k=6 28.4 / 26.7 →
k=8 32.0 / 28.7** (997 / 800 Hz), i.e. it grows by up to 12.6 dB at 997 Hz and does not shrink at
either tone. ⇒ **a carrier for session 76's mid-drive H5 deficit must lengthen the series at MID
drive while leaving LOW drive alone** — the same "selectivity, not size" verdict session 73 reached
about the J201 on the parity statistic, now established one statistic over, on authority-free data.

#### 7. ⚠ NOT CLAIMED

- **Nothing is proposed for shipping, and this time not even a located candidate.** `k=6,a=4` is the
  winner of one half of the evidence and the loser of the other; the 129-capture matrix was
  deliberately NOT run, because session 76's next-step (b) said to run it "on whatever that lands
  on" and nothing landed. Running it would have measured an ND regression for a candidate no
  weighting supports.
- **S_free rests on only TWO statistics**, so it is a narrow instrument. What makes it decisive here
  is not its width but the *direction* of the disagreement: it is monotone in `clipK` with the
  shipped point at the optimum, and the per-order tracker in §6 agrees with it independently.
- The two authority-free statistics agree between columns to 1 and 5 dB, **not exactly** — 5 dB on
  low LATE is inside `MIN_SPAN_DB` but is not nothing, and both columns could inherit a common
  bottom-of-PNG compression. So "authority-free" means *corroborated by a second source*, not
  *known to 1 dB*.
- The per-order tracker is the RAW convention and so carries this chain's own linear-shape error
  (session 72's note); it is used for SIGN and rough size, and the filter-corrected six statistics
  are the scored ones.
- All targets remain **chart reads** (`reference-sources.md` §5). `joint_even_fit.py` inherits
  `harmonic_ladder`'s anchor logic, stimulus, GATE 4 and `fit_nonlinear.min_slope` unchanged.
- The grid brackets `clipK` at 8 because session 76 measured the late-level sign change at k ≈ 7–8;
  it does not test k > 8, and `clipK = 1.2` remains excluded (its H3/H1 never reaches the low
  anchor, so the tool refuses it rather than extrapolating).

#### 8. ▶ NEXT, IN ORDER

**(a)** ⭐⭐ **RESOLVE THE REFERENCE — it is now the binding constraint on the whole even-order item,
not a background task.** §4 shows every candidate's verdict flips on whether HW's chart column is
right where ND contradicts it. Two routes, both cheap: obtain the chart's underlying data / its
stated operating point, or **capture ND hot enough to reach H3/H1 ≈ −12 dB** and test the chart's own
ND column against a capture. ⚠ Session 75 §5 found ND's H3 never exceeds ≈ −25 dB anywhere in the
129-capture matrix, so scope the request by first MEASURING where ND's H3/H1 actually maxes out and
at which condition (`matrix_harmonics.py` already extracts per-order Hn/H1) — a request that does not
bound this will not settle it. Captures are re-renderable on demand (`reference-sources.md` §0), so
this is a re-record, not a hardware session.
**(b)** ⛔ **DROP the joint `clipK` × `jfetSatNeg` fit from the queue** — §3 answers it, and §3's
separability result means no finer grid on these two axes will change the answer.
**(c)** ⭐ still owed, and now better motivated: the **harmonic-axis A3 instrument** (session 75's
next-step (a), session 76's (c)). The dilution term is order-INDEPENDENT in dB, so it separates
...
machinery.
**(d)** §6 names the requirement for a mid-drive series-length carrier (lengthen the series at mid
drive, leave low drive alone). Nothing in the clipper VTC or the J201 even bump does that; a
candidate needs to be drive-SELECTIVE, and the natural place to look is the clipper's input coupling
/ operating point rather than its static shape.
**(e)** re-examine session 70's §2 rejection under session 71 (4b). **(f)** `c21R` toward 130–150k.
**(g)** the A3 / GAP #3b low-mid item and the ATTACK-notch depth item. **(h)** everything session 70
listed behind that.
...
### ⭐⭐ THE REFERENCE IS RESOLVED, AND THE ANSWER IS THAT THE CHART'S H2−H3 COLUMNS ARE NOT USABLE AS TARGETS — the blocker was never the drive, and the right instrument was already on disk (session 78)

Session 77's next-step (a), which it called "the binding constraint on the whole even-order item".
Tooling + analysis only — **nothing in `src/`, `tests/` or the captures touched, no constant moved,
nothing proposed for shipping. No new capture was needed.** New `analysis/nd_tone_ladder.py`; one
false hardcoded claim in `analysis/matrix_harmonics.py` replaced by a computed one. New report
`analysis/reports/s78_nd_tone_ladder.json`.

**ctest 16/17** — the identical pre-existing session-44 `OSValidationTest`, same numbers to the digit
(`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`).

#### 0. ⚠⚠ THE PREMISE THAT BLOCKED THE ITEM IS FALSE AS STATED — a median quoted as a maximum

Session 77 scoped its own next step by "first MEASURING where ND's H3/H1 maxes out", citing session
75 §5: *"ND's H3 never exceeds ≈ −25 dB re fundamental anywhere in this matrix."* Independently,
`matrix_harmonics.py` printed *"nothing in this matrix is hotter than H3 ~ −35 dB, so the matrix
cannot speak to that condition"* — **as a hardcoded string.** The two disagree by 10 dB, which is the
tell: both are **medians**, of different sets, and the question they were answering ("does any
condition reach the chart's −12 dB?") is a question about the **MAXIMUM**.

Measured, with only condition-based exclusions (`matrix_grade`'s silent rows, BLEND = 0, LEVEL = 0,
and the known-bad `gain-n12` group), on the **same swept anchors those claims were made from**:

| statistic | value |
|---|---|
| MAX ND H3/H1 | **−7.1 dB** (`drive-0700_level-1700_attack-boost_base-od.wav`, `sweep_drv_-6`) |
| hottest per-sweep MEDIAN | −25.4 dB (reproduces session 75 §5 exactly) |
| band values at or above the chart's −12 dB | **20 of 720**, in 11 distinct captures, all bleed-free |

⇒ the chart's mid-drive operating point **is** in the matrix. ⭐ `computed-verdicts-not-narrated` and
`split-the-aggregate-check-reachability`, together: the narrated line outlived its own truth, and the
aggregate that replaced it was the wrong order statistic for the question.

#### 1. ⛔ BUT THE CONCLUSION SURVIVES ON THE SWEPT ANCHORS, FOR A BETTER REASON — the CONVENTION, not the drive

19 of those 20 hot cells are the **400 Hz anchor alone**, whose median runs 12–18 dB above the
100/200 Hz anchors. That is not a hotter operating point, it is the reference's own linear shape:
`comprehensive_report.THD_ANCHORS` is `(100, 200, 400)`, so a 100 Hz anchor's H3 lands at **300 Hz —
beside ND's own ~320 Hz notch** — while its fundamental sits on the pre-scoop shoulder, and a 400 Hz
anchor's H3 at 1200 Hz sits above the scoop entirely.

Measured from ND's own `sweep_clean` FR, the bridge from our anchor to the chart's 800 Hz tone,
`[g(n·a) − g(a)] − [g(n·800) − g(800)]`:

| anchor | H2 bridge | H3 bridge | shift it puts on H2−H3 |
|---|---|---|---|
| 100 Hz | −8.6 dB | −23.9 dB | **+15.4 dB** |
| 200 Hz | −17.7 | −23.2 | +5.4 |
| 400 Hz | −13.7 | −9.1 | −4.6 |

and the per-capture spread on those is ~5 dB. ⇒ **the swept anchors cannot be put on the chart's
convention at all.** So the operating point is reachable and the *instrument* was the problem — the
opposite of what next-step (a) assumed, and it means "capture ND hotter" would have settled nothing.

#### 2. ⭐⭐ AND THE RIGHT INSTRUMENT HAD BEEN SITTING IN EVERY CAPTURE, UNREAD, SINCE THE FIRST CAPTURE SESSION

`gen_test_signal.py` writes a **12-point 1 kHz level ladder** (`lvl_-36 … lvl_-3`, 3 dB steps) plus a
`tone_1000` segment into every capture, and `comprehensive_report.py` reads neither for harmonic
structure. Two reasons it is the right instrument, one of them load-bearing:

* **1 kHz is the chart's HW tone (997 Hz) to 0.3 %**, so no octave-scale tone bridge is needed.
* ⭐ **At a 1 kHz fundamental, H2 (2 kHz) and H3 (3 kHz) both land on the flat top of ND's mid
  plateau, so the H2−H3 filter correction is `g(3f) − g(2f)` = **−0.02 dB** (median over 81 captures,
  spread −2.75 … +4.53)** — against the swept anchors' 14 dB and the 800 Hz bridge's −9 … −24 dB.
  H4−H5's correction is larger and less certain (−3.82 dB, spread −9.99 … +0.03), so H2−H3 is the
  statistic this instrument actually supports.

⭐ Second occurrence of `check-for-unread-data-first` in this project — session 60 found
`sweep_clean_-36` sitting unread in every capture and it was the difference between "undecided" and
"converged". **Before asking for a capture, check what the existing stimulus already contains.**

#### 3. GATES — one of them was needed and killed 16 of 25 cells on the low-drive row

* **GATE 0 (extractor)** — a synthesised ladder recovered to **4.2e-12 dB**; the inharmonic residual
  is shown LIVE (adding a −34 dB inharmonic tone moves it −266.8 → −34.0 dB, so GATE 2 can fire); and
  the H3-crossing locator recovers a known crossing exactly **and refuses the falling branch** when
  the target is crossed twice. ⚠ **That last gate FAILED first and the locator was right** — I
  asserted −33.0 from a hand-arithmetic slip when the rising crossing is −32.0. It now states both
  crossings (rising −32.0, falling −16.0) so it is discriminating rather than merely satisfiable.
* **GATE 1 (settling)** — the matrix stimulus gives 0.8 s tones against ~220 ms of output-coupling
  settling, which cost session 72 a cell. Each cell is re-read on the two halves of its own window:
  **worst disagreement 0.022 dB over 912 cells, 0 contaminated.** Clean because the ladder ASCENDS,
  so each cell's contaminating neighbour is the quieter one.
* **GATE 2 (inharmonic residual)** — 98 of 912 cells over `INHARM_MAX_DB` = −25 dB, excluded.
* ⚠⚠ **PER-ORDER MEASURABILITY, and it is load-bearing.** The chart's low-drive column puts H4 at
  −60.5 and H5 at **−75.5 dB** re fundamental while this instrument's residual sits near −50 dB. An
  Hn under that residual is not a measurement — the LS fit returns whatever noise projects onto that
  basis vector, bounded BELOW by roughly the residual, so **a floored H5 reads too HIGH and makes
  H4−H5 read too LARGE.** That is exactly the direction of the unguarded reading. Guarding at
  `Hn > residual + 6 dB` **drops 16 of 25 captures on the low-drive H4−H5 row.** Same family as
  `ratio-statistics-need-a-denominator-guard` / `floor-guard-belongs-on-the-reference`.

#### 4. ⭐⭐ THE RESULT — at the chart's own tone and its own anchor, ND CONTRADICTS BOTH CHART COLUMNS

Anchored on H3/H1 (the chart's own definition of its operating point, and the project's established
anchor since session 72 item 1), first upward crossing only — ⚠ **H3 peaks before the last level in
68 of 76 captures**, i.e. the sessions-12/13 anti-phase null is present on the REFERENCE too, so the
target is generally crossed twice.

| chart condition | statistic | ND measured (filter-corrected) | 10–90 spread | chart ND | chart HW | verdict |
|---|---|---|---|---|---|---|
| mid drive (H3 = −12) | **H2−H3** | **−12.5 dB** (30/30 usable) | −23.9 … −7.4 | −27.0 | 0.0 | ⛔ **contradicts BOTH** (\|Δ\| 14.5 / 12.5, ~midway) |
| mid drive | H4−H5 | +9.2 (27/30) | +1.7 … +17.3 | −28.0 | 0.0 | ⛔ contradicts both, outside both |
| low drive (H3 = −42) | H2−H3 | +9.6 (24/25) | −1.8 … +18.1 | 0.0 | +18.5 | ⛔ spread (19.8 dB) **exceeds the whole HW-vs-ND separation** (18.5 dB) — cannot discriminate |
| low drive | H4−H5 | ≥ +16.9 (9/25, one-sided) | +16.3 … +25.0 | +14.0 | +15.0 | columns agree (1.0 dB apart) — not a discriminator |

⇒ **At mid drive, over 30 independent captures, with a measured −0.02 dB filter correction, the real
ND device's H2−H3 is −12.5 dB and NEITHER chart column lies inside the measured range.** Session 75
§5's direction was right — our captures do contradict the chart's ND column — but its number came
from the wrong tone and the wrong statistic; measured properly the discrepancy is **14.5 dB at the
chart's own operating point**, and the HW column is contradicted by 12.5 dB in the other direction.

⭐ **The low-drive H4−H5 row is a genuine one-sided BOUND and it matters for session 77.** The guard
drops cells where H5 is unmeasurably low, and those are exactly the cells with LARGE H4−H5, so the
surviving median is biased DOWNWARD — yet it still reads **+16.9 dB against both columns' +14/+15.**
So ND's own device is ~2–3 dB above a value the two chart columns AGREE on. That is small, but it is
the load-bearing direction: **"the two columns agree" is not evidence that either is right** — they
can share an error, which session 77 item (7) had already flagged as possible ("both columns could
share a bottom-of-PNG compression") and which is now measured rather than hypothesised.

#### 5. ⭐⭐ WHY NO CAPTURE CAN SETTLE IT — the chart under-specifies its own operating point by more than the quantity in dispute

The obvious next move would be to locate the chart's unstated "mid drive". It cannot be done. With
H3 pinned to the chart's own value, ND's H2−H3 broken down by the DRIVE knob:

| anchor | total spread | explained by DRIVE | **residual WITHIN one drive setting** |
|---|---|---|---|
| mid (H3 = −12) | 33.4 dB | 8.0 dB | **7.8 dB** |
| low (H3 = −42) | 27.6 dB | 11.1 dB | **18.4 dB** |

⇒ pinning **both** the H3 anchor and the drive knob still leaves 7.8–18.4 dB of H2−H3. The chart
states no blend, level, EQ or switch condition, and those residuals are larger than most of the
differences sessions 73–77 were fitting against.

⇒ **`reference-sources.md` §5 rule 3 was already right and sessions 73–77 drifted from it.** That
rule says of the §3/§4 chart data: *"Do not fit to these numbers directly … use them to set the SIGN
and the ORDER OF MAGNITUDE."* Session 73 built a four-statistic scored objective on them, session 76
a five-then-six-statistic one, and session 77 a four-weighting factorial — all treating chart reads
as targets. **The rule stands; the practice has to come back to it.**

#### 6. ⚠ WHAT IS AND IS NOT CLAIMED

* This measures **ND, not hardware.** It shows the chart is a document our own data contradicts on
  the one column we can independently test. It says nothing *directly* about the HW column — but it
  removes the basis for treating either column as a calibrated target.
* **It does not vindicate session 77's `k=6, a=4`.** That point won only on `S_hw`, the half of the
  evidence weighted by the chart's HW-only statistics; this session shows those statistics are not
  targets. The correct consequence is that **the joint-fit verdict should not be re-run against the
  chart at all**, not that its other branch wins.
* The anchor is on H3 and H2−H3 contains H3, so hot-cell selection biases H2−H3 downward. That is why
  the anchor is an interpolated **crossing** rather than a threshold-and-pool, and why the spread over
  independent captures is printed beside every median.
* H4−H5 carries a −3.82 dB filter correction with a −10 … 0 dB spread, so **quote H2−H3, not H4−H5**,
  from this instrument.
* The chart's `low` anchor uses its ND H3 = −42 dB; the HW column's low-drive H3 is −41 dB. The 1 dB
  difference is inside everything above.

#### 7. ▶ NEXT, IN ORDER

**(a)** ⛔ **STOP scoring even-order candidates against the chart's H2−H3 / H4−H5 numbers** (§5). The
remaining legitimate uses of `reference-sources.md` §4 are the SIGN (hardware is even-dominant where
ND is not) and the order of magnitude. Session 77's `S_hw`/`S_free` split, session 76's six
statistics and session 73's four all inherit the demoted numbers.
**(b)** ⭐ the one even-order statement that survives all of this is **structural, not numeric**:
hardware's evens sit at the level of its adjacent odds (H2 = H3, H4 = H5) while ND has no comparable
even mechanism, and session 7's standing bound caps H2/H1 at −12.04 dB for any monotone map with a
quadratic even part. Fit to that STRUCTURE (a real asymmetry plus a symmetric clipper), gated on the
129-capture ND matrix for everything it should not move.
**(c)** ⭐ `nd_tone_ladder.py` is a new, cheap, well-conditioned instrument that no other tool
duplicates: the reference's own ladder at one tone with a ~0 dB pair correction, 12 levels × 76
captures. Point it at **our own renders** next (it currently reads the pedal side only) — that gives a
model-vs-ND comparison on a statistic needing no filter correction, which is what
`harmonic_ladder.py`'s pair machinery exists to approximate.
**(d)** ⭐ still owed and unaffected: the **harmonic-axis A3 instrument** (session 75's (a), 76's (c),
77's (c)).
**(e)** session 70's §2 rejection under session 71 (4b). **(f)** `c21R` toward 130–150k. **(g)** the
A3 / GAP #3b low-mid and ATTACK-notch depth items. **(h)** everything session 70 listed behind that.

---

### ⭐⭐ OUR MODEL IS ON THE CORRECTION-FREE AXIS AT LAST, AND IT INVERTS SESSION 72's LOW-DRIVE READING — the first ~8 dB of the even-order correction is FREE against the ND matrix (session 79)

Session 78's next-step (c): *"point `nd_tone_ladder.py` at our own renders (it reads the pedal side
only today) — that gives a model-vs-ND comparison on a statistic needing no filter correction, which
is what `harmonic_ladder.py`'s pair machinery exists to approximate."* Done. Tooling + analysis only
— **nothing in `src/`, `tests/` or the captures touched, no constant moved, nothing proposed for
shipping.** ctest **16/17** (the identical pre-existing session-44 `OSValidationTest`, identical
`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`). Only `analysis/nd_tone_ladder.py` changed.

**(0) ✅ BASELINE FIRST, AND THE REFERENCE-ONLY PATH IS A PROVEN STRICT SUPERSET.** Before anything
was touched, the default (reference-only) path reproduced every session-78 figure exactly — mid
H2−H3 −12.5 (spread −23.9 … −7.4, 30/30 usable), low +9.6 (24/25), max ND H3/H1 −2.5 dB with 153
cells at or above −12, H3 non-monotone in 68 of 76 captures. After the model side was added,
**1334 shared JSON leaves bit-identical, worst |Δ| 0.000e+00, 0 lost, 4 new.** (The later GATE 3
fix then moves 167 of them deliberately — see (2); that is a correction, not a regression, and it is
reported as such.)

**(1) THE MODEL SIDE.** One shared cell extractor, two loaders: the ND side reads the `lvl_` segments
of each capture, the model side renders our own plugin **at that capture's own condition**, derived
per file via `captures.render_args(parse_capture())` — never hand-written (the session-65 missing
`--grunt` defect). 76 renders at OS 8 with `--trim-latency`, ~12 s each, 8-way parallel per
`build.md`. **GATE M** proves the renders TRACK the condition rather than all being the same file:
max |dH3| between the drive-min and drive-max renders is **63.1 dB**. The model is then held to the
**identical** GATE 1 (settling: worst 0.017 dB over 912 cells, 0 contaminated) and GATE 2
(inharmonic residual: worst −12.6 dB, **121 of 912 cells excluded** — more than the reference's 98,
because at OS 8 our own aliasing floor still reaches −12…−16 dB at the hottest corners, session 72
item 6). Every reported statistic is also given in a **both-sides-guarded** variant so the
one-sidedness of the measurability bound is visible; it moves the answer by ≤0.5 dB.

**(2) ⚠⚠ TWO REAL DEFECTS IN THE INHERITED GATE 3, AND BOTH WERE MINE TO FIND — session 78's
"−0.02 dB, negligible" was wrong twice over, though its conclusion survives.**
**(a) THE WRONG TRANSFER.** The correction was measured on the report's **blended** FR. But a
harmonic is generated at the clipper and reaches the output **only down the OD path** — the clean
bleed carries none — and H2−H3 is a difference of two Hn/H1 ratios, so H1, the one quantity the
bleed contributes to, **cancels exactly**. Read off a blended capture the bleed **fills the recovery
bridged-T's scoop that the 1 kHz fundamental sits in**, and the slope collapses and changes sign.
Measured on the captures that are bleed-free BY TOPOLOGY (BLEND = LEVEL = max, session 59 item 6 —
and the condition is **derived** per file, not matched against the `level-1700` token):

| transfer | ND `g₃−g₂` | model `g₃−g₂` | net |
|---|---|---|---|
| **bleed-free (correct)** | **+0.56** | **+1.48** | **+0.92** |
| blended (what session 78 read) | −0.07 | −0.25 | −0.18 |

⭐ **The tell that settles which is right is external**: the model's bleed-free **+1.48 dB**
reproduces `harmonic_ladder.py`'s completely independent render-based **+1.18 dB** at 997 Hz
(like-for-like — both are our chain at blend/level max), where the blended figure agrees with
nothing. GATE 3 now prints the blended value as a **labelled CONTROL** so the two can never be
confused again.
**(b) THE WRONG SIGN.** The correction was applied as `out − c23`, which **doubles** the chain's tilt
instead of removing it: since `Hₙ_out = Hₙ_gen + g(nf)`, `(H2−H3)_out = gen − [g(3f)−g(2f)]`, so
`gen = out + c23`. Invisible in session 78 because its c23 was −0.02 dB; a real 2·c23 error once the
transfer is measured correctly. New **GATE 3b** pins it against a closed-form case (a nonlinearity
generating H2 ≡ H3 behind a filter that lifts 3f by 6 dB must de-embed back to 0.00; the wrong sign
returns −12.00) and **exits** rather than printing numbers.
⇒ **ND's mid-drive H2−H3 moves −12.5 → −11.9 dB** (30 captures), so |Δ| to chart ND is 15.1 and to chart HW 11.9:
**still contradicts both, still ~midway. Session 78's verdict is unaffected** — the corrections are
~1 dB against a 13–15 dB discrepancy.
⚠ **Standing limitation, stated not hidden:** the correction uses a **full-chain** FR as a stand-in
for the **post-clipper** transfer a harmonic actually sees (`harmonic_ladder.py` shares this
approximation). Treat ~1 dB as its accuracy. Because it is small, both the RAW (output-domain,
assumption-free — and the domain the matrix scores) and corrected differences are now printed.

**(3) ⭐⭐ THE RESULT, AND THE TWO ANCHORS DISAGREE IN SIGN.** Anchored on each side's own H3/H1
crossing, first upward crossing only (the model's H3 is non-monotone in level in **61 of 76** renders
too, so it is anchored by the identical rule):
...
⭐ **Pairing is what makes this readable.** Session 78 found the low-drive anchor could not
discriminate the two chart columns because ND's spread over conditions (19.9 dB) exceeded the whole
HW-vs-ND separation. A **cell-matched paired difference** removes that: the paired 10–90 width is
8.5 dB and, critically, **the interval excludes zero at both anchors**, so both signs are robust
even though each side's absolute value is condition-dependent. The anchor input levels agree to
**−1.8 / −1.6 dB**, so the two sides reach the same H3 at nearly the same drive — this is not a
gain-staging artefact.
⛔ **AND THE POOLED NUMBER IS THEREFORE A TRAP.** The pooled cell-matched d(H2−H3) is **−2.9 dB**
(629 cells), which reads as "we are nearly on ND" — while the model is 7.9 dB BELOW ND at low drive
and 10.9 dB ABOVE it at mid drive. Pooling cancels ~7.9 dB of real error. The tool now **detects the
sign split itself** and refuses to summarise (`split-the-aggregate-check-reachability`,
`computed-verdicts-not-narrated`). **Gate the two regimes separately.**

**(4) ⭐⭐ WHAT THIS CORRECTS, AND IT IS A GATING RULE, NOT A NUMBER.** Session 72 localised the
even-order item to LOW drive on the reading *"at mid drive we already deliver hardware's asymmetry
(94 %); at low drive we sit at ND's (2–12 %)"* — both percentages computed against the **chart's** ND
column. Recomputed against **measured** ND:

- **MID drive: session 72 survives.** 94 % → **88 %** of the way from ND to hardware. Its model-side
  value also corroborates directly — this instrument's model raw H2−H3 is within **0.75 dB** of
  `harmonic_ladder.py`'s at essentially the same tone (997 vs 1000 Hz), across two tools that share
  no anchor machinery, no stimulus segment and no filter correction, and across condition sets
  (one fixed condition vs 76 pooled).
- **LOW drive: session 72 is INVERTED.** Its "2 %" used chart ND = 0.0. Measured, ND's own low-drive
  H2−H3 is **+10.1** and our model is **+1.4** ⇒ **−104 %**: the model is not "at ND", it is on the
  **far side** of ND, ~7.9 dB further from hardware than ND itself is.

⭐⭐ **THE CONSEQUENCE: "an even-order correction MUST regress the ND matrix" IS ONLY TRUE AT MID
DRIVE.** That expectation has been carried since session 72 (`reference-sources.md` §1(0), repeated
as sessions 72(a), 73 item 6, 76 item 7) and it assumed the model already sat AT ND at low drive. It
does not. **At low drive the first ~7.9 dB of the correction moves toward BOTH references at once and
should IMPROVE the 129-capture matrix.** At mid drive the model is already 88 % of the way past ND, so
there a regression is genuinely expected. ⇒ **the two drive regimes will move the matrix in OPPOSITE
directions, and a single aggregate will hide it** — exactly the failure mode (3) already flags one
level down.

**(5) ⚠ WHAT IS NOT CLAIMED.** This is **model vs ND**, and ND's even orders sit ~27 dB below
hardware's, so "matching ND" is not the target on an even-order statistic — it is the thing the item
exists to move away from. What the instrument gives is the **size and sign of our departure from the
column the 129-capture matrix encodes**, on a statistic needing a 0.92 dB correction instead of 14 dB.
**Nothing is proposed for shipping and no candidate was screened** — this is the acceptance
instrument, not a fit. H4−H5 is printed for completeness only: its ND-side correction is −3.16 dB
against H2−H3's +0.56, so it carries the correction risk (session 78's finding, unchanged). The
per-order absolutes are printed beside every pair statistic
(`difference-statistics-hide-common-mode`): model − ND is **H2 −4.8, H3 −0.9, H4 +0.1, H5 −0.6,
H6 +3.1 dB**, i.e. the pair statistic's low-drive deficit is carried by **H2**, not by a common-mode
level error.

**▶ NEXT, IN ORDER: (a)** ⭐⭐ **the low-drive even-order correction, now gated on this instrument at
BOTH anchors separately** — (4) says the first ~7.9 dB at low drive is free against the matrix, which
removes the reason the item has been parked since session 76. Session 73's `jfetSatNeg` screen is the
natural first candidate to re-score here, since (5)'s per-order row puts the deficit on **H2** and
session 73 localised that lever to the J201's small-signal quadratic — but **re-derive its value
against this axis rather than re-quoting 2.0 or 4.0** (`search-settings-are-derived-artefacts`).
**(b)** then the **129-capture matrix** on whatever lands, expecting **opposite signs at the two
drive regimes** and reporting both. **(c)** ⭐ still owed and unaffected: the **harmonic-axis A3
instrument** (session 75's (a), 76's (c), 77's (c), 78's (d)). **(d)** session 70's §2 rejection
under session 71 (4b). **(e)** `c21R` toward 130–150k. **(f)** the A3 / GAP #3b low-mid and
ATTACK-notch depth items. **(g)** everything session 70 listed behind that.

### ⭐⭐ THE LOW-DRIVE EVEN-ORDER CORRECTION IS LOCATED, AND IT IS THE FIRST EVEN-ORDER CANDIDATE THAT COSTS NOTHING AGAINST EITHER REFERENCE — plus three defective gates in my own screen, one of which could only ever return zero (session 80)

Session 79's next-step (a): *"the LOW-DRIVE even-order correction, gated on this instrument at BOTH
...
… but re-derive its value against this axis, do NOT re-quote 2.0 or 4.0."* Done. Tooling + analysis
only — **nothing in `src/`, `tests/` or the captures touched, no constant moved, nothing proposed
for shipping.** New `analysis/even_low_screen.py`; `analysis/nd_tone_ladder.py` gained an additive
`fit_tag` render namespace.

**(0) ✅ BASELINE FIRST, AND IT REPRODUCES TO 0.00 dB.** Before any candidate was rendered, the
shipped point was re-measured through the identical code path and GATE B scored it against the
session-79 record: **low-drive d(H2−H3) −7.93 vs −7.93, mid-drive +10.94 vs +10.94, |Δ| 0.00 dB on
both.** Every session-79 figure also reproduced on the reference side (ND mid −11.9 over 30
captures, low +10.1 over 24, max ND H3/H1 −2.5 dB with 153 cells at or above −12, H3 non-monotone in
68 of 76). The screen **refuses to rank** if that baseline does not reproduce — a cross-candidate
table is only meaningful if the point every candidate is measured against is the recorded one
(`verify-the-baseline-not-its-label`; the pattern is session 77's `SHIP_RECORD`).

**(1) ⭐⭐ THE RESULT — d(H2−H3) = model − ND, AT EACH ANCHOR SEPARATELY.** There is deliberately no
combined column: session 79 showed the two anchors disagree in sign, so any pooled score reads
"small" for a model that is badly wrong in both regimes in opposite directions.

| `jfetSatNeg` | d(low) | 10–90 | d(mid) | 10–90 | selectivity |
|---|---|---|---|---|---|
| **0.76054 (shipped)** | **−7.93** | −12.05 … −3.51 | **+10.94** | +5.86 … +22.31 | — |
| 1.2 | −5.47 | −8.36 … −1.43 | +11.53 | +6.11 … +22.50 | 4.15× |
| 1.9 | −2.38 | −5.28 … +2.33 | +12.40 | +6.49 … +22.80 | 3.81× |
| 3.0 | **+1.61** | −2.46 … +4.64 | +13.46 | +7.26 … +23.25 | 3.79× |
| 4.5 | +5.15 | +0.33 … +8.19 | +14.94 | +8.46 … +23.83 | 3.27× |
| CTL `jfetSatPos`=0.20 | −8.38 | −12.57 … −3.54 | +10.90 | +5.71 … +22.31 | — |
| CTL `jfetCeilNeg`=0.33 | −7.65 | −11.87 … −3.41 | +10.94 | +5.94 … +22.31 | — |

⭐ **d(low) crosses ZERO at `jfetSatNeg` ≈ 2.56 (interpolated by the tool over the swept
0.76 … 4.50), and |d(low)| is worse on BOTH sides of it** — the
non-degeneracy signature, not the "make the clipper see less" monotone degeneracy that killed the
sessions-5/6 fits. ⚠ **"Crosses zero" means "equals ND", NOT "equals hardware"**: ND lies BETWEEN us
and hardware at this anchor (measured ND +10.1, chart HW +18.5), so the crossing is the far edge of
the move that is *free against the 129-capture matrix*, not the target.

**(2) ⭐⭐ AND THIS IS THE FIRST EVEN-ORDER CANDIDATE IN THE PROJECT THAT COSTS NOTHING AGAINST
EITHER REFERENCE AT ITS OWN ANCHOR.** Every prior one had to be argued for against a documented
regression, because §1(0) of `reference-sources.md` says the captures ARE the ND column so moving
toward hardware must move away from them. Session 79 showed that rule is **only true at mid drive**.
At low drive the model sits 7.9 dB BELOW ND while hardware is 8.4 dB ABOVE it, so the whole of the
move in the table above is monotone toward **both** references simultaneously.

**(3) ⛔ THE SELECTIVITY IS 3.3–4.2× AND THE MID-DRIVE COST IS REAL BUT SMALL — and it is the whole
of the argument against pushing further.** At mid drive the model already sits +10.94 dB ABOVE ND
and (session 72, recomputed against measured ND in session 79) ~88 % of the way from ND to hardware,
so movement there is a COST, not a gain: `a=1.9` spends 1.46 dB of it to buy 5.56, `a=3.0` spends
2.52 to buy 6.32, `a=4.5` spends 4.00 to buy only 2.79 (it overshoots low drive). ⇒ **the admissible
range is roughly `a` = 1.9 … 3.0**, and which end wins depends on how the mid-drive cost is
weighted — this screen does not resolve that and does not pretend to. ⚠ Note this is the statistic
session 73 declared unreachable, and it is reachable here **only because the requirement changed**:
session 73 needed +17.2 dB of low-drive movement at 5.72× selectivity to reach the CHART's hardware
column; against measured ND the requirement is +7.9 dB, which 3.8× delivers.

**(4) ⚠⚠ THREE DEFECTIVE GATES IN MY OWN SCREEN, ALL FOUND BY READING ITS OWN OUTPUT AGAINST ITS OWN
TABLE — ONE COULD NEVER HAVE FAILED, ONE COULD ONLY HAVE FIRED SPURIOUSLY, AND THE THIRD WAS IN THE
FIX FOR THE FIRST.**
**(a) The odd-order control was VACUOUS.** It scored **H3**, and returned `dH3 = +0.00` for every
candidate at both anchors — which looked like a perfect pass and was in fact a tautology: **every
side is ANCHORED on its own H3/H1 crossing, so H3 is pinned to the anchor value by construction.**
A control whose answer is fixed by the construction of the measurement tests nothing, however
reassuring a table of zeros looks. Rebuilt on **H5** — odd, and NOT pinned, so it can actually
fail — and it then passes on evidence: at the low anchor H5 moves −70.2 → −71.6 dB across the whole
`a` sweep (1.4 dB) against H2's **13.6 dB**, ratio ≤ **0.13**; at mid drive H5 is −37.7 at every
candidate. ⭐ **GENERAL: a control measured on the quantity the instrument ANCHORS ON cannot fail.
Check that a gate's answer is not fixed by the construction of the measurement.**
⚠ **AND THE REBUILT GATE THEN FIRED SPURIOUSLY ON AN INERT CONTROL — a third defect, same class as
(b), in the fix for (a).** Its `|dH5/dH2| > 0.5` warning is guarded only at `|dH2| > 0.05 dB`, so
`CTL jfetCeilNeg=0.33` — which is inert at this anchor **by design** — printed *"an ODD order moves
comparably, NOT an even-only lever"* on **dH2 +0.13 / dH5 +0.19 dB**, i.e. one noise-level number
divided by another (`ratio-statistics-need-a-denominator-guard`, second occurrence this session).
The question the control asks is only ASKABLE of a candidate that moves H2 at all. Guard raised to
`|dH2| ≥ 1.0 dB` — well under the smallest real candidate (`a`=1.2 moves H2 by **3.26 dB**) and well
over both controls (0.13 / 0.30 at low, 0.00 / 0.03 at mid) — with the raw `dH5` still printed and
the row labelled *inert* rather than dropped. ⚠ **The guard is an OUTPUT-ONLY edit and has NOT been
re-run**: its effect is deterministic from the logged table (every real candidate is ≥ 3.26, every
control ≤ 0.30, so only the controls' rows change), and no scored quantity is touched — but it
should be confirmed on the next invocation rather than assumed.
**(b) GATE L narrated an inertness claim about a quantity it did not measure.** It printed only
`max |dH2|` over the WHOLE ladder and concluded "a control that reads ~0 here is INERT" — but the
two controls read **4.31 and 3.93 dB** there, comparable to `a=1.2`'s 4.08 dB, i.e. the printed
verdict was contradicted by the printed table (`computed-verdicts-not-narrated`, in a gate written
to catch exactly that). ⭐ **And the corrected reading is a FINDING, not a repair:** the controls are
large on the ladder and **≈0 AT THE ANCHOR** (d(low) moves −0.45 and +0.28 dB) — i.e. the J201 knee
and cutoff ceiling have several dB of H2 authority at the HOT end and contribute essentially nothing
where the statistic is read, which is exactly what the small-signal algebra requires (the even
coefficient there is `a/2`, independent of `s` and of both ceilings — session 73). **That is a far
sharper discriminator than a globally-dead control**, which would prove nothing. ⚠ It also had to be
measured rather than inherited: session 73 found these controls EXACTLY inert (−0.00/+0.00) because
its anchor sat at −42 … −54 dBFS, whereas this anchor reaches **−11 dBFS**.

**(5) ⭐ THE PER-ORDER ABSOLUTES LOCALISE THE CORRECTION TO H2, AND EXPOSE A COST THE PAIR STATISTIC
CANNOT SEE** (`difference-statistics-hide-common-mode`). At the low anchor, 22 captures:

| | H2 | H3 | H4 | H5 | H6 |
|---|---|---|---|---|---|
| ND (reference) | −32.4 | −42.0 | −64.4 | −86.7 | −102.4 |
| shipped | −42.1 | −42.0 | −60.1 | −70.2 | −90.8 |
| `a`=1.9 | −35.4 | −42.0 | −59.5 | −71.1 | −87.3 |
| `a`=3.0 | −31.9 | −42.0 | −58.4 | −71.6 | −84.4 |

⭐ The lever is **H2-selective**: at `a`=3.0 H2 lands essentially ON ND's (−31.9 vs −32.4) while H4
moves 1.7 dB and H5 moves 1.4 dB. That corroborates session 79's independent per-order finding that
the low-drive deficit is carried by **H2** (model−ND: H2 −4.8, H3 −0.9, H4 +0.1, H5 −0.6).
⛔ **But H4 is not a free ride and d(H2−H3) is blind to it by construction:** the model already sits
**4.3 dB ABOVE** ND on H4 at this anchor (−60.1 vs −64.4) while sitting 9.7 dB BELOW on H2, so a
lever that raises both **improves H2 and worsens H4** — `a`=3.0 takes H4 to +6.0 dB above ND. The
screen now prints this beside the pair statistic rather than leaving it inside the difference.

**(6) ⚠ NOT CLAIMED / WHY NOTHING IS PROPOSED.** This is a **LOCATED CANDIDATE**, on the same
footing as session 47's `btC17` and session 73's own `a`=4.0. **The 129-capture matrix has not
judged it**, and per session 79 the expectation is now explicitly **two-signed**: the LOW-drive
regime should IMPROVE there (the move is toward ND) while the MID-drive regime should REGRESS
(the move is away from it) — **both must be measured and reported, and a single aggregate will hide
the split.** It also **breaks the square-law identity** `2·a·cn = 1`, which holds exactly at the
shipped point (2 × 0.76054 × 0.65743 = 1.0000) and was session 44's one independent corroboration —
at `a` = 1.9–3.0 it reads 2.5–3.9. Session 73 measured that honouring the identity is **jointly
infeasible** (3 of 4 candidates fold back into a rectifier), so any proposal must choose between the
corroboration and the correction. ⚠ This is **model vs ND**, and ND's evens sit ~27 dB below
hardware's, so "reaching ND" is not the target — what makes the move defensible is only that ND lies
between us and hardware *at this anchor*. ⚠ `jfetGm` was again NOT swept (the session-4 anchor every
nonlinear fit rests on).

**▶ NEXT, IN ORDER: (a)** ⭐⭐ **run the 129-capture matrix on `jfetSatNeg` ≈ 1.9 and ≈ 3.0**, and
report the two drive regimes SEPARATELY — session 79 predicts opposite signs, and a single
`matrix_grade` total will cancel them. Gate with `matrix_harmonics.py`'s signed per-order rows
(which read H2/H4 directly, so the H4 cost in (5) is visible there) plus `shape_gate.py`'s THD
decomposition. **(b)** decide the 1.9-vs-3.0 question, which this screen does not resolve — it needs
the matrix and a stated weighting of the mid-drive cost. **(c)** ⭐ still owed: the **harmonic-axis
A3 instrument** (sessions 75(a), 76(c), 77(c), 78(d), 79(c)). **(d)** session 70's §2 rejection under
session 71 (4b). **(e)** `c21R` toward 130–150k. **(f)** the A3 / GAP #3b low-mid and ATTACK-notch
depth items. **(g)** everything session 70 listed behind that.

---

### ⭐⭐ THE MATRIX JUDGES THE LOW-DRIVE EVEN-ORDER CANDIDATE, AND IT NARROWS THE FREE MOVE SHARPLY — the split had to be forced bleed-free, because binning on the mixed set is contaminated by the very thing the statistic exists to avoid (session 81)

Session 80's next-step (a): *"run the 129-capture matrix on `jfetSatNeg` ≈ 1.9 and ≈ 3.0, and report
the two drive regimes SEPARATELY — session 79 predicts opposite signs, and a single `matrix_grade`
total will cancel them."* Done. Tooling + analysis only; **nothing in `src/`, `tests/` or the captures
touched, no constant moved, nothing proposed for shipping.** `analysis/matrix_harmonics.py` gained a
DRIVE-REGIME SPLIT.

**(0) ✅ BASELINE FIRST, AND FOUR INDEPENDENT SESSION-74 FIGURES REPRODUCE EXACTLY.** Nothing in
`src/` has changed since session 44 and `build/OfflineRender` still dates from before the session-74
baseline was rendered, so that baseline is current, not stale (`rebaseline-all-derived-artefacts` —
checked, not assumed). Re-read: **FR `OD ex gain-n12` 2.743 (ship) / 2.762 (`a`=2.0) / 2.778 (`a`=4.0)**
against session 74's recorded 2.743 / 2.762 / 2.778; **parity mean|e| EVEN 15.51 → 14.24 / 14.28 /
14.75 and odd 14.84 → 15.75 / 16.21 / 16.35** identical under `--no-silence-guard` (the flag that
reproduces that record); **THD `level` term 6.202 → 3.731** identical; and session 75's bleed-free
per-order row **+2.6 / −0.5 / +3.2 / −0.7 / +3.4 / +0.3** reproduces as +2.59 / −0.46 / +3.17 / −0.74
/ +3.44 / +0.25. **CLEAN is bit-identical at every candidate** (0.408 throughout), so the change is
OD-path-only as a J201 parameter must be — plumbing verified by the data, not by inspection.

**(1) ⚠⚠ AND SESSION 74's a=2.0 AND a=4.0 RENDERS ALREADY EXISTED AT 129 CAPTURES — the expensive
half of this step was already on disk and was nearly re-run.** They bracket session 80's admissible
`a` = 1.9 … 3.0 from both sides. Only `a` = 1.9 and 3.0 themselves needed rendering.
⭐ `check-for-unread-data-first`, third occurrence in this project.

**(2) ⚠⚠ THE SPLIT'S FIRST VERSION WAS DEFECTIVE, AND THE DEFECT WAS THE SAME CLASS THE STATISTIC
EXISTS TO AVOID.** The bin coordinate is the REFERENCE's own H3/H1 — the exact quantity both anchors
are defined on, and candidate-independent by construction since `pedal_db` is the capture side (now
asserted per cell, not assumed). But run on the tool's DEFAULT mixed-BLEND population it is **wrong**:
the clean bleed adds fundamental and no harmonics, so it pushes every Hn/H1 DOWN, and measured on this
matrix the reference's H3/H1 has **median −34.8 dB over the mixed set against −26.0 dB bleed-free — an
8.8 dB shift**. So a genuinely hot cell lands in a "low-drive" bin purely because A3 diluted it: the
bin coordinate would be contaminated by A3, which is precisely what a bleed-free harmonic statistic is
for. ⭐ **The split is now FORCED bleed-free whatever mode the rest of the table is in** (270 of 1368
cells), rather than relying on the user to remember `--bleed-free`. ⚠ The mixed-set version was not
merely noisier — it moved the low bin from 22 to 242 band values and would have read as the
better-populated, more trustworthy one.

**(3) ⭐⭐ THE SIGN REVERSAL SESSION 79 MEASURED ON THE 1 kHz LADDER IS REPRODUCED ON THE 129-CAPTURE
MATRIX, BY AN INSTRUMENT SHARING NO MACHINERY WITH IT.** Median `model − ND` per bin, bleed-free:

**H2 (`model − ND`, dB)** — the order the lever acts on:

| reference H3/H1 | n | ship | `a`=1.9 | `a`=2.0 | `a`=3.0 | `a`=4.0 |
|---|---|---|---|---|---|---|
| ≤ −42 (LOW anchor) | 22 | **−6.54** | **+0.53** | **+0.92** | +4.42 | +6.90 |
| −42 … −34 | 12 | −6.63 | +1.21 | +1.65 | +5.15 | +7.63 |
| −34 … −26 | 36 | +5.12 | +6.85 | +7.10 | +8.40 | +9.09 |
| −26 … −18 | 33 | +6.25 | +7.39 | +7.49 | +8.34 | +9.10 |
| −18 … −12 | 11 | +4.12 | +6.64 | +6.84 | +7.48 | +9.52 |
| > −12 (MID anchor) | 20 | +2.03 | +4.26 | +4.50 | +6.61 | +8.07 |

**H2−H3 (the session-79 statistic)**, same cells:

| reference H3/H1 | ship | `a`=1.9 | `a`=2.0 | `a`=3.0 | `a`=4.0 |
|---|---|---|---|---|---|
| ≤ −42 (LOW anchor) | **−2.61** | +4.72 | +5.13 | +8.83 | +11.52 |
| −42 … −34 | −3.80 | +4.47 | +4.94 | +8.70 | +11.37 |
| −34 … −26 | +3.69 | +5.82 | +6.14 | +7.63 | +8.59 |
| −26 … −18 | +5.87 | +7.12 | +7.23 | +8.41 | +9.30 |
| −18 … −12 | +4.90 | +8.04 | +8.29 | +8.34 | +10.44 |
| > −12 (MID anchor) | +4.66 | +8.38 | +8.69 | +11.66 | +13.17 |

Ship's H2−H3 runs **−2.61 → −3.80 → +3.69 → +5.87 → +4.90 → +4.66**: negative at the low-drive end,
positive from −34 dB up. Session 79's ladder read −7.93 at low and +10.94 at mid. **Same sign pattern,
different magnitudes — and they are NOT commensurate in level:** the swept anchors (100/200/400 Hz)
carry a **~14 dB** H2−H3 pair correction (session 78, measured) and **none of it is applied here**,
where the 1 kHz ladder's net model-vs-ND correction is **0.92 dB** (session 79). ⚠ Not to be confused
with session 78's *other* number, the −9 … −24 dB bridge from those anchors to the chart's 800 Hz
tone — a different quantity, and not what is being compared. ⭐ **What IS commensurate is the CHANGE per candidate, and
the two instruments agree there: the ladder moves d(low) +5.55 dB at `a`=1.9, the matrix +7.74 dB at
`a`=2.0** — no shared tone, anchor definition, stimulus segment, filter correction or capture
population. That corroborates the LEVER; the ~2 dB gap is the size of the disagreement, stated not
smoothed.

**(4) ⭐⭐ AND IT NARROWS THE FREE MOVE SHARPLY — THE HEADLINE CORRECTION TO SESSION 80.** In the
matrix's own domain the low-drive bin's H2 deficit is **6.54 dB, not the ladder's ~9.7**, and `a`=1.9
spends essentially all of it: **H2 −6.54 → +0.53, i.e. it lands ON ND** (interpolated crossing
**`a` ≈ 1.81**), `a`=2.0 → +0.92, while **`a`=3.0 overshoots to +4.42** and `a`=4.0 to **+6.90**.
⇒ **`a` ≈ 1.8–2.0 sits at the EDGE of the free move on this axis, not in the middle of a
1.9–3.0 admissible band.** ⭐ That the matrix's own optimum lands within 0.1 of the screen's lower
bound is a genuine convergence of two instruments on a VALUE, even though they disagree by 3.2 dB on
the size of the deficit (see the tone note below). Session 80's range was derived from the ladder's larger deficit; the matrix
is the ND reference in its own output domain, which is the domain a regression check is asked in, and
it says the headroom is smaller.
⚠ **Neither figure is "the" free move — it is TONE-DEPENDENT, and that is the whole of the 3.2 dB
disagreement.** Both are raw output-domain `model − ND` on H2 at low drive, bleed-free on both sides;
they differ because the matrix reads H2 at **200–800 Hz** (anchors 100/200/400) and the ladder at
**2 kHz**, and the model's and ND's linear responses diverge by different amounts in those two
regions — the recovery bridged-T's scoop sits in the first, the Sallen-Keys' rolloff in the second.
So the right reading is not "the ladder overstated it" but "the deficit is ~9.7 dB at 1 kHz and
~6.5 dB at the swept anchors", and **a regression check against the captures is owed the anchors'
number** because that is where the matrix scores. ⚠ The pair statistic H2−H3 goes 2.61 → 5.13 in that bin and reads as
a REGRESSION, because the model is low on H2 **and** H3 there (−6.54 / −3.94) and H2 crossing ND
inverts the sign of a difference that was never centred — `difference-statistics-hide-common-mode`,
which is why the absolutes are printed beside it and are what should be read here.

**(5) ✅ ROBUSTNESS OF THAT BIN, since 22 band values is thin.** It is **7 distinct capture files** ×
all four sweep levels, not one capture: `drive-0700_level-1700_{,attack-boost,attack-cut,grunt-flat}`,
`drive-1700_level-1700_grunt-boost`, `level-1700_{,attack-cut}`. **20 of 22 values are same-signed at
ship** and 20 of 22 move up at `a`=2.0. The neighbouring −42…−34 bin (12 values, disjoint) agrees on
both the level and the movement. ⚠ The two exceptions are `grunt-flat` rows where the model sits
**+13.0 / +9.7 dB ABOVE** ND already — GAP #3b's known-unfixed group, dragging the median the
conservative way (`defective-rows-must-not-vote`: flagged, and left in because excluding them would
make the finding look stronger).

**(6) ⚠⚠ THE POOLED ACCEPTANCE TEST DOES NOT DISCRIMINATE 1.9 FROM 3.0 — ONLY THE SPLIT DOES, WHICH
IS THE WHOLE POINT OF BUILDING IT.** Paired bootstrap over cells, bleed-free (10k resamples), Δmean|e|
vs shipped:

| | EVEN | odd (AUTHORITATIVE) | ALL |
|---|---|---|---|
| `a`=1.9 | −0.32 [−0.95, +0.39] | +0.27 [−0.20, +0.75] | −0.02 [−0.44, +0.41] |
| `a`=2.0 | −0.39 [−0.98, +0.20] | +0.25 [−0.25, +0.76] | −0.07 [−0.48, +0.32] |
| `a`=3.0 | +0.01 [−0.72, +0.72] | +0.16 [−0.51, +0.80] | +0.08 [−0.43, +0.56] |
| `a`=4.0 | +0.43 [−0.47, +1.27] | **+0.81 [+0.04, +1.58]** | **+0.61 [+0.00, +1.19]** |

**Only `a`=4.0 is significant** — on the odd (authoritative) column and on ALL. 1.9, 2.0 and 3.0 are
all statistically indistinguishable from shipped in every group, so **a reader who stopped at this
table would conclude 3.0 is as free as 1.9.** The split says otherwise (low-bin H2 +4.42 vs +0.53), and
it is right to be preferred here: pooling averages a bin where the candidate is spending real headroom
with five bins where it is already past ND, which is exactly the cancellation session 79 warned about
one level up. ⇒ this sharpens session 74's ranking (same ordering, reached on the mixed set with the
silence guard off) by making it bleed-free, paired and per-regime — and shows the pooled form is too
blunt to choose a value.

**(7) THE OTHER TWO AXES, both monotone and both small.** **FR** `OD ex gain-n12` **2.743 → 2.763 →
2.762 → 2.771 → 2.778** — monotone worse, no interior optimum, so FR pins nothing (session 74's
reading, reproduced), and **CLEAN is 0.408 at every point** (OD-path change, as required).
**THD** `rms(q)` **9.690 → 7.555 → 7.522 → 7.076 → 7.082**, monotone BETTER, carried by the **level**
term (6.202 → 4.313 → 4.277 → 3.783 → 3.731). ⚠ Read that with session 74's own warning rather than as
support: the model under-produces THD overall, so raising an even coefficient improves an AMOUNT
statistic whatever the shape is doing — **this is the axis on which a shape parameter absorbs a level
error**, and it is not evidence for the candidate. It is also the one axis that would prefer 3.0.

**(8) ⚠ TWO CELLS THAT MUST NOT BE READ, flagged because they sit in the headline bins.** `a` is a
smooth parameter, so a per-order median that is NON-MONOTONE in it is contaminated, not informative.
Measured: **H6 in the ≤−42 bin** runs +12.80 → +4.88 → +3.84 → **−15.34** → +3.77, and **H7 in the
−42…−34 bin** runs −7.68 → −16.76 → −20.81 → −7.39 → −2.60 — both wild, both in the two thinnest bins,
and both at orders that sit near the extractor floor at low drive. **H2, H3 and H4 are monotone in
every bin**, which is what the conclusions rest on. ⚠ This is the same class as the `--no-silence-guard`
lesson: a median over few cells at the floor reports the floor's arithmetic, not the model.

**(9) ⚠ NOT CLAIMED / STILL NOT PROPOSED.** Nothing shipped. ⭐ The matrix **does** settle session 80's
open 1.9-vs-3.0 question, and it settles it **against 3.0**: the free move is spent by `a` ≈ 1.8–2.0,
so 3.0 is 4.4 dB past ND at the low anchor and is not inside a band this instrument supports. But that
is a statement about where the FREE move ends, **not** an argument that `a` ≈ 1.9 should ship — it says
only that moving there costs nothing measurable against the captures. The square-law identity
`2·a·cn = 1` is still broken at any of these values (2.5 at `a`=1.9), and session 73's finding that
honouring it is jointly infeasible still stands. ⚠ The H4 disagreement between instruments is
unresolved: at the low anchor the 1 kHz ladder puts the model **+4.3 dB ABOVE** ND on H4 and predicts
the lever worsens it, while the matrix's low bin has it **−6.85 BELOW** and improving to +1.10 at
`a`=1.9 — different tone, different convention, and the matrix's H4 carries no filter correction. Do
not quote either as settled. ⚠ And this is still **model vs ND**, whose evens sit ~27 dB below
hardware's; what the matrix can say is only what a move costs against the captures, never that a move
is right.

---

### ⭐⭐ THE `a`-SWEEP WAS ALREADY DENSER THAN ANY SESSION READ IT — three unread 129-capture renders turn session 81's two-point extrapolation into a measurement, RAISE the free-move ceiling from 3.0 to ~3.5, and show the H4 half of the open disagreement rests on FIVE band values (session 82)

**Tooling + analysis only. NOTHING in `src/`, `tests/` or the captures touched, no constant moved,
NOTHING PROPOSED FOR SHIPPING.** No tool changed either — this is a re-read of existing renders
through the existing `matrix_harmonics.py` / `shape_gate.py`, plus two throwaway probes and **one
new 129-capture render (`a`=1.3)** made specifically to bracket the crossing §3 had to extrapolate.

#### 0. Baseline first — the session-81 record reproduces to the leaf

`matrix_harmonics.py --selftest` passes all four gates (identity 0.000e+00, common-mode recovery
0.000e+00, the model-side guard-bias demonstration 11.58 dB, the silent-capture MEAN/MEDIAN
contrast). Re-running session 81's exact five-report invocation and diffing every numeric leaf of
the result against the archived `analysis/reports/s81_matrix_harmonics.json` gives **0 differing
leaves**. Membership is unchanged at 1368 cells / 2775 band values / 270 bleed-free / 16 silent OD
rows, and adding the three extra reports leaves all four counts identical — so nothing below is an
`aggregate-moved-check-membership-first` artefact.

#### 1. ⭐ THE DATA WAS ALREADY ON DISK — `check-for-unread-data-first`, FOURTH occurrence

`analysis/reports/` holds **seven** 129-capture `jfetSatNeg` renders plus the baseline:
`a` = 1.9, 2.0, **2.5**, 3.0, **3.5**, 4.0, **5.6**. Session 81's split read four of them (1.9, 2.0,
3.0, 4.0) and session 74 had read 2.0 / 4.0 / 5.6 on a different statistic; **2.5 and 3.5 had never
been read by any session at all**, and 3.5 sits exactly in the gap session 81 could only bracket.
**Every finding in §4–§6 came out of renders that already existed** — the same pattern as session
60's `sweep_clean_-36`, session 78's 1 kHz `lvl_` ladder and session 81's own discovery that half its
step was already rendered. Only §2–§3 needed new work, and only because reading the existing points
revealed *where* the one genuinely missing render was: the empty 0.76 → 1.9 interval (§3).

#### 2. ⛔⛔ THE POOLED ACCEPTANCE TEST DEFINES NO FREE REGION — it is NON-MONOTONE in `a`, which refutes the first version of this very section

Paired bootstrap, bleed-free, Δmean|e| vs shipped (10k resamples; this is the population session 81
item (6) quotes, and 1.9 / 2.0 / 3.0 / 4.0 reproduce its numbers exactly):

Δmean|e| vs shipped, with 95 % CI; `*` = significant. 1.9 / 2.0 / 3.0 / 4.0 reproduce session 81's
numbers exactly:

| group | 1.3 | 1.9 | 2.0 | 2.5 | 3.0 | 3.5 | 4.0 | 5.6 |
|---|---|---|---|---|---|---|---|---|
| EVEN | **−0.44\*** [−0.78,−0.11] | −0.32 | −0.39 | −0.24 | +0.01 | +0.13 | +0.43 | **+1.25\*** [+0.26,+2.22] |
| odd | **+0.35\*** [+0.03,+0.72] | +0.27 | +0.25 | +0.17 | +0.16 | +0.42 | **+0.81\*** [+0.04,+1.58] | **+1.34\*** [+0.29,+2.34] |
| ALL | −0.05 | −0.02 | −0.07 | −0.04 | +0.08 | +0.27 | **+0.61\*** [+0.00,+1.19] | **+1.29\*** [+0.55,+1.99] |

⛔⛔ **MY OWN FIRST WRITE-UP OF THIS SECTION WAS WRONG, AND THE `a`=1.3 RENDER IS WHAT CAUGHT IT.**
Before 1.3 existed I had recorded the ladder as *"≤3.5 clearly free | 4.0 on the threshold | 5.6
clearly costly"* — a **threshold with a free region below it**. That is refuted: **`a`=1.3 is
SIGNIFICANT on the authoritative odd column (+0.35 [+0.03,+0.72]) while 1.9, 2.0, 2.5, 3.0 and 3.5
are not.** The significant set is **{1.3, 4.0, 5.6}**, the non-significant set **{1.9 … 3.5}**. That
is not a threshold.

⭐ **AND THE MECHANISM IS LEGIBLE, WHICH IS WHAT MAKES IT A FINDING RATHER THAN A CURIOSITY.** The
odd Δ is non-monotone in `a` (+0.35 → +0.27 → +0.25 → +0.17 → **+0.16** at 3.0 → +0.42 → +0.81 →
+1.34) while the **CI WIDTH grows monotonically** (0.69 → 0.95 → 1.01 → 1.19 → 1.31 → 1.40 → 1.54 →
**2.05**, a 3× spread). So at `a`=1.3 the degradation is *small but highly consistent across cells*
and clears the bar; by `a`=2–3.5 the per-cell spread has widened faster than the mean has grown and
a **larger** error fails to. Significance here is tracking coherence, not size.

⇒ ⭐⭐ **THE POOLED ACCEPTANCE TEST DOES NOT DEFINE A FREE REGION, AND THIS IS AN INDEPENDENT
CONFIRMATION OF SESSION 81 ITEM (6) FROM A NEW DIRECTION.** That item argued the pooled test "cannot
choose a value" because pooling averages the one bin spending headroom with five where the model is
already past ND. The denser sweep shows the same conclusion as a *shape* property of the statistic
itself: it is not monotone in the parameter, so "significant / not significant" cannot be read as
"costly / free". **Gate on the split, not the total** — now demonstrated, not merely argued.

⚠ What survives, stated carefully: **only `a`=5.6 is ROBUSTLY significant** (all three groups at
once). Both 1.3 and 4.0 are **marginal** — odd CI lower bounds **+0.03** and **+0.04**, which
two-decimal printing hides. And **2.5 and 3.5, which no session had read, are as indistinguishable
from shipped as 1.9 and 2.0 are** — so the matrix does not penalise 3.0 or 3.5. ⇒ session 81's "3.0
overshoots" is a statement about **ND's own H2 at the low anchor** (§3), *not* about measurable cost;
per session 80 item (1) ND lies *between* us and hardware at low drive, so passing ND there is not by
itself a fault — it is the point beyond which the matrix stops corroborating the move.

#### 3. ⭐⭐ THE CROSSING IS MEASURED AT 1.77, NOT EXTRAPOLATED — and the concavity prediction made before the render held

Low-drive bin (H3/H1 ≤ −42), median `model − ND`, forced bleed-free:

| order | ship | **1.3** | 1.9 | 2.0 | 2.5 | 3.0 | 3.5 | 4.0 | 5.6 |
|---|---|---|---|---|---|---|---|---|---|
| **H2** | **−6.54** | **−2.00** | +0.53 | +0.92 | +2.84 | +4.42 | +5.75 | +6.90 | +9.81 |
| H3 | −3.94 | −4.06 | −4.19 | −4.21 | −4.31 | −4.41 | −4.52 | −4.62 | −4.95 |
| H4 | −6.85 | −2.28 | +1.10 | +1.56 | +1.47 | +1.31 | +1.16 | +2.04 | +5.05 |
| H2−H3 | −2.61 | +2.05 | +4.72 | +5.13 | +7.16 | +8.83 | +10.26 | +11.52 | +14.76 |

⭐ **ROBUSTNESS IS MUCH STRONGER THAN SESSION 81 COULD SHOW.** It reported "20 of 22 same-signed at
ship and 20 of 22 moving up at `a`=2.0". Measured across the full 9-point sweep, **22 of 22 band
values are STRICTLY increasing in `a` at every step** — there is no cell anywhere in the low bin
that moves against the lever.

⭐⭐ **AND THE CROSSING IS NOW MEASURED, NOT EXTRAPOLATED — WITH A CONFIRMED PREDICTION ATTACHED.**
Session 81 read its crossing off the chord between the shipped point (`a`=0.76054) and `a`=1.9 — a
**1.14-wide interval with no render in it**, and, as the slope table below shows, the steepest and
therefore least reliable segment of the whole curve. This session rendered **`a`=1.3** (129 captures,
verified live at 34.2 dB on the OD path and **bit-identical on CLEAN**, as an OD-path parameter must
be) specifically to bracket it.

Before that render landed, the concavity argument below predicted the true crossing would fall
**below** 1.81. **Measured: the median curve crosses at `a` = 1.77**, now interpolated across a
0.6-wide bracket (1.30 → 1.90) instead of 1.14. **The prediction held.** Per-band-value crossings:
**20 of 22 cross inside the sweep, median `a` = 1.81, 10–90 1.08 … 2.07, full range 0.94 … 2.17.**
⚠ The two that never cross are both `drive-0700_level-1700_grunt-flat` rows, already **+13.0 / +9.7
dB above** ND at ship: GAP #3b's known-unfixed group, exactly as session 81 flagged.

⚠ **THE CURVE IS CONCAVE, SO EVEN 1.77 IS AN UPPER BOUND — just a much tighter one.** Median H2 local
slope, dB per unit `a`: **+8.41 (ship→1.3) → +4.22 → +3.93 → +3.84 → +3.15 → +2.66 → +2.31 → +1.82**
— falling at every one of the eight measured segments, and the new point sits exactly on that trend
rather than breaking it. A concave function lies *above* its chord, so the residual bias still points
the same way; over a 0.6-wide bracket at a local slope of ~4.2 dB/unit it is now small. ⇒ **quote the
crossing as `a` ≈ 1.8 (upper bound 1.77 on the median curve, per-band-value 10–90 spread 1.1 … 2.1)**,
not as a point estimate.

#### 4. ⚠⚠ THE BIN LABEL IS THE H2 COUNT ONLY — per-order support collapses with order, and session 81's item (8) had the right verdict for the wrong reason

`matrix_harmonics.py` labels each bin with its H2 band-value count ("22 H2 band values"). That is
**not** the support of the other orders in the same bin:

| bin | H2 | H3 | H4 | H5 | H6 | H7 |
|---|---|---|---|---|---|---|
| **H3/H1 ≤ −42 (LOW anchor)** | 22 | 17 | **5** | **1** | **1** | **1** |
| −42 … −34 | 12 | 12 | 9 | 7 | **3** | **2** |
| −34 … −26 | 36 | 36 | 36 | 34 | 26 | 24 |
| −26 … −18 | 33 | 34 | 33 | 34 | 29 | 30 |
| −18 … −12 | 11 | 11 | 10 | 11 | 10 | 11 |
| H3/H1 > −12 (MID anchor) | 20 | 20 | 20 | 20 | 20 | 20 |

Session 81 item (8) told the next session not to read **H6 in the ≤−42 bin** or **H7 in −42…−34**,
attributing it to non-monotonicity in "the thinnest bins at orders near the extractor floor". The
verdict is right and the reason is sharper: **H6-low and H7-low are n = 1** — a single band value
from a single cell (`drive-1700_level-1700_grunt-boost_base-od.wav / sweep_drv_-6 / band 2`). A
median over one value is that value.

⚠ **I nearly published the opposite reading of the same cell.** With the gaps filled, H6-low traces
a clean smooth V (+12.8 → +9.7 → +4.9 → +3.8 → −3.9 → **−15.3** → −1.0 → +3.8 → +10.2) with a
well-defined minimum near `a` ≈ 3.0 — exactly the shape of a cancellation null, and *not* what noise
looks like. It is very likely a real null in that one cell. But **n = 1 cannot support a claim
whatever shape it traces**, and the per-cell probe is what stopped it: the smoothness made the wrong
conclusion more attractive, not less. ⭐ **GENERAL: a suggestive SHAPE is not a substitute for
support — check n before interpreting a trend, especially when the trend looks physical.**

#### 5. ⭐⭐ H4 IS NOT MONOTONE ONCE YOU HAVE NINE POINTS, AND THAT WEAKENS THE MATRIX'S HALF OF THE OPEN H4 DISAGREEMENT

Session 81 item (8) closed with *"H2/H3/H4 are monotone in every bin and are what the conclusions
rest on."* On the denser sweep **H2 survives that completely** (monotone in all six bins, and 22 of
22 band values monotone in the low bin, §3); H3 is monotone in **4 of 6** bins, wiggling only in the
two thin interior ones (−18…−12 and the mid anchor). **H4 does not.** Its low-bin median
runs +1.10 → +1.56 → **+1.47 → +1.31 → +1.16** → +2.04 → +5.05: an interior maximum at 2.0 and a
minimum at 3.5 that five points could not see.

⭐ **The cause is n = 5, and it is a median-crossover, not physics.** The five band values' own ship
values are **−6.9, −14.7, +15.1, +2.0, −16.4** — a **31.5 dB** spread — and their trajectories
disagree in *sign*: three rise strongly with `a`, one falls, one rises from deep negative. As the
candidate moves, *which* cell is the middle one changes, and the median hops between them.

⇒ **the matrix's half of session 81 item (9)'s H4 disagreement is far more weakly supported than the
handover implies.** That item sets the 1 kHz ladder's "+4.3 dB ABOVE ND" against the matrix's
"−6.85 BELOW", as two comparable readings from two instruments. The −6.85 is the median of five
values spanning 31 dB with inconsistent signs. **The disagreement is NOT hereby resolved in the
ladder's favour — but the two sides should no longer be quoted at equal weight, and no H4 conclusion
should rest on the matrix's low bin.**

#### 6. THE OTHER TWO AXES, RE-READ ON THE DENSE SWEEP

**FR** (`OD ex gain-n12`, rms(a)): 2.743 → 2.763 → 2.762 → **2.755** → 2.771 → **2.768** → 2.778 →
2.794. Session 81 called this "monotone worse, no interior optimum". It is **not monotone** — 2.5
and 3.5 both sit below their lower-`a` neighbours. But the whole spread across a **7×** change in
`a` is **0.05 dB**, and the wiggle is ~0.01 dB, an order of magnitude under the 0.144 dB floor. ⇒
**session 81's CONCLUSION stands unchanged and is if anything strengthened: FR pins nothing.** Only
its "monotone" descriptor was wrong, and nothing rested on it.

**THD** (`OD ex gain-n12`, rms(q)): 9.690 → 7.555 → 7.522 → 7.259 → 7.076 → **7.021** → 7.082 →
7.407, with the **level** term 6.202 → 4.313 → 4.277 → 3.946 → 3.783 → **3.691** → 3.731 → 4.059.
⇒ there is a genuine **interior optimum at `a` ≈ 3.5**, worse on both sides, carried by the level
term — session 81 saw only 7.076 → 7.082 and read the axis as monotone.

⛔ **This is NOT support for 3.5, and the coincidence is the warning.** The model under-produces THD
overall, so raising an even coefficient improves an AMOUNT statistic no matter what the shape does —
session 81 named this "the axis on which a shape parameter absorbs a level error". That axis now has
a sharp, well-located optimum sitting **exactly at the top of the statistically-free range** (§2).
A future session that fits `a` on THD will land on 3.5 and find it corroborated by the acceptance
test, and both readings will be the same artefact seen twice.

#### 7. ⚠ ARTEFACT HYGIENE — the archived session-81 JSON is a DIFFERENT population from the numbers beside it

`analysis/reports/s81_matrix_harmonics.json` is the **mixed-BLEND** run, but CLAUDE.md item (6) and
this document quote the **bleed-free** bootstrap. Read straight out of the JSON, `odd/a1.9` is
**+0.76 [+0.55, +0.97] SIGNIFICANT**, which flatly contradicts the prose's "+0.27, CI spans zero" —
and a future session checking the record against the file would conclude the handover was wrong. It
is not: both runs are correct and the split section is forced bleed-free either way. **The stored
artefact simply does not carry the flag the quoted numbers were taken under.** Anything quoted from
that file must state `--bleed-free` or not.

#### 8. ⚠ WHAT IS NOT CLAIMED

Nothing is proposed for shipping and no value is preferred. §2 raises the ceiling on what the matrix
*permits*; it does not argue for a move, and session 81's central point — that the matrix bounds the
free move but cannot select inside it — is unchanged. The crossing in §3 remains an extrapolation
across an interval with no render (§3's concavity argument makes 1.81 an upper bound, not a
measurement). The `2·a·cn = 1` square-law identity is still broken at every value here and session
73's joint-infeasibility finding still stands. All of this is **model vs ND**, whose evens sit ~27 dB
below hardware's. And `jfetGm` is still not swept — the session-4 anchor every nonlinear fit rests on.

⚠ **SUPERSEDED IN PART BY SESSION 83 (below):** "session 73's joint-infeasibility finding still
stands" does NOT survive — that verdict was scored against the chart's +17.2 dB requirement on a
4-point grid, and against the corrected requirement the identity goes from 3.25× over budget to
~2 % over. See §5 of the session-83 entry.

### ⭐⭐ SESSION 82's NEXT-STEP (a) — THE H4 DISAGREEMENT IS NOT A DISAGREEMENT (both halves are medians over ~30–40 dB spreads), AND SESSION 73's "THE IDENTITY IS JOINTLY INFEASIBLE" DOES NOT SURVIVE THE CORRECTED REQUIREMENT (session 83)

**Tooling + analysis only. NOTHING in `src/`, `tests/` or the captures touched, no constant moved,
NOTHING PROPOSED FOR SHIPPING.** Two tools changed, both additively: `analysis/even_low_screen.py`
(H4 support + guarded read + the paired/unpaired labelling) and `analysis/matrix_harmonics.py`
(per-order `n` on every split row). Session 82's next-step (a), both halves.

#### 0. Baseline first — the session-79/80 record reproduces to 0.00 dB

`even_low_screen.py` at the shipped point, before anything was touched: **GATE B low −7.93 vs
recorded −7.93, mid +10.94 vs +10.94, |d| 0.00 dB at both anchors**; 22 low-anchor / 16 mid-anchor
captures; H4 low-drive **+4.30 dB**, reproducing session 80 item (5) exactly. `matrix_harmonics.py
--selftest` passes all four gates after the edit (identity 0.000e+00, common-mode 0.000e+00,
guard-bias 11.58 dB, silent-capture MEAN/MEDIAN contrast), and the split's every median is
bit-unchanged by the per-order-`n` addition.

#### 1. ⛔ MY GOING-IN SUSPICION ABOUT THE LADDER's H4 WAS REFUTED — the support is fine

The anchored subsets (`msub`/`nsub`) select on **`ok23`** — H2 AND H3 measurable — because the
headline is the H2−H3 pair. **`ok45` is computed per hit and never consulted**, so every H4 number
printed since session 80 is a median over cells selected for H2/H3 reliability and not for H4's.
Since a floored `Hn` reads too HIGH (the LS fit returns whatever noise projects onto that basis
vector, bounded below by the cell's residual), and a floored MODEL cell biases (model − ND) UPWARD —
exactly the direction of the claim — this looked like the ladder-side twin of session 82's n=5.

**It is not.** Measured: reference H4 clears its own residual by ≥6 dB in **21 of 22** low-anchor
cells and **16 of 16** mid (H2 control: 22/22 and 16/16), and the **model** side is measurable in
**22/22** and **16/16**. So the specific flooring mechanism is not operating on either side.

#### 2. ⭐⭐ BUT THE ROW IS STILL UNQUOTABLE, FOR A DIFFERENT REASON — a 41.9 dB SPREAD

Dropping the ONE unmeasurable reference cell moves the low-drive shipped value
**+4.30 → +2.43 dB** unpaired / **+4.45 → +3.04 dB** paired — **1.4–1.9 dB from one cell in 22.**
That sensitivity is the tell: the per-cell `d(H4)` distribution at the low anchor runs
**−18.1 … +23.8 dB (41.9 dB wide)**, p10 −8.4 … p90 +18.7. Mid drive is 21.6 dB wide.

⇒ **BOTH halves of the H4 disagreement are medians over ~30–40 dB-wide distributions.** The ladder
half is far better SUPPORTED than the matrix half (n = 21 vs n = 5) but is MORE DISPERSED
(41.9 dB vs 31.5 dB). The gap between the two instruments (+4.3 dB ladder vs −6.85 dB matrix,
~11 dB) is **smaller than either instrument's own dispersion.**

⇒ **The open "H4 disagreement" is not a disagreement to be adjudicated — it is two weak summaries of
the same badly-dispersed quantity, and neither instrument can carry an H4 conclusion.** Session 82
demoted the matrix half on SUPPORT; this demotes the ladder half on DISPERSION. Stop quoting either
absolute level.

#### 3. ⭐ WHAT SURVIVES ABOUT H4: the DIRECTION over the sweep — but NOT step-by-step monotonicity

| candidate | low unpaired | low PAIRED | low paired+guarded | mid unpaired | mid PAIRED |
|---|---|---|---|---|---|
| shipped | +4.30 | +4.45 | +3.04 | +1.76 | +1.98 |
| a=1.2 | +4.46 | +4.63 | **+2.90** | +2.44 | **+1.24** |
| a=1.9 | +4.92 | +5.58 | +3.65 | +2.77 | +3.75 |
| a=3.0 | +6.04 | +7.78 | +6.02 | +5.38 | +6.12 |
| a=4.5 | +8.07 | +8.69 | +8.45 | +8.38 | +8.13 |

Across the whole sweep H4 rises by **+3 to +6 dB on every convention**, so session 80 item (5)'s
qualitative claim — the model already sits ABOVE ND on H4 while sitting BELOW on H2, so the same
lever improves H2 and worsens H4 — **survives in DIRECTION**.

⚠ **It is NOT strictly monotone, and an earlier reading of this session's own first run said it
was.** That reading used the UNPAIRED column only, where it is monotone. On the PAIRED statistic
there is a **dip at `a`=1.2** at both anchors (low+guarded 3.04 → 2.90; mid 1.98 → 1.24). The dips
are 0.14 and 0.74 dB against per-cell spreads of 41.9 and 21.6 dB, i.e. exactly the scale of
structure such a distribution manufactures — which is the same lesson as §2 rather than a separate
one. **Quote the direction over the sweep; do not read step-by-step ordering, and never the level.**
Controls behave: `CTL jfetSatPos` +3.11, `CTL jfetCeilNeg` +3.03 against shipped's +3.04
(low, paired+guarded).

#### 4. ⚠ A DEFECT IN MY OWN ADDITION, caught by reading its output against itself

The first version printed `median(model) − median(ND)` (session 80's convention, +4.30) on the
candidate row and `median(model − ND)` (the PAIRED statistic, +4.45) in the summary line —
**two different statistics side by side, unlabelled**, differing by 0.15–0.6 dB. The cells are
paired, so the second is the correct one. Both are now printed and each is named; the dropped cell
is printed with its own `d(H4)` and residual so the guard can never be a silent filter.

#### 5. ⭐⭐ THE OTHER HALF — THE `2·a·cn = 1` IDENTITY, AND SESSION 73's VERDICT DOES NOT SURVIVE

Session 73 recorded the identity-honouring family as **JOINTLY INFEASIBLE** with the correction.
Two things about that verdict have expired, and neither is a re-measurement of the shaper:

**(a) It was scored against a requirement that has since moved by 2×.** Session 73 needed
**d(low) = +17.2 dB** to reach the CHART's hardware column, and its optimum sat at `a` ≈ 4.0–5.6.
Session 78 demoted that chart column; session 79 measured the requirement against **ND** at
**7.9 dB**; session 81 measured it in the **matrix's own output domain** at **6.5 dB**, spent by
**`a` ≈ 1.77–1.81** (session 82, upper bound; per-band-value 10–90 spread 1.1 … 2.1).

**(b) Its feasibility test was a 4-point grid that only BRACKETED the boundary.** Session 73 swept
Vov = 0.60 / 0.30 / 0.18 / 0.09 and refused 3 of 4 as non-monotone — **which reproduces exactly
here** — leaving the fold-back boundary known only to lie in `a` ∈ [1.667, 3.333].

**Located exactly, and validated two independent ways.** Bisecting `min_slope` along `cn = 1/(2a)`:
at shipped `s` the identity caps **`a` at 1.7709**. Cross-checked against a finite-difference of an
INDEPENDENT transcription of `JfetStage.h`'s `coreLimit()`/`waveshape()` — agreeing to **5 decimal
places** at s = 0.4559 / 0.30 / 0.20, so the analytic `T'(w)` is right.

⭐ **And the cap is not on `a` — it is on the PRODUCT.** Sweeping `s`, the identity-feasible
`a_max` tracks **`a·s ≲ 0.798–0.807`**, against **`< 2.598`** without the identity, i.e. the identity
tightens the fold-back budget by **3.25×**. (This REPRODUCES session 44's recorded "the square law
caps `|a|·s` at ~0.80–0.95 (β-dependent)" — my β sweep gives 0.798 at β=0 rising to 0.833 at β×4.)
`cp` has **no** influence whatever (boundary identical to 4 dp across a 4× range), so `cn` is the
binding side, as the mechanism requires.

⇒ **The arithmetic of the verdict flips.** Session 73's `a` ≈ 5.7 at shipped `s` needs
`a·s` ≈ 2.6 = **3.25× over** the identity's budget. The corrected requirement, `a` ≈ 1.8, needs
`a·s` ≈ 0.82 = **~2 % over** — reachable either by taking `a` in the lower part of its own interval
or by shrinking `s` by ~3 %. **The identity and the correction are no longer jointly infeasible.**

⚠⚠ **BUT THE REPLACEMENT VERDICT IS "EXACTLY MARGINAL", NOT "COMPATIBLE".** The identity permits
the free move only by sitting essentially ON its own fold-back boundary —
`bound-resting-means-unidentified`, which sessions 47/51/73 all treat as uninformative. Buying
clearance means moving `s`, a second session-44 fitted constant with its own provenance and its own
matrix exposure.

⚠ **AND THE NEAR-COINCIDENCE IS A COINCIDENCE — I nearly built the finding on it.** The identity's
cap at shipped `s` is **1.7709** and the matrix's measured crossing is **1.77**. These share NO
machinery: one is algebra on the waveshaper, the other a measurement against the ND captures. There
is no mechanism by which ND's H2 level should know about our shaper's fold-back point. It is
session 82 item (4)'s lesson at full strength — **a suggestive number is not support** — and the
argument above deliberately rests on the `a·s` budget (3.25× → 2 %), which does not depend on it.

#### 6. ⭐⭐ AND THE SCREEN CONFIRMS IT EMPIRICALLY — HONOURING THE IDENTITY IS FREE, AND AN IDENTITY-HONOURING POINT WINS THE SCREEN OUTRIGHT

§5 is arithmetic (a feasibility boundary and a requirement). It leaves one thing untested: whether
tightening `cn` from 0.657 to ~0.26–0.29 to honour the identity SPOILS the correction — the
`CTL ceilNeg` row was only ever measured at `cn` = 0.33. Two identity-honouring candidates were
therefore rendered and screened on the same 1 kHz axis. GATE B reproduced first at **0.00 dB** at
both anchors; GATE L shows both new candidates live at the anchor (|dH2| 6.28 and 6.98 dB) against
the two controls at **0.032 and 0.003 dB**.

⭐ **PRE-REGISTERED, before the renders were read:** interpolating session 80's free-`a` sweep and
allowing for `cn`, SQ a=1.75 should give d(low) ≈ −2.8…−3.0 and SQ a=1.9 s=0.40 ≈ −2.2 dB.

| candidate | `2·a·cn` | `a·s` | d(low) | d(mid) | gain | cost | net |
|---|---|---|---|---|---|---|---|
| shipped | 1.000 | 0.347 | −7.93 | +10.94 | — | — | — |
| a=1.9 (free) | 2.50 | 0.866 | −2.38 | +12.40 | +5.56 | +1.46 | +4.10 |
| a=3.0 (free) | 3.94 | 1.368 | +1.61 | +13.46 | +6.32 | +2.52 | +3.80 |
| **SQ a=1.75** | **1.000** | 0.798 | **−3.00** | +12.23 | +4.93 | +1.29 | +3.64 |
| **SQ a=1.9 s=0.40** | **1.000** | 0.760 | **−2.28** | +12.40 | +5.66 | +1.46 | **+4.20** |

**Both predictions held** (−3.00 against −2.8…−3.0; −2.28 against −2.2).

⭐⭐ **The identity costs NOTHING measurable on this axis.** `SQ a=1.9 s=0.40` and the free `a`=1.9
differ by **0.10 dB at the low anchor and 0.00 dB at the mid anchor**, despite the identity-honouring
version tightening `cn` by 2.5× and shrinking `s` by 12 %. So the "physically coherent" form of the
move is not a compromise version of it — it is the same move.

⇒ **the screen's own computed verdict picks an IDENTITY-HONOURING point as best low-drive movement
net of mid-drive cost.** ⚠ But read that margin honestly: it beats free `a`=1.9 by **0.10 dB**, which
is noise. The defensible statement is **"indistinguishable from the best free point"**, not "better".
And even at SHIPPED `s`, with no knee change at all, `SQ a=1.75` closes **62 %** of the low-drive gap
(7.93 → 3.00 dB) for 1.29 dB of mid-drive cost.

⚠ **NOT PROPOSED, and the reasons are unchanged from session 82.** The 129-capture matrix has not
judged either identity point, and per `reference-sources.md` §1(0) it must be expected to improve the
LOW-drive regime and regress the MID — both reported, never a pooled total. This is model-vs-ND
throughout, and ND's evens sit ~27 dB below hardware's. `jfetGm` is still not swept. What changes is
narrower and worth stating exactly: **session 82's framing that "a proposal must CHOOSE between the
corroboration and the correction" no longer holds. The square-law corroboration is available at
essentially no cost — but only inside a fold-back budget 3.25× tighter than the unconstrained one,
which the corrected requirement can now afford and session 73's could not.**

---

### ⛔⛔ SESSION 83's NEXT-STEP (a) — THE MATRIX JUDGES THE IDENTITY, AND THE ANSWER IS NEGATIVE: honouring `2·a·cn = 1` is NOT free, it buys a fraction of a dB on the NON-authoritative even column and pays multiples of it on the AUTHORITATIVE odd one, coherently, in BOTH drive regimes (session 84)

Session 83 measured the identity-honouring point `SQ a=1.9 s=0.40` at **0.10 dB and 0.00 dB** from the
free `a`=1.9 on the 1 kHz screen's two anchors, and recorded it as *"free in practice, not merely
affordable"* — with the explicit caveat that the 129-capture matrix had never judged either point.
It has now. **The identity is not free there.**

Tooling + analysis only. **NOTHING in `src/`, `tests/` or the captures was touched, no constant moved,
and NOTHING is proposed for shipping.** One tool changed, additively (`matrix_harmonics.py`).

#### 0. ✅ Baseline first, to the leaf — and three independent figures reproduced

`matrix_harmonics.py --selftest` passes all four gates. Session 83's exact 9-report bleed-free split
reproduces `analysis/reports/s83_matrix_harmonics.json` with **927 shared leaves, 0 differing, worst
|Δ| 0.000e+00**, and membership unchanged at **270 cells / 661 band values / 199 silent OD rows**.
`shape_gate` reproduces the recorded FR `OD ex gain-n12` rms(a) **2.743**, THD rms(q) **9.690** with
level term **6.202**, and CLEAN **0.408**. `build/OfflineRender_artefacts/Release/OfflineRender`
predates every report in use and nothing in `src/`/`tests/` is newer, so the binary is current — checked,
not assumed (`rebaseline-all-derived-artefacts`).

#### 1. ⭐ `check-for-unread-data-first`, FIFTH occurrence — half the step was already on disk

The handover asked for two 129-capture renders. **Free `a`=1.9 already existed** as
`analysis/reports/s80_matrix_a1.9.json`, verified commensurable with the baseline on all nine
measurement-condition fields (OS 8, same 30 bands, same THD anchors, same orders, same sweep levels) —
`--fit` renders made in different sessions are not automatically comparable. Only the identity-exact
point needed rendering: one render, not two. Same pattern as sessions 60, 78, 81 and 82.

#### 2. ⭐ The candidate is MONOTONE, and the check reproduces the mechanism `s` exists to fix

Scored against `fit_nonlinear.min_slope()` — the project's own feasibility oracle, not a new one:

| point | a·s | 2·a·cn | min slope | verdict |
|---|---|---|---|---|
| shipped | 0.3467 | **1.0000** | 4.077e-06 | monotone |
| free `a`=1.9 | 0.8662 | 2.4982 | 4.077e-06 | monotone, identity broken |
| **`SQ a=1.9 s=0.40`** | 0.7600 | **1.0000** | 2.190e-07 | **monotone — the render is valid** |
| `SQ a=1.9` at *shipped* `s` | 0.8662 | 1.0000 | **−3.624e-02** | **FOLDS BACK** |

The last row is why `s` has to move at all, and it reproduces session 73's fold-back mechanism exactly.
⚠ The `a·s ≲ 0.80` budget session 83 measured holds: 0.76 clears it with ~5 % margin.

⭐ **But "min slope" is a TAIL value (it occurs at the scan edge, |w| = 60), so it is the wrong number
to quote for in-band behaviour.** The informative one is where the map starts compressing: the |w| at
which the slope first falls below 0.10 is **1.902 (shipped) / 1.898 (free a=1.9) / 0.409 (SQ)** — the
identity point's cutoff-side knee sits **4.6× closer to the origin**, because the identity forces `cn`
down 2.5× (0.657 → 0.263) as `a` rises. That is a measured property of the shipped shaper, and §5 shows
the matrix cost lands exactly where it predicts.

#### 3. ✅ Plumbing verified BOTH ways — and my own gate was wrong first

All three keys (`jfetSatNeg`, `jfetCeilNeg`, `jfetSatPos`) are J201 parameters and the J201 is in the
OD path only, so CLEAN must be bit-identical and the OD path must be live. Verified via `--print-fit`
before launching that all three reach the DSP (not the session-59 zsh word-splitting trap), then on the
finished render: **43 CLEAN bit-identical | 80 OD-engaged all live (worst |Δ| 115.2 dB) | 6
OD-out-of-circuit exactly inert → PASS.**

⚠⚠ **My first version of that gate required "every OD capture must move", and it FAILED on
`s80_matrix_a1.9.json` — a render session 81 had already established as sound.** The six offending rows
are `blend-0700*` (BLEND = 0 ⇒ the OD path is out of circuit entirely, which session 59 item 2 verified
directly) and `level-0700` (LEVEL = 0, the session-74 silent capture). Those rows are **required** to be
inert. Rebuilt as three groups with the inert rows as their own **POSITIVE** check rather than an
exclusion — a movement there would be as much a fault as a CLEAN one — which is strictly more
discriminating than dropping them. ⭐ **GENERAL: validate a new gate against a case whose answer is
already known; that is the only thing that separates "the candidate is bad" from "my gate is bad".**
⚠ Incidental refinement: session 81 recorded *"all 86 OD captures live"*; on this statistic 6 of the 86
are exactly inert by construction. Its conclusion (OD-path-only and live) is unaffected.

#### 4. ⚠⚠ MY OWN FIRST READING OF THE RESULT WAS 4 dB TOO LARGE, AND THE TRAP IS IN THE TABLE

The split's per-order table is a table of LEVELS. Differencing two of its columns by eye gives
**median(SQ) − median(a1.9) = −5.46 dB** on low-bin H3, and that is what I nearly published. The cells
are PAIRED (one shared mask by construction), so the correct statistic is **median(SQ − a1.9) = −1.45
dB**. Medians are not additive; the two differ by 4 dB.

⭐ Session 83 fixed this exact class one level down — `even_low_screen.py` printed `median(m) − median(ND)`
beside `median(m − ND)` unlabelled, 0.15–0.6 dB apart. It survived here because this table had **no
paired column at all**. So the fix went into the tool, not into prose: the split now prints
**`PAIRED median of (cand − ref)` per order**, and **COMPUTES** the worst disagreement with the
difference-of-medians rather than asserting one, so the warning cannot go stale
(`computed-verdicts-not-narrated`). It fires at **7.27 dB (low bin, H4 `a`=2.0)** and **4.69 dB (mid
bin, H5 `a`=5.6)** — i.e. on exactly the thin rows sessions 82/83 demoted. ✅ Proven a **STRICT
SUPERSET: 1024 shared JSON leaves bit-identical, 0 lost, 360 new**, all four selftest gates still PASS.

#### 5. ⛔⛔ THE RESULT — the identity costs the AUTHORITATIVE column in BOTH regimes

Paired medians of `(candidate − shipped)` per cell, from the tool's own new row, on the rows with real
support (H4/H5/H6/H7 in the low bin are n = 5/1/1/1 and **do not vote** — session 82 item 4):

| regime | order | n | free `a`=1.9 | **`SQ a=1.9 s=0.40`** | authority |
|---|---|---|---|---|---|
| **LOW** (H3/H1 ≤ −42) | H2 | 22 | +7.92 | **+8.15** | none (even; ND runs ~27 dB low) |
| **LOW** | **H3** | 17 | **−0.10** | **−3.27** | **AUTHORITATIVE (ND == hardware)** |
| **MID** (H3/H1 > −12) | H2 | 20 | +2.28 | **+2.63** | none (even) |
| **MID** | **H3** | 20 | **−0.66** | **−1.44** | **AUTHORITATIVE** |
| **MID** | H5 | 20 | −0.75 | −0.80 | **AUTHORITATIVE** |

And directly paired against the free move (`SQ − a1.9`, the comparison the step exists to make):

| regime | order | n | paired median | paired mean | coherence |
|---|---|---|---|---|---|
| **LOW** | H2 | 22 | **+0.23** | +0.49 | 21/22 UP (better) |
| **LOW** | **H3** | 17 | **−1.45** | **−5.37** | **14/17 DOWN (worse)** |
| **MID** | H2 | 20 | **+0.30** | +1.07 | 17/20 UP |
| **MID** | **H3** | 20 | **−0.42** | **−2.71** | **17/20 DOWN** |
| MID | H5 | 20 | −0.06 | −0.12 | 12/20 |

⇒ **In both drive regimes the identity buys a fraction of a dB on the even column — which carries NO
authority, ND being ~27 dB below hardware there — and pays a multiple of it on the odd column, which is
the ONE fully-trustworthy harmonic target we have.** At the low anchor that is 0.23 dB bought for
1.45 dB paid; against shipped it is a 33× difference in odd-order cost (−0.10 vs −3.27).

⭐ **It is a COHERENT degradation, not a median hop, and checking that is what makes it publishable.**
The adjacent bin (−42…−34) moves H3 the *other* way (+0.81 on levels), and two thin neighbouring bins
disagreeing in sign is precisely the artefact session 82 item (5) diagnosed for H4 — and n = 17 clears
the tool's flag threshold, so nothing would have warned me. Read per cell: **14 of 17 low-bin band
values and 17 of 20 mid-bin band values move the same way.** ⚠ But the distribution is heavy-tailed —
paired median −1.45 against paired mean −5.37 — so quote both: most cells pay ~0–3 dB and a minority pay
far more.

⭐⭐ **AND THE COST CONCENTRATES EXACTLY WHERE §2's KNEE SHIFT PREDICTS.** Splitting the low-bin H3 cells
by DRIVE: **drive MIN (`drive-0700`, n=11) paired median −4.27 / mean −7.40 / worst −24.79**, against
**drive noon (`level-1700`, n=6) −1.29 / −1.66 / −3.04**. At drive min the clipper is idle so the J201's
own small-signal map dominates, and that is where a cutoff-side knee moved 4.6× toward the origin bites.
The mechanism is coherent with the signs too: an earlier one-sided compression onset makes the
small-signal map MORE asymmetric, which raises H2/H1 and suppresses H3/H1 — i.e. **the benefit and the
cost are two faces of one mechanism, not independent effects that could be traded apart.** ⚠ Stated as
a mechanism consistent with the data (the knee location and the drive concentration are measured; the
harmonic consequence is standard asymmetry theory plus the observed signs), not as a measured causal chain.

#### 6. ⭐⭐ WHY THE SCREEN COULD NOT SEE THIS — it is a documented blind spot firing, not an error

Session 83's screen scored `d(H2−H3)` and put the two points 0.10 dB apart. **`H2−H3` cannot see this
cost, because session 80 item (4a) already established that the screen ANCHORS on each side's own H3/H1
crossing, so H3 is pinned BY CONSTRUCTION** — that is exactly why its odd-order control was found
vacuous and had to be rebuilt on H5. The same construction that makes the control vacuous makes an H3
*regression* unobservable. The matrix bins on the reference's H3/H1 but scores the model's absolute H3
against ND's, so it can see it. ⇒ **the identity's cost lands precisely on the quantity the screen's own
anchoring makes invisible.** Session 83's measurement was not wrong; its instrument was blind here, and
it said so itself. ⭐ **GENERAL: a candidate that scores "free" on a difference statistic must be
re-scored per ORDER before that is believed — and check what the instrument ANCHORS on, because an
anchored quantity cannot register a cost.**

#### 7. ⛔ The other three axes: one is silent, one is a trap, one confirms the plumbing

- **Pooled paired bootstrap (10k resamples, bleed-free):** `SQ1.9` is **NOT significant in any group** —
  EVEN −0.30 [−1.02, +0.46], odd **+0.44 [−0.37, +1.26]**, ALL +0.07 [−0.50, +0.61]. It reads as free,
  and as indistinguishable from the free move (+0.44 vs +0.27, CIs heavily overlapping). ⇒ **THIRD
  independent confirmation that the pooled test ranks nothing** (session 81 item 6 argued it, session 82
  item 2 showed it is non-monotone in `a`, this shows it positively misleading on a candidate the split
  convicts). Its a1.3 / a4.0 / a5.6 flags reproduce session 82's exactly, so the instrument is unchanged.
  **Gate on the split, never the total.**
- **FR** `OD ex gain-n12` rms(a): ship **2.743** → free a1.9 **2.763** → SQ **2.766**. Reproduces
  session 82's recorded a1.9 figure exactly; the SQ−a1.9 gap is **0.003 dB**, two orders under the
  0.144 dB floor. **FR pins nothing here either.**
- **THD** rms(q) / level: ship 9.690/6.202 → a1.9 **7.555**/4.313 → SQ **7.425**/4.120. SQ is *better*.
  ⛔ **NOT support — this is session 82 item (6)'s artefact a third time.** The model under-produces THD,
  so tightening an even-order/ceiling parameter improves an AMOUNT statistic whatever the shape does. The
  one axis that prefers the identity point is the one already known to reward it for the wrong reason.
- **CLEAN** 0.461 / rms(a) 0.408, bit-identical for both candidates — the OD-path-only claim confirmed on
  a third independent instrument.

#### 8. ⚠ What is and is not claimed

**Settled:** honouring `2·a·cn = 1` at `a` = 1.9 costs the authoritative odd column, coherently, in both
drive regimes, and the cost is ~6× the even-order benefit at the low anchor and ~2× at the mid. Session
83's "free in practice" does **not** survive the matrix, and the reason is structural (§6), not a
measurement error. Session 83's *other* half — that session 73's joint-infeasibility verdict has expired,
and that the identity is **affordable** within a 3.25× tighter `a·s` budget — is untouched and stands;
what falls is only "free".

**NOT settled / not claimed.** Nothing is proposed, and this does not select free `a`=1.9 either: per
session 82 the matrix cannot rank inside its free region, and per session 81 it can only say what a move
COSTS against the captures, never that a move is RIGHT. **`SQ a=1.75` at shipped `s` was deliberately NOT
rendered** — it sits essentially ON its fold-back boundary (measured min slope 2.82e-07 against a cap of
`a` ≤ 1.7709, i.e. ~1 % margin in `a`), so it is an unidentified point (`bound-resting-means-unidentified`)
and a third render would conflate a different `a` with the identity change rather than isolating it.
Model vs **ND** throughout. `jfetGm` still not swept — the session-4 anchor every nonlinear fit rests on.
⚠ The H4 rows remain unreadable at both anchors (n = 5 low, and session 83 demoted the ladder half on
41.9 dB of dispersion); nothing here re-opens them.

### ⭐⭐ THE HARMONIC-AXIS A3 INSTRUMENT IS BUILT, AND IT MEASURES A3 WITH NO SOLVE, NO TAPER, NO b0 AND NO BLEED MODEL — one constant OD-vs-bleed balance error of **k = −6.50 dB** explains the whole 0–43 dB dilution range on ONE parameter, and it independently reproduces the ATTACK gap it was not built to see (session 85)

Session 84's next-step (a)(ii) — owed since session 75's (a), and re-listed unchanged by 76(c),
77(c), 78(d), 79(c), 80(c), 81(b), 82(b), 83(c) and 84(a). **Nine sessions.** Tooling + analysis
only; **nothing in `src/`, `tests/` or `analysis/captures/` touched, no constant moved, NOTHING
PROPOSED FOR SHIPPING.** ✅ **ctest 16/17 RUN** (`-j 8` per `build.md`) — the identical pre-existing
session-44 `OSValidationTest` failure, to the digit (`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`). New
`analysis/a3_harmonic_axis.py`; new report `analysis/reports/s85_a3_harmonic_axis.json`.

#### 0. ✅ BASELINE FIRST

`matrix_harmonics.py --selftest` passes all four gates unchanged (identity 0.000e+00, common-mode
recovery 0.000e+00, the model-side-guard bias demonstration 11.58 dB, the silent-capture mean/median
28.72 / 0.00 dB) before anything was written. The instrument then runs on
`analysis/reports/s74_baseline129.json` — the shipped-default 129-capture render, which session 81
item 0 established is current (`build/OfflineRender` predates it and nothing in `src/` has moved
since session 44).

#### 1. ⭐⭐ WHAT THE INSTRUMENT IS, AND WHY IT IS NOT A RE-RUN OF THE BLEND AXIS

Session 75 item 3 found that `Hn/H1` measured at the OUTPUT is **diluted** by the clean BLEND bleed —
the bleed adds fundamental and no harmonics — and that the dilution differs between the two sides
exactly when the model's OD-vs-clean balance is wrong, which is A3. That was recorded as a **defect
to be excluded** (`--bleed-free`). It is also a **measurement**, and this tool reads it as one.

With `O(c)`, `C(c)` the OD and clean contributions to the output FUNDAMENTAL and `rn` the OD path's
own `Hn/H1`, and using the fact that BLEND and LEVEL both sit AFTER every nonlinearity so `rn` is
identical along a ladder (session 59 item 6):

```
R_n(c) := 20log10(Hn/H1)_out = rn − 20log10|1 + C(c)/O(c)|
d(c)   := R_n(anchor) − R_n(c) = 20log10|1 + C(c)/O(c)|      FOR EVERY ORDER n
```

the anchor being BLEND max **and** LEVEL max, where the bleed is exactly zero (sessions 59/60). Three
properties make it worth a tool rather than a re-derivation:

1. **`rn` cancels.** `d` contains no statement about the nonlinearity at all, so a model with the
   wrong harmonic structure and the right balance reads `d_mdl = d_ped`. It is the mirror image of
   `--bleed-free`: that throws the dilution away to see the harmonics, this throws the harmonics away
   to see the dilution.
2. **The whole post-BLEND chain cancels EXACTLY** — every EQ band, the master divider, the output
   makeup **and the report's per-capture gain-match** are shared multipliers applied identically to
   `Hn` and `H1` in both the anchor and the cell. So unlike `a3_blend_axis.load_totals` and session
   23's `grunt_span_probe`, **no gain-match un-applying is needed here.**
3. ⭐⭐ **It carries NO HARMONIC-POWER BIAS, which every prior blend-axis number does.** Session 52
   item 3(b) derived that `a3_blend_axis`'s `r = √(|g1|² + H)` is an UPPER BOUND on the fundamental
   inflated by the band's harmonic power, biasing `θ` toward 90°, and that caveat has ridden on
   `r_ped` / `s_blend` ever since (51 item 7, 52 item 3b, 54 item 8). `d` is built from `H1` and `Hn`
   as **separate narrowband estimates**, so harmonic power is never summed into the fundamental.
   **This is the fundamental-only measurement that caveat asked for.**

⇒ `Δd(c) = d_ped(c) − d_mdl(c)` is A3's OD-vs-clean balance error at knob position `c`, with **no
solve, no taper fit, no `b0` estimate, no bleed model and no model of the nonlinearity.**

**Population:** grouping the 129-capture matrix on the settings that define the OD path's operating
point (`drive`, `attackIdx`, `gruntIdx`, EQ, master) and requiring a BLEND-max/LEVEL-max anchor gives
**14 usable groups / 53 diluted captures / 325 (group, cell, sweep, band) measurements**, over LEVEL
ladders (7 interior points at drive noon) and BLEND ladders (3 interior points) at three drive
settings and both ATTACK and both GRUNT throws. The two `D0.25` and one `D0.75` groups have no
bleed-free anchor and are dropped, printed.

#### 2. ✅ SIX SELFTEST GATES, ALL EXACT — AND TWO OF THEM EXIST TO STOP A VACUOUS PASS

* **GATE 1 recovery** — synthesise `Hn/H1` from a known `rn` spanning **60 dB across the orders** and
  a known dilution: `d` recovers to **0.00000 dB with spread 0.00e+00** at d = 0 / 0.5 / 3 / 12 dB.
  The wild `rn` is the point: a tool that leaked `rn` into `d` cannot pass this.
* **GATE 2 liveness** — a deliberately parity-dependent contamination must SHOW as spread and be
  separated by the parity split: 4.000 dB spread, even +6.000 vs odd +2.000. Without it the
  order-independence gate could be vacuous.
* **GATE 3 law recovery** — generate `d` at the real ladder knob positions from a chosen complex
  `ρ = clean₁/OD₁` through `level_blend_tf`, then recover it: `|err| 3.3e-16` and `6.7e-16`, rms
  `~1.5e-15 dB`. This is the only gate that exercises the superposition split and the taper.
* **GATE 4 the law DISCRIMINATES** — a wrong LEVEL taper must NOT fit as well as the right one, or a
  pedal-side residual would mean nothing: correct taper **0.00e+00** vs wrong taper **0.6309 dB**.
* **GATE 5 the anchor premise is ONE-SIGNED** — a residual bleed at the anchor must make `d` an
  UNDER-estimate on BOTH sides (session 60 item 2's bound, ≤ 0.87 dB), so it cannot manufacture a
  `Δd`. Checked numerically (6.7877 → 6.7747 dB, must fall), not argued.
* **GATE 6 balance recovery + 6b REFUSAL** — the balance fit of item 5 recovers its own `k` to
  **0.00 dB at rms 0.0000** at k = −6 / −2 / +3.5, **and refuses a constant-Δ shape at rms 3.53 dB.**
  Both halves are required: without recovery a large residual is unreadable (session 77 item 1);
  without refusal the fit would "explain" anything and `k` would mean nothing.

#### 3. ⚠⚠ FOUR DEFECTS IN MY OWN FIRST RUN, ALL CAUGHT BY READING ITS OUTPUT AGAINST ITSELF

**(a) The OD-path-out-of-circuit cells were not excluded.** `matrix_harmonics.no_od_path_rows` has
dropped BLEND = 0 and LEVEL = 0 since session 75 item 2 and I did not reuse it. `d` is **infinite by
construction** there, and the model duly returned **+141.27 / +146.42 / +149.80 / +163.53 / +363.30 dB**.
They dragged one group's median to **−110.20 dB** and the pooled MEAN to **−11.05** against a median
of −4.30. Excluded on the CONDITION, decidable before any number is read.

**(b) A cell with one surviving order printed `spread 0.00`, which READS AS A PERFECT PASS on the
order-independence gate while carrying no check at all.** Several rows did. That is session 80 item
(4a)'s construction — a control that cannot fail — one tool over. Cells now need **≥ 3 measurable
orders** to vote, and the 170 that do not are printed.

**(c) The law fit's search SENTINEL was printed as a residual.** `cost` returns 1e9 for an
unrepresentable point, so a failed fit came back as `√1e9 = 31622.777 dB` and the first run reported
that as the p90 **and** max "law residual". A refusal dressed as a measurement — session 40 item 2's
`need = +24.00` trap. Infeasibility is now returned as infeasibility and COUNTED (currently 0 / 0).

**(d) ⭐⭐ AND THE WORST ONE WAS A NARRATED VERDICT THAT CONTRADICTED MY OWN STATISTIC'S ALGEBRA — see
item 5.** The tool printed *"a swing comparable to the effect means the effect is the artefact"* above
exactly the shape that a constant balance error is REQUIRED to produce. Fourth-plus occurrence of
`computed-verdicts-not-narrated`, this time in the sentence that would have thrown the finding away.

#### 4. ⭐ THE PREMISE FAILS ON THE PEDAL, AND THE REASON MATTERS: IT IS INCOHERENT NOISE, NOT A STRUCTURED CONTAMINANT

`d` is order-independent BY CONSTRUCTION, so the order-to-order spread within a cell is the
instrument's own validity test, needing no external reference. Measured:

| | median | p90 | max |
|---|---|---|---|
| **model** order-to-order spread | **0.00** | 0.05 | 0.06 dB |
| **pedal** order-to-order spread | **4.24** | 13.33 | 44.63 dB |

⭐ The model's spread is at the numerical floor, so this is a property of the **captures**, not of the
extraction. But it is **not a structured contamination**, and that is what makes the instrument
usable: both structured splits come back at zero.

* **parity** (even vs odd): median **+0.11 dB**, 10–90 −2.68 … +3.76.
* **order magnitude** — ⭐⭐ THE FLOORING DISCRIMINATOR, and it had to be run because the artefact's
  direction is the SAME as the finding's. A pedal harmonic near the capture's own noise reads too
  HIGH ⇒ `Hn/H1` at the cell too high ⇒ that order's `d` too LOW ⇒ **Δd too NEGATIVE**, and every
  group's Δd below is negative. The high orders sit 20–40 dB below the low ones so they floor FIRST:
  measured, `d(H2,H3) − d(H6,H7)` is **+0.23 dB** (10–90 −2.52 … +5.03, n = 251) against a MODEL
  control of **−0.00**, and the Spearman ρ of `d(order)` against that order's own anchor level is
  **+0.200**. Weakly positive but an order of magnitude too small to carry a −6.5 dB result.
  ⭐ It is an ORDER-based split, not a value-based one, which is why it is the right test: guarding on
  the pedal's own `Hn/H1` at the cell would select away exactly the large-dilution cells under test
  (`self-selecting-scores`), whereas the order index is fixed in advance.

⇒ the per-order scatter is **noise**, the median over ≥ 3 orders is the estimator, and the
instrument's per-cell precision is **~1 dB, not 0.1 dB**. `coherence-not-share-identifies-a-feature`,
applied to a spread instead of a share: **a large spread with zero structure in every axis that could
bias it is a precision statement, not a validity failure.**

⚠ **GATE B monotonicity reads CHECK: 2 of 8 ladders violate** (was 9 of 18 before item 3's
exclusions). Not chased; with per-cell precision ~1 dB against a 0.288 dB gate, a 2-in-8 violation
rate is what that precision predicts.

#### 5. ⭐⭐ THE RESULT — ONE CONSTANT BALANCE ERROR, `k = −6.50 dB`, OVER-DETERMINED ACROSS 43 dB OF DILUTION

Pooled `Δd` is **−4.22 dB** (n = 110 after the spread gate; **−4.26 dB on H2/H3 only**, so the
flooring-robust subset agrees to 0.04 dB). Every one of the 14 groups and all three bands are
NEGATIVE. But the pooled number **trends with the dilution depth**, binned on the MODEL's own exact
`d` so the binning coordinate carries none of the pedal's noise:

| model `d` bin | n | Δd (all orders) | Δd (H2/H3) | **predicted at k = −6.50** | resid |
|---|---|---|---|---|---|
| 0–5 dB | 10 | −1.61 | −1.73 | **−1.71** | +0.10 |
| 5–10 dB | 33 | −3.06 | −2.85 | **−3.13** | +0.08 |
| 10–20 dB | 34 | −5.07 | −5.43 | **−4.80** | −0.27 |
| 20–∞ dB | 33 | −5.50 | −5.38 | **−6.06** | +0.56 |

⭐⭐ **That trend is the SIGNATURE of a constant balance error, not evidence of an artefact — and this
is the item my own tool got backwards first.** `d` is a log-modulus, so a *constant multiplicative*
balance error `k` gives

```
small d :  d ≈ 8.686·|C/O|   ⇒  Δd ≈ (k_lin − 1)·d_mdl     PROPORTIONAL to d
large d :  d → 20log10|C/O|  ⇒  Δd → k_db                  SATURATES at k
```

so Δd **must** grow with dilution and then flatten. Fitting ONE `k` across all 110 cells spanning
`d_mdl` **2.2 … 43.0 dB** gives **k = −6.50 dB at rms 1.95 dB**, with the four bins tracked to
**+0.10 / +0.08 / −0.27 / +0.56 dB** — one parameter, four bins, 110 cells, and GATE 6b confirms the
family REFUSES a constant-Δ shape (rms 3.53 dB), so the fit is not vacuous.

⇒ **`k = −6.50 dB` means the pedal dilutes LESS than the model: the model's OD sits too LOW relative
to its own clean bleed. That is A3's known direction** (session 20: *"the plugin's OD is too weak vs
the clean bleed in the mids"*; session 46 item 3 measured 11–14 dB too weak at 254–640 Hz), now
measured on an axis that shares **no machinery** with any prior A3 instrument.

⚠⚠ **DO NOT QUOTE −4.22 dB.** The pooled Δd is a mixture over dilution depths and **under-reads |k|
by construction**; the tool prints the warning itself.

**A3's curve on this axis** (per band, fitted `k`, with the pooled Δd beside it so the two cannot be
confused):

| band | n | **k** | rms | pooled Δd |
|---|---|---|---|---|
| 100 Hz | 24 | **−10.64** | 2.11 | −4.57 |
| 200 Hz | 42 | **−6.14** | 1.57 | −3.99 |
| 400 Hz | 44 | **−4.99** | 2.49 | −4.93 |

⚠ **Compare to `a3_shape_gate`'s drive-axis `20log10 s` (session 47: +4.43 @101 / +5.20 @202 /
+7.60 @403 dB — the scale the model's OD needs) and state the disagreement rather than reconciling
it.** `−k` = +10.64 / +6.14 / +4.99. **The SIGN agrees at every band and the order of magnitude
agrees (5–11 dB vs 4–8 dB), and at 200 Hz the two land 0.9 dB apart** — on instruments sharing no
anchor, no solve, no stimulus segment and no bleed model. ⛔ **But the SHAPE disagrees**: this axis
FALLS with frequency, the drive axis RISES. Two caveats cut both ways — the drive solve **rails at
202 and 254 Hz** (session 51 item 6), so its mid values are known uninformative; and our 100 Hz row
has the fewest cells (n = 24) and its 6 dB excess over the drive axis is the largest single
discrepancy in the comparison. **Neither shape is settled by this.**

#### 6. ⭐⭐ AND IT INDEPENDENTLY REPRODUCES THE ATTACK GAP, WHICH IT WAS NOT BUILT TO SEE

Sessions 56/57/60 established that the pedal's ATTACK **boost** throw puts a broadband **≈ +8.6 dB**
into the OD path while the modelled ladder is magnitude-INERT (≤ 0.08 dB, session 56 item 2), that
**cut** is ≈ **−2.4 dB**, and that the boost gap is **level-dependent while cut is level-invariant**
(session 57 item 3). A hotter pedal OD dilutes LESS, so that gap must appear here as extra NEGATIVE
`k` on boost, extra POSITIVE `k` on cut, and the boost excess must SHRINK with drive as the clipper
compresses it. None of those predictions was used in building this tool. Measured, `k` per group:

| drive | default | atk BOOST | excess | atk CUT | excess |
|---|---|---|---|---|---|
| D0.00 | — | −14.94 | — | — | — |
| D0.50 | −7.06 | −10.14 | **−3.08** | −4.33 | **+2.73** |
| D1.00 | −5.13 | −7.22 | **−2.08** | — | — |

⭐ **Both signs are right and the boost excess shrinks with drive**, exactly as predicted. ⚠ The
SIZES are compressed relative to session 60's bleed-free **drive-min** +8.6 / −2.4 dB — expected,
since these are drive noon and max where the clipper compresses (session 57 item 3), and **drive min
is precisely where this instrument is thin**: weak harmonics put most of its cells under the
reference floor, so `D0.00 grn1` retains only one usable cell and the drive-min excess cannot be
formed. **Read this as corroboration of DIRECTION and drive-dependence, not of size.**

#### 7. ✅ GATE C — THE MODEL'S OWN MIXING LAW, A KNOWN-ANSWER CHECK ON THE WHOLE EXTRACTION

The model's law is exactly `level_blend_tf`, so if `d_mdl` is being read correctly it must be
reproducible by ONE complex `ρ = clean₁/OD₁` per (group, sweep, band) — two real unknowns against
≥ 3 ladder points, so every fit leaves spare equations that TEST the law instead of assuming it.
**69 fits, 0 infeasible: MODEL residual median 0.003 / p90 0.016 / max 0.029 dB** — three orders
under the 0.288 dB propagated floor. The extraction is sound.

⚠ **The PEDAL residual (median 0.848 / p90 1.775 / max 4.390 dB) is NOT the same kind of number and
must not be read as a balance statement.** It is fitted with the **MODEL's** tapers, and the pedal's
are measurably different — BLEND effective 0.212 / 0.482 / 0.739 at knob 0.25 / 0.50 / 0.75 (session
51 item 4) and LEVEL exponent ~1.90 against the shipped 2.25 (session 54 item 7). A non-zero pedal
residual is therefore **expected and is a taper statement.**

#### 8. ⚠ NOT CLAIMED

* **NOTHING PROPOSED FOR SHIPPING.** `k = −6.50 dB` is a MEASUREMENT of A3's size on a new axis, not
  a candidate: it does not name a carrier, and session 50 item 1 already established that A3 will
  not close on one element (C1 + C2 + C3).
* `d` is indexed by **KNOB POSITION**, so a per-cell Δd mixes **taper conformity** with the balance.
  The two-ladder split is the discriminator: **LEVEL ladder −2.82, BLEND ladder −5.31 dB** pooled.
  Those two ladders have independently-measured and DIFFERENT taper errors, so their disagreement
  bounds the taper contribution at roughly 2.5 dB — which is why `k` is quoted from the pooled fit
  and the per-ladder numbers are printed beside it rather than averaged away.
* **The captures ARE authoritative here.** Unlike every even-order statistic in sessions 71–84, A3 is
  a LINEAR-path / mixing quantity, and per `reference-sources.md` §1 ND tracks hardware to ≤ 1.4 dB
  on the linear path. So this is one of the few places the 129-capture matrix has full authority.
* The anchor's zero-bleed premise is **topological**, bounded at ≤ 0.87 dB (session 60 item 2) and
  **one-signed** (GATE 5), so it cannot manufacture a Δd.
* Per-cell precision is ~1 dB (item 4), and `mix_ratio` reduces `|1 + C/O|` to `1 + |C/O|`, i.e. it
  assumes a small phase on C/O. The fit is scored on `d` itself, so a bad reduction shows up as the
  1.95 dB residual rather than as a silently wrong `k` — but `k` should be quoted as **≈ −6.5 dB**,
  not to two decimals.
* **`jfetGm` still not swept** (the session-4 anchor every nonlinear fit rests on).

#### 9. ▶ NEXT, IN ORDER

**(a)** ⭐⭐ **A3 now has a size, a sign and a curve on an axis with no bleed model — so the live
question is the CARRIER, and item 5's shape disagreement with `a3_shape_gate` is what to settle
first.** Both instruments say the model's OD is 5–11 dB too weak vs its bleed; they disagree on
whether the deficit falls or rises with frequency, and the drive axis is known to rail at two of the
three bands being compared. Re-run `a3_shape_gate` restricted to bands where the drive solve is
interior (session 52's next-step (a), still open) and compare against this tool's per-band `k`.
**(b)** ⭐ this instrument can be pointed at any candidate render with no new machinery — it reads
`comprehensive_report` JSONs only — so any A3 candidate should be scored on `k` per band alongside
the matrix. **(c)** ⛔ **DROP "the harmonic-axis A3 instrument" from the queue — it is built.**
**(d)** the even-order item is unchanged and still needs the WEIGHTING judgement session 84 item
(a)(i) named, which no instrument here can supply. **(e)** session 70's §2 rejection under session 71
(4b). **(f)** `c21R` toward 130–150k. **(g)** the A3 / GAP #3b low-mid and ATTACK-notch depth items —
note item 6 gives the ATTACK gap a third independent measurement. **(h)** everything session 70
listed behind that.

---

### ⭐⭐ A3's TWO INSTRUMENTS DESCRIBE ONE CURVE — session 85's shape disagreement does NOT survive the intervals, and the "restrict CORE to interior bands" item is CLOSED as unnecessary because θ=0 is a SYMMETRY POINT, not a fence (session 86)

Session 85's next-step (a): *"Re-run `a3_shape_gate` restricted to bands where the drive solve is
interior (session 52's next-step (a), still open) and compare against this tool's per-band `k`."*
Both halves are done. **Tooling + analysis only; NOTHING in `src/`, `tests/` or the captures
touched, no constant moved, NOTHING PROPOSED FOR SHIPPING.** ✅ **ctest 16/17 RUN** (`-j 8` per
`build.md`) — the identical pre-existing session-44 `OSValidationTest`, to the digit
(`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`). New `analysis/a3_axis_compare.py`.

#### 0. ✅ BASELINE FIRST, BOTH INSTRUMENTS, BEFORE EITHER WAS READ

`a3_shape_gate --selfcheck` reproduces the recorded baseline at **worst 0.027 dB / score 5.808**
(β = −16.80 dB), and the harmonic axis's per-band `k` recomputed from `s85_a3_harmonic_axis.json`'s
own rows — never transcribed — reproduces **−10.64 / −6.14 / −4.99 and pooled −6.50 to 0.005 dB**
(n = 110). GATE 0 in the new tool refuses to print a comparison if either side moves, because a
moved instrument and a moved result are indistinguishable otherwise.

#### 1. ⭐⭐ THE COMPARISON IS FREQUENCY-ALIGNED BY ALGEBRA, NOT BY ASSUMPTION

`d` looks like a harmonic statistic, and the obvious worry is that it is being compared against a
band of the swept response at a different frequency. **It is not — the harmonics cancel out of `d`
exactly.** Only the OD path carries harmonics, so at a diluted cell

    Hn_out = O·hn                  H1_out = O·g1 + C
    d(c) = R_n(anchor) − R_n(c) = 20log10|1 + C/(O·g1)|

`hn` cancels ⇒ **`d` is a property of the OD-vs-clean balance AT THE FUNDAMENTAL**, i.e. at
100 / 200 / 400 Hz, which is exactly what the shape gate's 101 / 202 / 403 Hz bands measure. The
ORDERS enter only as repeated measurements of one number — which is what makes item 3's test real.

#### 2. ⚠⚠ THE PREMISE FOR RESTRICTING `CORE` DOES NOT SURVIVE — θ = 0 IS A SYMMETRY POINT

Session 51 item 6 found the drive solve at θ = 0.0 at 202 and 254 Hz, called it *"sitting ON that
boundary"*, and concluded the shape gate's SCORE is partly set by a solve at its own bound. Sessions
52 and 79–85 carried *"restrict CORE to bands where the drive solve is interior"* as an open item.
**It is the wrong reading.** θ is a FOLDED parameter — conjugating θ leaves every `|·|` unchanged, so
only `|θ|` is identifiable and the cost is EXACTLY symmetric about both endpoints. GATE 1
demonstrates it numerically rather than arguing it:

| band | `|cost(+ε) − cost(−ε)|` at θ=0 | at θ=180 |
|---|---|---|
| 202 Hz | **0.000e+00** | 0.000e+00 |
| 254 Hz | **0.000e+00** | 3.411e-13 |
| 40 Hz (control) | 0.000e+00 | 1.599e-14 |

⇒ θ = 0 and θ = 180 are **stationary by symmetry**, so an optimum resting there is **not** the
`bound-resting-means-unidentified` case and carries none of that implication. What IS true near
θ = 0 is that `dcos θ/dθ = 0`, so θ is weakly determined — but ⭐ **the SCORED quantity is `s`, and
`s` is correspondingly INSENSITIVE to θ exactly where θ is loose**; the two uncertainties
anti-correlate rather than compound. Measured: re-solving `s` with θ pinned to the model's own phase
instead of free moves `s` by **−0.00 dB at both 202 and 254 Hz**.

⭐ **AND THE MEASURED RESTRICTION SELECTS DIFFERENT BANDS — the opposite ones.** Read the +0.25 dB
joint `(s, θ)` interval, in dB of `s`, across the CORE set: 202 and 254 Hz are **2.67 and 3.12 dB**
wide, while **403 and 508 Hz are 3.92 and 5.79 dB** — the two widest in CORE. So the bands session 51
wanted to drop are better identified than the two it kept, **and 403/508 are precisely the bands
carrying the shape gate's "the deficit RISES with frequency" claim.**

| restriction | bands | score |
|---|---|---|
| CORE as shipped | 14 | **5.808** |
| ex θ-at-a-symmetry-point (202, 254) — session 51's proposal | 12 | 5.939 |
| ex `s`-interval > 2.5 dB — the measured one (drops 20, 160, 202, 254, 403, 508) | 8 | 4.287 |

⇒ **session 51 item (a) / session 52 item (b) — "stop ranking A3 candidates on a score containing
railed bands" — is CLOSED as unnecessary.** Dropping the two bands moves the score 0.13 dB, in the
wrong direction, and changes no ranking. **DROP it from the queue.** ⚠ Whichever set is ever used
must still be FROZEN at the shipped baseline and never re-derived per candidate
(`self-selecting-scores`); `CORE` itself is unchanged so every recorded number stays comparable.

#### 3. ⭐⭐ THE SHAPE DISAGREEMENT REDUCES TO ONE BAND — AND THEN DOES NOT SURVIVE AT ALL

Both columns are the same quantity in the same units at the same frequency: *dB by which the model's
OD sits below where it should, relative to its own clean bleed*. The harmonic axis's interval is a
**group-level bootstrap** (the 11–25 `(group, sweep)` clusters resampled, **not** the cells — cells
within one ladder share an anchor, an operating point and a taper, so resampling cells would report
an interval several times too tight).

| | shape gate `20log10 s` | interval | `\|k\|` harmonic | interval | verdict |
|---|---|---|---|---|---|
| 100 Hz | +4.43 | [+3.46, +5.29] | 10.64 | [+6.49, +14.79] | **DISJOINT by 1.19 dB** |
| 200 Hz | +5.20 | [+4.33, +7.00] | 6.14 | [+4.61, +8.26] | OVERLAP |
| 400 Hz | +7.60 | [+5.21, +9.12] | 4.99 | [+4.11, +7.51] | OVERLAP |

So **2 of 3 bands already agree** and the "shape disagreement" is a single-band disagreement worth
1.19 dB — stable across five bootstrap seeds (1.02–1.21 dB), so not a resampling artefact.

⭐⭐ **AND 100 Hz IS THE ONE BAND THAT FAILS THE HARMONIC AXIS'S OWN PREMISE.** Per item 1 the orders
are repeated measurements of ONE number, so `k` fitted on H2/H3 alone must equal `k` fitted on H6/H7
alone. GATE 2 tests exactly that, and it is over-determined (two disjoint order groups, one predicted
number):

| | k(all) | k(H2,H3) | k(H6,H7) | split |
|---|---|---|---|---|
| 100 Hz | −10.64 | **−7.96** | −11.18 | **3.23 dB** |
| 200 Hz | −6.14 | −5.61 | −6.37 | 0.76 dB |
| 400 Hz | −4.99 | −5.59 | −6.52 | 0.93 dB |

**3.8× the median split elsewhere**, at the band that is also the thinnest (n = 24 over 11 clusters
vs 42/44 over 20/25) and has the least **dilution leverage** — `predict_from_balance` only saturates
at `k` once the dilution is deep, and 100 Hz spans only **2.2 … 24.4 dB** against 400 Hz's 4.7 … 43.0.

⭐⭐ **RE-RUN ON THE ROBUST ORDER SUBSET, ALL THREE BANDS OVERLAP:**

| | shape gate | interval | `\|k\|` (H2,H3) | interval | verdict |
|---|---|---|---|---|---|
| 100 Hz | +4.43 | [+3.46, +5.29] | 7.96 | [+4.50, +14.10] | **OVERLAP** |
| 200 Hz | +5.20 | [+4.33, +7.00] | 5.61 | [+5.04, +7.97] | **OVERLAP** |
| 400 Hz | +7.60 | [+5.21, +9.12] | 5.59 | [+4.71, +7.68] | **OVERLAP** |

⭐ **That subset is NOT chosen to get this answer** (`self-selecting-scores`): `LOW_ORDERS = H2, H3`
is designated the robust subset by `a3_harmonic_axis` itself, for a stated physical reason (they are
the last orders to reach the capture's own noise), and it was named before this comparison existed.

⇒ **SESSION 85 ITEM 5's "the SHAPE disagrees" SHOULD NOT BE CARRIED FORWARD. The two instruments —
sharing no anchor, no solve, no stimulus segment, no bleed model and no capture population — describe
ONE curve.**

#### 4. ⚠ WHAT IS NOT CLAIMED

* **This does NOT license calling the curve flat, or fitting a slope to either column.** The
  intervals are wide (shape gate 1.8–3.9 dB across these bands, harmonic axis 3.2–9.9 dB); what is
  established is **compatibility**, not a shape. Quote A3 as **≈ 5–7 dB over 100–400 Hz** and treat
  both the shape gate's rise and the harmonic axis's fall as inside the noise.
* The two intervals are **different constructions** — a +0.25 dB cost-slack region against a 5–95
  group bootstrap — so OVERLAP/DISJOINT is a legibility verdict, not a formal test. The 1.19 dB gap
  at 100 Hz on all orders is small enough that this matters, which is a second reason not to rest
  anything on that band.
* **Session 85's k = −10.64 dB at 100 Hz should be quoted as ≈ −8 dB, or not quoted.** Its all-order
  value is carried by the H6/H7 half at the band with the least dilution leverage.
* Nothing proposed, no carrier named. `jfetGm` still not swept.
* The pooled `k ≈ −6.5 dB` is untouched by all of this — the pooled fit is dominated by the 200 and
  400 Hz rows (86 of 110 cells), both of which agree with the drive axis on all orders.

#### 5. ▶ NEXT, IN ORDER

**(a)** ⛔ **DROP two items from the queue: "restrict `a3_shape_gate`'s CORE to interior bands"
(item 2) and "settle the shape disagreement" (item 3).** Both are answered, both negatively.
**(b)** ⭐⭐ A3 now has a size (**≈ 5–7 dB over 100–400 Hz**), a sign, and **two mutually-corroborating
instruments**, so the live question is the **CARRIER** and nothing in the measurement queue blocks it
any more. Session 50 item 1 established A3 will not close on one element (C1 flat floor + C2 low-mid
+ C3 LF), and session 52 established no causal post-clipper LINEAR element can supply it — so the
next move is to re-derive C1/C2/C3 against the corroborated curve rather than against the drive axis
alone. **(c)** `analysis/a3_axis_compare.py --fit KEY=VALUE` scores a candidate on BOTH axes in one
run, so any A3 candidate should be gated on it alongside the 129-capture matrix. **(d)** the
even-order item is unchanged and still needs the WEIGHTING judgement session 84 item (a)(i) named,
which no instrument here can supply. **(e)** session 70's §2 rejection under session 71 (4b).
**(f)** `c21R` toward 130–150k. **(g)** the A3 / GAP #3b low-mid and ATTACK-notch depth items.
**(h)** everything session 70 listed behind that.

### ⭐⭐ SESSION 86's NEXT-STEP (b) — THE BUDGET IS RE-DERIVED, AND THE CORROBORATION REACHES **C2 ONLY**. C1 and C3 are structurally unreachable by the second instrument; what the corroborated curve actually does is NARROW β — and the whole of that lands on **C3**, the component it cannot see (session 87)

Session 50 wrote A3's requirement down as three numbers, and every A3 candidate since has been aimed
at them: **C1 +2.68 dB** (broadband floor, mean of 50/64 Hz), **C2 +3.20 dB** (low-mid rise on top of
C1, mean of 101–508 Hz), **C3 +7.86 dB** (LF rise on top of C1, at 20 Hz, slope 9.18 dB/oct). All
three were read off ONE instrument as POINT values with no uncertainty, at a bleed level `β` the same
instrument fits. Session 86 then established that a SECOND instrument (`a3_harmonic_axis`: no solve,
no taper, no `b0`, no bleed model) describes the same curve, and its next-step (b) asked for the
budget to be re-derived against both. New tool **`analysis/a3_budget_rederive.py`**.

Tooling + analysis only. **NOTHING in `src/`, `tests/` or `analysis/captures/` was touched, no
constant moved, nothing proposed for shipping, and no existing tool changed.** ✅ **ctest 16/17 RUN**
(`-j 8` per `build.md`) — the identical pre-existing session-44 `OSValidationTest`, to the digit
(`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`).

#### 0. ✅ BASELINE FIRST — BOTH INSTRUMENTS *AND* THE PRIOR BUDGET

GATE 0 refuses to print anything unless three records reproduce, because a moved instrument and a
moved result are indistinguishable otherwise:

| record | reproduces |
|---|---|
| harmonic axis, per-band `k` and pooled | worst \|Δk\| **0.005 dB**, pooled **0.004** (n=110, rms 1.95) |
| shape gate, `20log10 s` vs `BASELINE_DB` | worst **0.027 dB**, β **−16.80 dB** |
| **session 50's budget, at its own β −16.75** | C1 **+2.68** / C2 **+3.20** / C3 **+7.86** / slope **9.18** → worst \|Δ\| **0.003 dB** |

⚠ **TWO β CONVENTIONS EXIST AND BOTH RECORDS ARE CORRECT** — stated with its consequence rather than
quietly reconciled, because an unexplained 0.05 dB reads as a discrepancy for the rest of the
project's life. `a3_component_budget` takes the min-cost β off its own **0.25 dB** sweep (−16.75);
`a3_shape_gate.fit_beta` scans at **0.1 dB** and lands on −16.80, which is what the axis comparison
and every session-86 number use. Consequence for the budget: **C1 +2.68 → +2.68, C2 +3.20 → +3.23,
C3 +7.86 → +7.72 dB.** Everything below is at −16.80.

#### 1. ⛔⛔ GATE 1 — THE COVERAGE FACT, AND IT IS THE HEADLINE

`a3_harmonic_axis.ANCHOR_HZ` **IS** `comprehensive_report.THD_ANCHORS` — (100, 200, 400) Hz, a
property of the STIMULUS, not a choice. Computed per component band set:

| component | bands | anchors |
|---|---|---|
| C1 floor | 50, 64 | **0 / 2 — NONE** |
| C2 low-mid | 101, 127, 160, 202, 254, 403, 508 | **3 / 7** (100, 200, 400 Hz) |
| C3 LF | 20, 25, 32 | **0 / 3 — NONE** |

⇒ **the corroboration reaches C2 and nothing else. C1 and C3 remain single-instrument numbers no
matter how well the two axes agree at 100–400 Hz**, and an anchor count of zero is the ABSENCE of a
measurement, not a weak one. The tool refuses to describe a component as corroborated when its count
is zero.

#### 2. ⚠⚠ GATE 2 — β DOES NOT SHIFT THE CURVE, IT **TILTS** IT, AND THAT WAS NOT THE EXPECTATION

`ds/dβ` by least squares over the 23 β points inside the identified interval:

| Hz | 20 | 50 | 64 | 101 | 127 | 160 | 202 | 254 | 403 | 508 |
|---|---|---|---|---|---|---|---|---|---|---|
| dB of `s` per dB of β | **+3.32** | **+0.09** | −0.22 | −0.54 | −0.58 | −0.53 | −1.40 | −1.37 | +0.41 | +0.90 |

**POSITIVE at 20/50/403/508, NEGATIVE at 64–254.** The naive expectation — β is a flat level knob, so
raising it lowers every band's `s` by the same amount — is **wrong**: `s` is solved from a two-phasor
cancellation, so a band near anti-phase (the LF end, θ → 180°) moves the OPPOSITE way to one near
quadrature. ⭐ **And `|ds/dβ|` is smallest at 50 Hz (0.09) — which is now the MECHANISM behind session
50's "β can account for at most 0.26 dB of the floor", where before it was only an empirical span.**
⇒ any argument of the form *"β is flat, so a flat component is degenerate with it"* must be checked
per band against this table, never asserted.

✅ Session 50's own β-robustness spans reproduce exactly on its own grid: 20 Hz **2.51**, 50 Hz
**0.07**, 254 Hz **1.10**, 508 Hz **0.68**.

⚠ **AND THE IDENTIFIED INTERVAL IS WIDER HERE — THE CAUSE IS THE GRID, VERIFIED NOT ASSUMED.** This
tool sweeps at 0.05 dB and reports **[−17.40, −16.30]** against session 50's **[−17.25, −16.50]**.
Restricted to session 50's own 0.25 dB sub-grid it reproduces **exactly**, and the tool **EXITS** if
it does not. ⭐ That check earned its keep immediately: a first version swept at 0.1 dB from −18.00,
which *looks* like it contains the 0.25 grid and does not (it never lands on −17.25) — the gate fired,
correctly, because at that point the two intervals genuinely were measured over different β sets.
**A reproduction check between two grids has to be run on the coarser grid's own points.**

#### 3. THE CURVE AS INTERVALS, AND WHAT IS RESOLVED AGAINST WHAT

A component is a DIFFERENCE of two solved quantities that share a β, so an independent "±" on each is
the wrong object — the errors are correlated and differencing two independent ranges overstates the
width. What IS well defined is each band's own +0.25 dB joint (s, θ) region and the **GAP** between
two regions. Floor band [50, 64] combined region = **[+2.12, +3.25] dB**.

* **C1 is real and no bleed level removes it** — the floor's own lower bound excludes zero by
  **2.12 dB**, and GATE 2's 50 Hz span of 0.10 dB reproduces session 50's ≤0.26 dB figure.
* **C3: 3/3 bands resolved above the floor** (gaps 4.67 / 2.50 / 0.79 dB at 20 / 25 / 32).
* **C2: 7/7 bands resolved above the floor** (gaps 0.21 … 1.96 dB).
* ⭐ **40 Hz OVERLAPS the floor**, and 80 Hz does too ⇒ **C3's onset sits between 40 and 32 Hz, so
  session 50's C3 band set [20, 25, 32] is the right one and 40 Hz belongs to the floor, not to C3.**
  That was an unstated choice in session 50; it is now measured.

⭐ **C2's SIZE IS CARRIED BY ITS WORST-CONDITIONED BANDS.** Restricted to the bands whose s-region is
≤2.5 dB wide — **[101, 127]** — C2 = **+2.06 dB** against **+3.23 dB** over all seven. The dropped
bands are 160 (3.04), 202 (2.67), 254 (3.12), 403 (3.92) and 508 (**5.79**, the widest in CORE). ⇒
**the "deficit RISES with frequency" half of C2 rests on 403 and 508 Hz, and 508 has no harmonic
anchor at all.** ⚠ That set is FROZEN at the shipped baseline and must never be re-derived per
candidate (`self-selecting-scores`); it is printed BESIDE the full set, never instead of it.

#### 4. ⭐⭐ WHAT THE SECOND INSTRUMENT ADDS — TWO THINGS, AND THE SECOND IS THE INTERESTING ONE

**(a) C2 is corroborated in EXISTENCE.** The harmonic axis's robust-subset LOWER bound at 200 Hz is
**+5.04 dB**; the drive axis's floor region tops out at **+3.25 dB**. Margin **+1.79 dB** ⇒ a tool
with no β in it says A3 at 200 Hz exceeds the floor, so the low-mid rise is **not** an artefact of the
drive axis's own bleed fit. ⚠ **HALF-INDEPENDENT ONLY**: C2 = `s(low-mid) − s(floor)` and the harmonic
axis has no floor band (GATE 1), so this pairs one instrument's numerator with the other's
denominator. It is not two independent measurements of C2.

**(b) ⭐⭐ AND IT NARROWS β — READ OFF THE SWEPT CURVE, NOT LINEARISED.** Which β values put `s(band)`
inside that band's own bootstrap interval:

| band | `s` spans | target | admits β |
|---|---|---|---|
| 100 Hz | +4.01 … +5.11 | [+4.50, +14.10] | [−18.00, **−16.95**] |
| 200 Hz | +4.68 … +7.15 | [+5.04, +7.97] | [−18.00, −16.70] |
| 400 Hz | +6.99 … +7.88 | [+4.71, +7.68] | [−18.00, −16.65] |

**joint [−18.00, −16.95]** (⚠ its lower end rests ON the sweep edge, so the harmonic axis does not
bound β from that side at all — read that width as a lower bound, `bound-resting-means-unidentified`)
against the **drive axis's [−17.40, −16.30]**, giving **BOTH: [−17.40, −16.95], width 0.45 vs 1.10.**
⭐ **The two axes bound OPPOSITE sides** — the drive axis sets the lower end, the harmonic axis the
upper — so on β they are complementary, not redundant.

⭐⭐ **AND THE DRIVE AXIS'S OWN OPTIMUM (β = −16.80) IS *OUTSIDE* THAT RANGE, by 0.15 dB.** So the
curve every A3 candidate has been RANKED on since session 47 is solved at a β the second instrument
excludes. That does not invalidate the ranking (a common β shift is mostly a level offset on the
score), but **any ABSOLUTE component size taken off that curve inherits it** — and C3 above all.

#### 5. ⭐⭐ THE RESULT: THE CORROBORATION'S WHOLE EFFECT ON THE BUDGET LANDS ON **C3**

Read off the sweep at the endpoints of the jointly-admitted β range:

| β | fit rms | C1 | C2 | C3 |
|---|---|---|---|---|
| −16.80 (fitted) | 0.0927 | +2.68 | +3.23 | **+7.72** |
| −16.95 | 0.0951 | +2.69 | +3.30 | **+7.23** |
| −17.40 | 0.1424 | +2.72 | +3.50 | **+4.99** |

**C1 moves 0.03 dB and C2 0.20 dB across that range; C3 moves 2.24 dB.** The corroborated curve
constrains β, β tilts the LF end (§2), and so **the second instrument's real effect on the budget
falls on the one component GATE 1 says it cannot measure.** ⇒ **NEVER quote a C3 size without the β it
was read at.**

**THE RE-DERIVED BUDGET, against the β range both axes admit:**

| | session 50 quoted | re-derived |
|---|---|---|
| **C1** | +2.68 dB | **+2.69 … +2.72 dB** |
| **C2** | +3.20 dB | **+3.30 … +3.50 dB** (and +2.06 on its well-conditioned bands alone) |
| **C3** | +7.86 dB | **+4.99 … +7.23 dB** ⬅ **MOVES, by up to 2.9 dB** |

✅ The **ORDERING C3 > C2 > C1 holds at every admitted β** — checked, because the C3−C2 margin narrows
from +4.49 dB at the fitted β to **+1.49 dB** at the bottom of the range.

#### 6. ⚠ WHAT IS NOT CLAIMED

* **Nothing is proposed, no candidate screened, no constant moved.** Session 50 item 1 (A3 will not
  close on one element) and session 52 (no causal post-clipper LINEAR element can supply it) both
  stand; this re-states the requirement they are measured against, with the corroboration's reach made
  explicit.
* ⚠ **The two instruments' intervals are DIFFERENT CONSTRUCTIONS** — a +0.25 dB cost-slack region vs a
  5–95 group bootstrap — so every OVERLAP / DISJOINT / margin verdict here is a **legibility verdict**,
  exactly as session 86 recorded, never a formal test.
* ⚠ **C3's SIZE IS STILL OPEN, AND THE TWO AVAILABLE READINGS DISAGREE IN DIRECTION.** The corroborated
  curve pushes C3 **DOWN** (to +5.0 … +7.2 dB); session 51 item 7's `r_ped` reading pushed it **UP**
  ("C3 is the DOMINANT A3 term, not a ~+8 dB tail"). Both come from below 40 Hz, where session 52
  item 1 measured the blend axis to be unreliable (up to 2.774 dB of self-validation error at 20 Hz,
  θ railed at 180° at 32 Hz). **Do not treat either as settled.**
* ⚠ **No instrument reaches A3 below 100 Hz except the drive axis**, whose own conditioning is worst
  there (the 20 Hz s-region is 3.88 dB wide, second-widest in CORE). That is a STIMULUS limitation, not
  an analysis one — the harmonic axis is pinned to `THD_ANCHORS`.
* Model vs **ND** throughout — but per `reference-sources.md` §1 A3 is a linear-path/mixing quantity
  where ND tracks hardware to ≤1.4 dB, so the captures keep full authority here. `jfetGm` still not
  swept.

#### 7. ⚠⚠ FOUR DEFECTS IN MY OWN GATES, ALL CAUGHT BY READING THE TOOL'S OUTPUT AGAINST ITSELF

Recorded because three of them printed a **plausible verdict over no data**, which is the failure mode
this project keeps re-encountering:

* **(a) `ds/dβ` came back EMPTY and nothing failed.** It took a central difference at `fitted ± step`
  where `step` came from `betas[1] − betas[0]`, but the sweep carried two off-grid points (the two β
  conventions) so neither neighbour existed in it. The table printed **zero rows**; section C's loop
  then produced no items, its `lows and highs` test was False on empty lists, and it **narrated "ALL
  THREE BANDS WANT β HIGHER" over nothing** while the interval intersection **printed its own ±99
  initialiser as a value** (`computed-verdicts-not-narrated`, plus session 40's sentinel-as-a-value
  trap). Now a least-squares slope over the β points inside the identified interval — the right
  estimator anyway — and it **EXITS** rather than printing a partial table.
* **(b) ⚠⚠ A SIGN ERROR THAT INVERTED THE VERDICT.** It solved `β + (s − k)/m` where setting
  `s(β₀) + m·(β − β₀) = k` gives `β₀ + (k − s)/m`. Every band actually wants β **LOWER**; it printed
  **HIGHER**. Same class as session 33's lost sign and session 79's wrong-sign correction — and it was
  in a **verdict line**, not a table.
* **(c) EVEN WITH THE SIGN RIGHT, THE EXTRAPOLATION WAS INVALID.** Closing a 3.5 dB gap at
  `|ds/dβ| = 0.54` needs a 6.5 dB move in β, i.e. the linearisation pushed **~9× past the window it
  was measured in** — and the sweep itself falsifies it (`s(101)` never reaches 7.96 at ANY swept β;
  its maximum over the whole range is +4.96). ⭐ **The fix is not a wider linearisation, it is not
  linearising**: ask the swept curve directly. `ds/dβ` stays in GATE 2 as the tilt statement it is and
  is not used to travel.
* **(d) The grid-reproduction gate fired on my own sweep** (§2) and was right to.

⭐ **THE GENERAL LESSON, and it is the same one three ways: a gate that produces NO DATA must FAIL, not
fall through to its else-branch.** Each of (a)'s three symptoms was a downstream consumer treating an
empty collection as a legitimate answer.

#### 8. ▶ NEXT, IN ORDER

**(a)** ⛔ **DROP "re-derive C1/C2/C3 against the corroborated curve" from the queue — it is DONE**,
and its answer is that the corroboration reaches C2 only. **(b)** ⭐⭐ **the highest-value measurement
this exposes is an anchor BELOW 100 Hz.** C3 is the largest component, the least determined, the only
one two readings disagree on in DIRECTION, and the only one no second instrument can reach — and the
reason is a stimulus property (`THD_ANCHORS`), not an analysis limit. A fourth THD anchor at ~40–50 Hz
would put C3 on the β-free axis, and captures are re-renderable on demand
(`reference-sources.md` §0), so this is a stimulus + report change, not a hardware session. ⚠ It
re-keys `comprehensive_report`'s result cache and changes every record's shape, so it is a deliberate
re-baseline like session 69's `sweep_clean_-36` item, not a drive-by edit. **(c)** the CARRIER question
is unchanged and is now aimed at a budget with intervals: **C1 ≈ +2.7 dB flat (β-robust, needs a
broadband OD-level lever), C2 ≈ +2.1 … +3.5 dB over 101–508 Hz (corroborated in existence), C3 ≈ +5 …
+7 dB at 20 Hz (open)** — gate any candidate on `a3_axis_compare.py --fit KEY=VALUE` (both axes in one
run) alongside the 129-capture matrix. **(d)** the even-order item is unchanged and still needs the
WEIGHTING judgement session 84 item (a)(i) named. **(e)** session 70's §2 rejection under session 71
(4b). **(f)** `c21R` toward 130–150k. **(g)** the A3 / GAP #3b low-mid and ATTACK-notch depth items.
**(h)** everything session 70 listed behind that.

---

### ⛔⛔ SESSION 87's NEXT-STEP (b) — THE PREMISE IS WRONG IN BOTH DIRECTIONS. A sub-100 Hz anchor needs **no new capture, no stimulus change and no re-baseline**; it does **not** re-key the cache (which makes it a silent no-op, not a deliberate cost); and the frequency it named cannot reach the component it named. 4 of the 5 target bands are readable from the captures already on disk (session 88)

**Tooling + analysis only.** NOTHING in `src/`, `tests/` or the captures touched, no constant moved,
NOTHING PROPOSED FOR SHIPPING, and `THD_ANCHORS` deliberately NOT changed (see item 7).
✅ **ctest 16/17 RUN** (`-j 8` per `build.md`) — the identical pre-existing session-44
`OSValidationTest`, to the digit (`amp 0.35: 2x −25.6 / 4x −32.1 / 8x −23.6`).
New **`analysis/lf_anchor_gate.py`**; `analysis/a3_harmonic_axis.py` gained one additive guard.

#### 0. ✅ BASELINE FIRST

`a3_harmonic_axis.py --selftest` passes all six gates unchanged (recovery 0.00000 dB / spread
0.00e+00, law recovery |err| 3.3e-16, the taper discriminator 0.00 vs 0.6309 dB, GATE 6b's refusal
at rms 3.53 dB), and the full run reproduces the session-85/86 record **exactly**: pooled
**k = −6.50 dB** (rms 1.95, n=110, d_mdl 2.2 … 43.0 dB), per band **100 Hz k −10.64 (n=24) /
200 Hz −6.14 (n=42) / 400 Hz −4.99 (n=44)**, over **325 measurements / 53 distinct diluted
captures**. Reproduced again AFTER this session's edit to that file, so the guard in item 6 is a
proven no-op on the record and not merely believed to be one.

#### 1. ⚠⚠ THE ANCHORS ARE A PROPERTY OF THE **REPORT**, NOT OF THE STIMULUS

Session 87 recorded `a3_harmonic_axis.ANCHOR_HZ` as being
`comprehensive_report.THD_ANCHORS` = (100, 200, 400) Hz — *"a property of the STIMULUS"* — and
scoped the fix as a stimulus change plus a re-capture. Read against the code that is wrong.
`comprehensive_report.harmonics_at_anchors` calls `analyze.harmonic_thd_curve`, which returns a
**continuous** Farina curve `(fr, thd, Hn)` spanning the whole sweep band, and then does

```python
idx = int(np.argmin(np.abs(fr_c - ahz)))      # THD_ANCHORS is an INDEX, not a segment
```

Measured on the stimulus that has been on disk since 2026-07-20: the curve carries **524 289 bins,
lowest 0.046 Hz, 2 184 of them BELOW the lowest shipped anchor.** The sub-100 Hz data is already in
every capture; nothing has to be re-recorded and no segment offset moves.
⭐ `check-for-unread-data-first`, **sixth occurrence** in this project (sessions 60, 78, 81, 82, 84).

#### 2. ⚠⚠ AND THE COST STATEMENT IS BACKWARDS — IT IS A LANDMINE, NOT A RE-BASELINE

Session 87 also recorded that changing the anchors *"re-keys `comprehensive_report`'s cache and
changes every record's shape ⇒ a deliberate re-baseline"*. Probed against the live function,
`_cache_key` hashes (capture path, render args, OS factor, binary, **bands**) — **the anchors are not
in it.** GATE 0(c) computes both keys and they are **IDENTICAL**, with a control confirming the key
*does* move when `bands` moves (a key that never changed would make the probe vacuous).

⇒ changing `THD_ANCHORS` and re-running **silently returns the OLD three-anchor cached records** and
prints a table that looks entirely fine. The correct instruction is not "expect a re-baseline", it is
**"pass `--no-cache` or the change does nothing"**.

#### 3. ⚠⚠ AND ~40–50 Hz CANNOT REACH C3, WHICH IS WHAT IT WAS ASKED FOR

`a3_component_budget`'s band sets are **C3 = [20, 25, 32] Hz** and **C1 = [50, 64] Hz**, and session
87's own item (6a) measured C3's onset as sitting between 40 and 32 Hz and assigned **40 Hz to the
floor**. So an anchor at "~40–50 Hz" lands on **C1 and the floor — not on C3** — whatever else is
true. The handover named 40–50 Hz and C3 in one sentence; they are different regions.

#### 4. ⭐⭐ SO THE REAL QUESTION IS VALIDITY, AND IT SPLITS INTO **TWO INDEPENDENT LIMITS**

Not "can we record it" but "is it readable". Two limits, which fail in completely different ways and
which the new tool therefore reports as **separate columns, combined explicitly**:

**(a) IS THE EXTRACTOR SOUND THERE? — GATE 1 / 1b, known-answer.** Push the real stimulus's own
`sweep_drv_-18` through a static shaper built from **Chebyshev polynomials**: for `x = A·sin θ`,
`T_n(x/A) = cos(n(θ − π/2))`, so `g(x) = Σ c_n T_n(x/A)` has `|Hn| = |c_n|` **exactly, at every
frequency**. ⭐ The truth is therefore **FLAT in frequency**, so any frequency dependence the
extractor reports *is its own error* — there is no threshold to choose. GATE 1b repeats it behind a
known 1st-order highpass, where the truth is `c_n/c_1 · |G(nf)|/|G(f)|`, exact and computed from the
**digital** filter actually applied.

**(b) DOES THE REFERENCE HAVE ANYTHING TO EXTRACT? — GATE 4.** `a3_harmonic_axis` guards on the
REFERENCE (`REF_FLOOR_DB` = −60 dB; session 74 item 6 — a floor guard belongs on the reference, never
on the quantity under test) and needs `MIN_ORDERS` = 3 orders to form a cell at all. So the yield is a
property of the ND device's LF harmonic content, measured here from **81 OD captures × 3 driven
sweeps, pedal side only — no render, no model, no anchor change.**

#### 5. ⭐⭐ THE RESULT — 4 OF THE 5 TARGET BANDS ARE READABLE ON THE CAPTURES WE ALREADY HAVE

| band | extractor err (flat / sloping) | ref yield | component | **verdict** |
|---|---|---|---|---|
| **20.00** | **5.93 / 5.33 dB** | 59.3 % | C3 | ⛔ **UNREADABLE** |
| 25.00 | 0.10 / 0.58 | 56.8 % | C3 | READABLE |
| 32.00 | 0.02 / 0.35 | 54.3 % | C3 | READABLE |
| 40.00 | 0.00 / 0.20 | 58.0 % | — | READABLE |
| **50.00** | 0.00 / 0.11 | 55.6 % | **C1** | READABLE |
| **64.00** | 0.00 / 0.05 | 63.0 % | **C1** | READABLE |
| 82.41 | 0.00 / 0.02 | 65.4 % | tone | READABLE |
| 100.00 | 0.00 / 0.01 | 69.1 % | anchor | READABLE |
| 200.00 | 0.00 / 0.00 | 85.2 % | anchor | READABLE |
| 400.00 | 0.00 / 0.00 | 92.6 % | anchor | READABLE |

⇒ **C1 (50/64 Hz) is fully reachable** — both bands, from data already on disk, taking C1 from
session 87's 0/2 anchors to a potential 2/2. **C3 is 2 of 3 reachable** (25 and 32 Hz).

⛔ **20 Hz is the one genuine stimulus limit, and it is `SWEEP_F0` itself** — the deconvolution
divides by a reference spectrum that carries no energy below 20 Hz, so the edge blows up exactly as
the `order_limit` docstring describes at the other end of the band. **No re-reading of these captures
lifts it; only lowering `SWEEP_F0` would, and that IS a re-capture.** ⇒ session 87's "stimulus change"
conclusion survives for **one band of five**, not for the item.

⚠⚠ **AND 20 Hz IS WHY THE YIELD COLUMN MUST NEVER BE READ ALONE.** Its yield (59.3 %) is the
*highest* in the C3 group — higher than 25 and 32 Hz — because the edge artefact is large and the
yield merely counts orders above a floor. A band the extractor cannot read at all still looks
well-populated. **Validity and support are different questions and neither substitutes for the other.**

⚠ The yield is an **upper bound** on the cell count: `dilution_cell` needs the anchor AND the cell to
clear the floor at the same order, and GATE 4 counts captures independently.

#### 6. ⭐⭐ TWO INSTRUMENTS AGREE AT LF — GATE 3, and it is threshold-free

The captures carry discrete tones at **82.41 and 110 Hz** (`TONE_FREQS`) at −14 dBFS, sharing no
machinery with the swept curve: different segment, different extractor, no deconvolution, no gating.
The driven sweeps **bracket** −14 dBFS at −18 and −12, so the tone's `Hn/H1` must land BETWEEN them —
a bracket, not a tuned tolerance. On `ref-od`: **12 of 12 in bracket** (both frequencies × all six
orders, H2 through H7, spanning −33 to −130 dB). ⚠ Deliberately not an equality test — the tone is a
steady 0.8 s excitation and the sweep is swept, so a nonlinearity **with memory** (the clipper's
RC-coupled Newton solve) may legitimately differ; falling inside the level bracket is the claim.

#### 7. ⚠⚠ FOUR DEFECTS IN MY OWN WORK, EACH CAUGHT BY A CHECK RATHER THAN BY INSPECTION

**(a) MY GOING-IN MECHANISM WAS REFUTED, AND THE REFUTATION IS THE USEFUL PART.** I expected the H1
gate's fixed ±0.04 s half-width (~50 Hz of Hann mainlobe = 100 % of a 50 Hz fundamental) to smear
`Hn/H1` at LF, and wrote that into the tool as the reason it might fail. It does not: order 1's "IR"
for a memoryless system is a **delta**, so an 80 ms window truncates nothing and GATE 1 reads
**0.02 dB at 32 Hz**. Frequency resolution is the wrong frame — the gate holds an impulse response,
not a tone. ⭐ And the measured errors run the *other* way (0.35 dB at 32 Hz for a 30 Hz corner
against 0.07 for a 7.2 Hz one, the shipped `c21R` corner and the longest tail in the chain), so what
the residual actually tracks is the **SLOPE of the response across the harmonic spacing**, not the
tail length. Both corners are printed.

**(b) THE TONE EXTRACTOR FAILED ITS OWN KNOWN-ANSWER GATE — and the defect is `analyze.thd`'s.**
GATE 2 caught the peak-bin convention losing **−0.09 / −0.23 / −0.44 / −0.70 / −1.03 / −1.37 dB** at
H2…H7 at 82.41 Hz, while reading **0.000 dB at 110 Hz**. Cause: a 0.8 s window bins at 1.25 Hz, so
110 Hz is **88 cycles exactly** but 82.41 Hz is **65.93** — and the fractional-bin offset is
multiplied by the order, so the peak bin loses progressively more of a Hann mainlobe as `n` rises,
arriving at the textbook 1.42 dB half-bin scalloping loss. ⭐ **The 110 Hz row passing at 0.000 dB
either way is what makes the diagnosis scalloping rather than "the LF tone is just harder".** Fixed in
the new tool by summing POWER across the mainlobe; both conventions are printed side by side so the
fix cannot read as a free choice. ⚠ **Scope, stated rather than glossed:** of the eight `TONE_FREQS`
only 82.41 Hz is non-integer-cycle at 0.8 s, and `thd_at_bands` only reaches for a discrete tone
ABOVE the Farina ceiling (~9.5 kHz), where none of these live — **so no shipped number moves.** The
convention is still wrong wherever a non-integer-cycle tone is read.

**(c) ⚠⚠ MY OWN GUARD WAS VACUOUS, AND ONLY MUTATION-TESTING IT FOUND THAT.** `ANCHOR_HZ` is used as
a **positional index** into each record's `harmonics[sweep][order][side][bi]` arrays
(`for bi, hz in enumerate(ANCHOR_HZ)`) — i.e. it is not a label, it is an unchecked claim about the
shape of a JSON written by a different tool on a different day. I added `assert_anchors_match` to
close that, and its first draft read `report["thd_anchors"]` when the field lives under
**`report["meta"]`** — so it returned `None` on every real report and fell through to its
"cannot verify" branch, **printing a warning that reads as diligence while checking nothing.** Caught
by mutating a report to 4 anchors and finding the guard did not fire. ⭐ Same class as session 80 item
(4a) and the `empty-gate-must-fail` rule: **a control that cannot fail is not a control.** Now
verified in BOTH directions — the mutant exits with an actionable message, and the real report passes
while reproducing every recorded number to the digit.

**(d) I TESTED ONE OF C3's THREE BANDS AND WAS ABOUT TO REPORT A C3 VERDICT.** The first draft's
`TEST_HZ` started at 32 Hz. Adding **20 and 25 Hz** is what produced item 5's single most important
row — 20 Hz is the one band that fails, it is one of C3's three, and it is the row that decides
whether the stimulus needs changing at all.

⚠ **A fifth, smaller one worth recording because it inverted a conclusion:** I first inferred the
yield from `ref-od` alone, where at 82 Hz only H2 clears the floor, and concluded the LF yield would
be **zero**. Over the 81-capture population it is **65 %**. **One capture is not the population** —
the same lesson session 46 recorded for `trebleLadderDampR`, one instrument over.

#### 8. ⚠ NOT CLAIMED

`THD_ANCHORS` was deliberately **NOT changed** and no report was re-rendered. Extending it re-shapes
every record's `harmonics` arrays from 3 entries to N, and **every number sessions 81–87 quote is
indexed against the 3-entry shape** — so that is a genuine baseline decision about the project's
recorded results, not a mechanical edit, and item 6(c)'s guard now refuses the mixed state rather than
reporting it silently. Nothing is proposed, no candidate screened, and **C3's size remains open and
still has two readings disagreeing in DIRECTION** (session 87 item 8). The 25/32 Hz bands being
*readable* is a statement about the instrument, not yet a measurement of C3.

#### 9. ▶ NEXT, IN ORDER

**(a)** ⛔ **DROP "this needs a stimulus change and a re-capture" — it does not**, for 4 of the 5
bands. **(b)** ⭐⭐ the concrete step is now small and fully specified: set
`comprehensive_report.THD_ANCHORS = (25, 32, 50, 64, 100, 200, 400)`, set
`a3_harmonic_axis.ANCHOR_HZ` to match, and re-render the 129-capture baseline **with `--no-cache`**
(~16–25 min at `--jobs 8`) — ⚠ **without `--no-cache` it is a silent no-op** (item 2), and ⚠ the new
report is NOT comparable to `s74_baseline129.json` by index, which is what item 6(c)'s guard now
enforces. Then re-run `a3_harmonic_axis`, `a3_axis_compare` and `a3_budget_rederive`; **C1 goes from
0/2 anchors to 2/2** and C3 from 0/3 to 2/3. **(c)** ⚠ 20 Hz stays unreachable at any anchor setting;
reaching it needs `SWEEP_F0` below 20 Hz, i.e. a real re-capture — decide that on its own merits,
**after** (b) shows what 25/32 Hz alone deliver. **(d)** the CARRIER question is unchanged: **C1 ≈
+2.7 dB flat, C2 ≈ +2.1 … +3.5 dB over 101–508 Hz, C3 ≈ +5 … +7 dB at 20 Hz (open)** — gate any
candidate on `a3_axis_compare.py --fit KEY=VALUE` alongside the 129-capture matrix. **(e)** the
even-order item still needs the WEIGHTING judgement session 84 item (a)(i) named. **(f)** session 70's
§2 rejection under session 71 (4b). **(g)** `c21R` toward 130–150k. **(h)** the A3 / GAP #3b low-mid
and ATTACK-notch depth items. **(i)** everything session 70 listed behind that.
