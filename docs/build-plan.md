...
| 8 — Full UI (polish, VU gate, headless) | **✅ DONE (2026-07-24)** | Data-driven centre pedal face (`src/ui/PedalFace.{h,cpp}` from base image + CSV): 8 knobs, 2 footswitches + LEDs, ATTACK/GRUNT icon glyphs, LO-MID/HI-MID + freq selectors, all APVTS-bound. Peripheral chrome (PedalLookAndFeel, VUMeter, halo trims + Trim Link, OS/scale strip). **User-confirmed working 2026-07-24.** | UI functional + bound; user interaction pass |
| 9 — Reference validation + performance pass | **▶ IN PROGRESS (2026-07-24)** | Phase-7 calibration LANDED (session 17, `e7a4d83`); now A/B the full chain vs the capture matrix to close the remaining sound gap ("not yet matching the real pedal" per user). Harness ready (`analyze.py`, `comprehensive_report.py`, `farina_validate.py`, `gap_audit.py`). | A/B vs capture matrix (FR / swept-THD / null / knob-tracking); HQ decision |
| 10 — Final sweep + release | Not started | — | — |
